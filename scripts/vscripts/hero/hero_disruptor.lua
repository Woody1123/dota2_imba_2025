CreateTalents("npc_dota_hero_disruptor", "hero/hero_disruptor.lua")
...
function imba_disruptor_thunder_strike:OnSpellStart()
	...
	for i=1, #enemy do
		if  caster:Has_Aghanims_Shard()  then
			enemy[i]:AddNewModifier_RS(caster, self, "modifier_silence", {duration=3})
		end
...
function modifier_imba_thunder_strike:IsPurgable() 			return not self:GetCaster():TG_HasTalent("special_bonus_imba_disruptor_3") end
function modifier_imba_thunder_strike:IsPurgeException() 	return not self:GetCaster():TG_HasTalent("special_bonus_imba_disruptor_3") end
...
function modifier_imba_thunder_strike:OnCreated(keys)
	...
	if IsServer() then
		...
		if not keys.strikes then
			self:SetStackCount(self:GetAbility():GetSpecialValueFor("strikes") + self:GetCaster():TG_GetTalentValue("special_bonus_imba_disruptor_2"))
...
function modifier_imba_thunder_strike:OnIntervalThink()
	...
	for i=1, #enemy do
		ApplyDamage({victim = enemy[i], attacker = caster, ability = ability, damage = ability:GetSpecialValueFor("strike_damage") + caster:TG_GetTalentValue("special_bonus_imba_disruptor_1"), damage_type = ability:GetAbilityDamageType()})
	end
end
...
function imba_disruptor_glimpse:OnSpellStart()
	...
	if  caster:Has_Aghanims_Shard()  then
		target:AddNewModifier_RS(caster, self, "modifier_silence", {duration=3})
		local ab=caster:FindAbilityByName("imba_disruptor_thunder_strike")
		if ab and ab:GetLevel() then
			target:AddNewModifier(caster, ab, "modifier_imba_thunder_strike", {})
		end
	end
...
function imba_disruptor_kinetic_field:GetCooldown(iLevel) return (self.BaseClass.GetCooldown(self, iLevel) + self:GetCaster():TG_GetTalentValue("special_bonus_imba_disruptor_4")) end

function imba_disruptor_kinetic_field:OnSpellStart()
	local caster = self:GetCaster()
	local pos = self:GetCursorPosition()
	local sound = CreateModifierThinker(caster, self, "modifier_dummy_thinker", {duration = self:GetSpecialValueFor("formation_time") + self:GetSpecialValueFor("duration") + self:GetCaster():TG_GetTalentValue("special_bonus_imba_disruptor_5") + FrameTime()}, pos, caster:GetTeamNumber(), false)
...
function modifier_imba_kinetic_field_delay:OnDestroy()
	if IsServer() then
		CreateModifierThinker(self:GetCaster(), self:GetAbility(), "modifier_imba_kinetic_field_thinker", {duration = self:GetAbility():GetSpecialValueFor("duration") + self:GetCaster():TG_GetTalentValue("special_bonus_imba_disruptor_5"), sound = self.sound}, self:GetParent():GetAbsOrigin(), self:GetCaster():GetTeamNumber(), false)
		AddFOWViewer(self:GetCaster():GetTeamNumber(), self:GetParent():GetAbsOrigin(), self:GetAbility():GetSpecialValueFor("radius"), self:GetAbility():GetSpecialValueFor("duration") + self:GetCaster():TG_GetTalentValue("special_bonus_imba_disruptor_5"), false)
		self.sound = nil
	end
end
...
function modifier_imba_static_storm_debuff:CheckState() return {[MODIFIER_STATE_SILENCED] = true, [MODIFIER_STATE_MUTED] = self:GetCaster():HasScepter(), [MODIFIER_STATE_PASSIVES_DISABLED] = self:GetCaster():TG_HasTalent("special_bonus_imba_disruptor_6")} end
