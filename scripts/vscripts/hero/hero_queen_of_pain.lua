...
function imba_queenofpain_shadow_strike:AllowIllusionDuplicate()
    return false
end
function imba_queenofpain_shadow_strike:OnSpellStart(bAttack)
	...
	if not bAttack and caster:TG_HasTalent("special_bonus_imba_queenofpain_2") then
		local radius = caster:TG_GetTalentValue("special_bonus_imba_queenofpain_2")+caster:GetCastRangeBonus()
...
function modifier_imba_shadow_strike_base_slow:OnCreated()
	...
	if IsServer() then
		...
		local hp=math.floor(self:GetAbility():GetSpecialValueFor("strike_damage")/2)
		self:GetCaster():Heal(hp, self:GetCaster())
...
function modifier_imba_shadow_strike_base_slow:OnIntervalThink()
	...
	local duration_damage=self:GetAbility():GetSpecialValueFor("duration_damage")
	local damage=duration_damage*self:GetStackCount()
	local damageTable = {
						victim = self:GetParent(),
						attacker = self:GetCaster(),
						damage = damage,
	...
	self:GetCaster():Heal(math.floor(damage/2), self:GetCaster())
	SendOverheadEventMessage(self:GetCaster(), OVERHEAD_ALERT_HEAL, self:GetCaster(),math.floor(damage/2), nil)
	ApplyDamage(damageTable)
	SendOverheadEventMessage(nil, OVERHEAD_ALERT_BONUS_POISON_DAMAGE, self:GetParent(), damage, nil)
end

function modifier_imba_shadow_strike_base_slow:OnRefresh()
	if not IsServer() then
		return
	end
	...
	if self.pfx2 then
		...
		local ten=math.floor(stack/10)
		local bits=math.mod(stack,10)
		if stack>19 then
			ten=1
			bits=9
		end
		ParticleManager:SetParticleControl(self.pfx2, 1, Vector(ten, bits, 0))
	end
end
...
function modifier_imba_shadow_strike_passive:AllowIllusionDuplicate()
    return false
end
...
function modifier_imba_shadow_strike_passive:OnAttackLanded(keys)
	...
	if keys.attacker ~= self:GetParent() or keys.target:IsMagicImmune() or not IsEnemy(keys.target, keys.attacker) or keys.target:IsBuilding() or keys.target:IsOther() or keys.target:IsCourier() or not keys.target:IsAlive() or self:GetParent():IsIllusion() then
		return
	end
	...
	--end
	keys.target:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_imba_shadow_strike_base_slow", {duration = self:GetAbility():GetSpecialValueFor("duration_tooltip")})
end
...
function modifier_imba_confusion:CheckState()
		return {
			[MODIFIER_STATE_SILENCED] =(self:GetRemainingTime()>1.25 and self:GetCaster():TG_HasTalent("special_bonus_imba_queenofpain_4")) ,}
end
...
function modifier_imba_delightful_torment:OnCreated()
	if not IsServer() then
		return
	end
	self.non_trigger_inflictors={
			["item_radiance_v2"] = true,
			["imba_queenofpain_shadow_strike"]= true,
			["item_blade_mail"]= true,
			["item_veil_of_evil"]= true,
	}
end
...
function modifier_imba_delightful_torment:OnTakeDamage(keys)
	...
	if keys.attacker == self:GetParent() and keys.unit:IsTrueHero() then
		...
		local current_ability = self:GetParent():FindAbilityByName("imba_queenofpain_sonic_wave")
	...
	if keys.attacker == self:GetParent() and self:GetParent():TG_HasTalent("special_bonus_imba_queenofpain_1") and not keys.unit:IsMagicImmune() and not keys.unit:IsBuilding() and not keys.unit:IsCourier() and not keys.unit:IsOther() and not self:GetParent():IsIllusion() then
		...
		--print(keys.inflictor:GetName())

		if self:GetParent():HasModifier("modifier_imba_shadow_strike_passive") and (not keys.inflictor or not self.non_trigger_inflictors[keys.inflictor:GetName()]) and  keys.damage_category~=1 then
			local shadow_strike=self:GetParent():FindAbilityByName("imba_queenofpain_shadow_strike")
			keys.unit:AddNewModifier(self:GetCaster(), shadow_strike, "modifier_imba_shadow_strike_base_slow", {duration = shadow_strike:GetSpecialValueFor("duration_tooltip")})
			--self:GetParent():SetCursorCastTarget(keys.unit)
			--self:GetParent():FindAbilityByName("imba_queenofpain_shadow_strike"):OnSpellStart(true)
		end
...
function imba_queenofpain_sonic_wave:OnSpellStart()
	...
	if caster:TG_HasTalent("special_bonus_imba_queenofpain_3") then
		--eggs
...
-- function imba_queenofpain_sonic_wave:OnUpgrade()
-- 	if IsServer() then
-- 		local caster = self:GetCaster()
-- 		local ability = caster:FindAbilityByName("imba_queenofpain_delightful_torment")
-- 		ability:SetLevel(1)
-- 	end
-- end
...
function imba_queenofpain_sonic_wave:OnProjectileHit_ExtraData(target, location, keys)
	if target and not IsInTable(target, EntIndexToHScript(keys.thinker).hitted) then
		...
		local caster = self:GetCaster()
		local dmg = caster:HasScepter() and self:GetSpecialValueFor("damage_scepter") or self:GetSpecialValueFor("damage")
		local ability = caster:FindAbilityByName("imba_queenofpain_scream_of_pain")
		...
		if not target:IsBossCreature() then
			target:AddNewModifier_RS(caster, self, "modifier_imba_sonic_wave_daze", {duration = self:GetSpecialValueFor("debuff_duration")})
		end

		local duration_damage=0
		if caster:HasAbility("imba_queenofpain_shadow_strike")then
			duration_damage=caster:FindAbilityByName("imba_queenofpain_shadow_strike"):GetSpecialValueFor("duration_damage")
		end
		if target:HasModifier("modifier_imba_shadow_strike_base_slow") then
			local buff=target:FindModifierByName("modifier_imba_shadow_strike_base_slow")
			dmg=dmg+buff:GetStackCount()*duration_damage*math.floor(buff:GetRemainingTime()/1.5)*self:GetSpecialValueFor("shadow_Mag")/100
			target:RemoveModifierByName("modifier_imba_shadow_strike_base_slow")
		end
