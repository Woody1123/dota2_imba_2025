...
function modifier_imba_static_remnant_thinker:OnCreated()
	...
	if IsServer() then
		...
		AddFOWViewer(self:GetCaster():GetTeamNumber(), self:GetParent():GetAbsOrigin(), self:GetAbility():GetSpecialValueFor("static_remnant_radius"), 1.6, false)
...
function modifier_imba_static_remnant_thinker:OnIntervalThink()
	...
	AddFOWViewer(caster:GetTeamNumber(), self:GetParent():GetAbsOrigin(), ability:GetSpecialValueFor("static_remnant_radius"), 1.6, false)
...
function imba_storm_spirit_electric_vortex:OnSpellStart()
	...
	if not caster:HasScepter() then
		if target and target:TG_TriggerSpellAbsorb(self)  then
			return
		end
	end
...
imba_storm_spirit_overload = class({}) 
...
function modifier_imba_ball_lightning_travel:OnIntervalThink()
	...
	local next_pos = GetGroundPosition(self.current_pos + self.direction * distance, nil)
