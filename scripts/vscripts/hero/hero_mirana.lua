...
LinkLuaModifier("modifier_imba_starfall_debuff_2", "hero/hero_mirana", LUA_MODIFIER_MOTION_NONE)
...
function imba_mirana_starfall:OnSpellStart()
	...
	for _,enemy in pairs(enemies) do
		enemy:AddNewModifier(caster, self, "modifier_imba_starfall_debuff", {duration = self:GetSpecialValueFor("secondary_duration")+0.5})
...
function modifier_imba_starfall_debuff:OnDestroy()
	if IsServer() then
		...
		for _,enemy in pairs(enemies) do
			enemy:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_imba_starfall_debuff_2", {duration = self:GetAbility():GetSpecialValueFor("secondary_duration")+2})
			StarFallAttack(self:GetAbility(), self:GetCaster(), enemy, self:GetAbility():GetSpecialValueFor("damage"))
		end
	end
end

modifier_imba_starfall_debuff_2 = class({})

function modifier_imba_starfall_debuff_2:IsDebuff()			return true end
function modifier_imba_starfall_debuff_2:IsHidden() 			return false end
function modifier_imba_starfall_debuff_2:IsPurgable() 		return true end
function modifier_imba_starfall_debuff_2:IsPurgeException() 	return true end
function modifier_imba_starfall_debuff_2:GetAttributes() return MODIFIER_ATTRIBUTE_MULTIPLE end
function modifier_imba_starfall_debuff_2:GetEffectName() return "particles/hero/mirana/mirana_cosmic_dust.vpcf" end
function modifier_imba_starfall_debuff_2:GetEffectAttachType() return PATTACH_OVERHEAD_FOLLOW end
function modifier_imba_starfall_debuff_2:ShouldUseOverheadOffset() return true end
function modifier_imba_starfall_debuff_2:OnDestroy()
	if IsServer() then
		local enemies = FindUnitsInRadius(self:GetCaster():GetTeam(), self:GetParent():GetAbsOrigin(), nil, self:GetAbility():GetSpecialValueFor("secondary_radius"), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NO_INVIS, FIND_ANY_ORDER, false)
		for _,enemy in pairs(enemies) do
			StarFallAttack(self:GetAbility(), self:GetCaster(), enemy, self:GetAbility():GetSpecialValueFor("damage"))
...
function imba_mirana_arrow:OnSpellStart()
	local caster = self:GetCaster()
	if  caster:GetAbsOrigin().z < -1000 then
		return
	end
...
function imba_mirana_arrow:OnProjectileHit_ExtraData(target, location, keys)
	...
	if distance > self:GetSpecialValueFor("arrow_max_stunrange") then
		...
		local dmg_add = 0
		if kill_creeps then 
			dmg_add = self:GetSpecialValueFor("base_damage")*(distance - self:GetSpecialValueFor("arrow_max_stunrange")) / 1000
		end
	
		dmg = dmg + dmg_add+target:GetMaxHealth() * (pct_dmg / 100)
	end
