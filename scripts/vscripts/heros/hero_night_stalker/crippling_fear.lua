...
LinkLuaModifier("modifier_crippling_fear_buff", "heros/hero_night_stalker/crippling_fear.lua", LUA_MODIFIER_MOTION_NONE)
...
function crippling_fear:OnSpellStart()
    ...
    if caster:IsAlive() then
        ...
        caster:AddNewModifier(caster, self, "modifier_crippling_fear", {duration = dur+caster:TG_GetTalentValue("special_bonus_night_stalker_2"),rd=rd})
		 caster:AddNewModifier(caster, self, "modifier_crippling_fear_buff", {duration = dur+caster:TG_GetTalentValue("special_bonus_night_stalker_2"),rd=rd})
    end
end
modifier_crippling_fear_buff=class({})

function modifier_crippling_fear_buff:IsHidden() 			
    return true 
end

function modifier_crippling_fear_buff:IsPurgable() 			
    return false 
end

function modifier_crippling_fear_buff:IsPurgeException() 	
    return false 
end

function modifier_crippling_fear_buff:RemoveOnDeath() 	
    return true 
end
function modifier_crippling_fear_buff:DeclareFunctions()
    return
    {
...
function modifier_crippling_fear_buff:OnCreated()	
...
function modifier_crippling_fear_buff:OnRefresh()	
	self:OnCreated()
end
function modifier_crippling_fear_buff:GetModifierDodgeProjectile()
    if RollPseudoRandomPercentage(20*self:GetStackCount(),0,self:GetParent()) then
        return 1
    else
        return 0
    end
end
function modifier_crippling_fear_buff:OnAttackLanded(keys)
