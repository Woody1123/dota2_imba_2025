...
function mirror_image:OnSpellStart()
    ...
    caster.mirror_imageillusions=CreateIllusions(caster, caster, modifier, self:GetSpecialValueFor("images_count"), 0, true, true)
    for _, target in pairs(caster.mirror_imageillusions) do
        ...
        --给分身添加攻速书BUFF
		target:SetBaseAgility(caster:GetAgility()*1.2)
		target:SetBaseStrength(caster:GetStrength()*1.2)
		local item = caster:FindItemInInventory("item_bkb") 
		if item then
			caster:EmitSound("DOTA_Item.BlackKingBar.Activate")
			target:AddNewModifier(caster,item,"modifier_item_bkb_buff",{duration = 7})
		end
    end
end
