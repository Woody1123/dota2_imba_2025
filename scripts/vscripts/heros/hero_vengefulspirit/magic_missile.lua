...
function magic_missile:OnSpellStart()
    ...
    local sp=self:GetSpecialValueFor( "magic_missile_speed" )+500
    self.num=math.floor(caster:GetMaxMana()/self:GetSpecialValueFor( "mana" ))+1
...
function modifier_magic_missile_buff:OnDeath(tg)
	if IsServer() then
		...
		print(tg.unit:GetName())
		if  tg.unit==self.caster and tg.unit:GetName() == "npc_dota_hero_vengefulspirit" then
