...
function repel:OnSpellStart(tg,half)
	...
	if target == caster then
		caster:Purge(false, true, false, true, true)
        caster:AddNewModifier(caster, self, "modifier_repel_buff", {duration=dur*2})
	end
...
function modifier_repel_buff:GetModifierStatusResistanceStacking()
    return self.re
end
...
function modifier_repel_buff:OnCreated(tg)
	...
	if self.ability ~= nil then
		self.r = self.ability:GetSpecialValueFor("status_resistance")
		if self.caster == self.parent then
		self.re = self.r*2 else
		self.re = self.r
		end
	end
