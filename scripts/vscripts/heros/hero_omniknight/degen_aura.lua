...
function modifier_degen_aura_pa:DeclareFunctions()
    return
    {
...
function modifier_degen_aura_pa:OnAttackLanded(keys)
	if not IsServer() then
		return
	end
	if keys.attacker ~= self:GetParent() or self:GetParent():IsIllusion() or not self.ability or not keys.target:IsAlive() then
		return
	end
	if PseudoRandom:RollPseudoRandom(self:GetAbility(), self:GetAbility():GetSpecialValueFor("chance")) then
...
function modifier_degen_aura_pa:OnCreated()
    if IsServer() then
		...
		self.ability = self:GetParent():FindAbilityByName("omniknight_hammer_of_purity")
    end
end
