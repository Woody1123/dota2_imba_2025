            ...
            if heros and heros[num1] then 
                if Is_Chinese_TG(heros[num1],caster) then    
                    ...
                    if heros[num1]:IsRealHero() then
						...
						unit:AddNewModifier(caster, self, "modifier_unit_remove", {duration=howl_duration})
...
function modifier_howl_buff:OnCreated() 	
	...
	self.int=self.ability:GetSpecialValueFor("vision_duration")
	self.vision_d = true
	if IsServer() then
...
function modifier_howl_buff:OnIntervalThink() 	
	if IsServer() then
...
function modifier_howl_buff:GetBonusNightVision() 	
	return self:GetStackCount()==1 and self.vision or 0
end
