...
function freezing_field:OnProjectileHit_ExtraData(target, location,kv)
        ...
        if not target:IsMagicImmune() then
            target:AddNewModifier_RS(caster,self,"modifier_stunned",{duration=1.5})
                        local damageTable = {
                            attacker = caster,
                            victim = target,
...
function modifier_freezing_field:OnCreated()
    ...
    self.damage=self:GetAbility():GetSpecialValueFor("damage")+self:GetCaster():GetIntellect(false)
    self.explosion_radius=self:GetAbility():GetSpecialValueFor("explosion_radius")
	
	if self:GetParent() == self.caster then
		--print("123")
		self.explosion_radius = self.explosion_radius + 150
    ...
    self.TEAM=self:GetParent():GetTeamNumber()
        self.fx = ParticleManager:SafeCreateParticle("particles/econ/items/crystal_maiden/crystal_maiden_maiden_of_icewrack/maiden_freezing_field_snow_arcana1.vpcf", PATTACH_ABSORIGIN_FOLLOW , self:GetParent())
...
function modifier_freezing_field:OnIntervalThink()
        ...
        local fx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_crystalmaiden/maiden_freezing_field_explosion.vpcf", PATTACH_CUSTOMORIGIN, nil)
...
function modifier_freezing_field_move:OnCreated(tg)
	...
	if IsServer() then
		self.direction=ToVector(tg.dir)
            ...
            local particle = ParticleManager:SafeCreateParticle("particles/econ/courier/courier_donkey_ti7/courier_donkey_ti7_ambient.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
            self:AddParticle( particle, false, false, 20, false, false )
            local particle1 = ParticleManager:SafeCreateParticle("particles/econ/events/ti7/teleport_end_ti7_lvl3_rays.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
