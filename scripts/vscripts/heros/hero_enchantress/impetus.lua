...
function modifier_impetus:OnAttackLanded(tg)
    ...
    if tg.attacker == self.parent  then
		...
		local max_dis = self.parent:TG_HasTalent("special_bonus_enchantress_2") and 1500 or 600
		local dis = TG_Distance(tg.target:GetAbsOrigin(),tg.attacker:GetAbsOrigin())
        if dis>=1050 and  self.parent:TG_HasTalent("special_bonus_enchantress_4") then
                 tg.target:AddNewModifier(self.parent,self.ability,"modifier_imba_stunned",{duration=0.2})
        end
		dis=math.min(dis,max_dis)
