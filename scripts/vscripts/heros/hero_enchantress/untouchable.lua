...
function modifier_untouchable:OnAttackStart(tg)
    ...
    if tg.target == self.parent and not self.parent:IsIllusion() and self.ability:GetLevel()>0  then
        ...
        if ( self.parent:PassivesDisabled() and not self.parent:TG_HasTalent("special_bonus_enchantress_7")  ) or tg.attacker:IsBuilding() or tg.attacker==self.parent then
		    return
        end
			
                tg.attacker:AddNewModifier(self.parent, self.ability, "modifier_untouchable_debuff", {duration=5})
                if not tg.attacker:IsMagicImmune() and RollPseudoRandomPercentage(self.chance,0,self.parent) then
