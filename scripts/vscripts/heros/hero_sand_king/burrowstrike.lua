...
function burrowstrike:OnSpellStart()
    ...
    if dis>800 or (caster:HasModifier("modifier_sand_storm_inv")) then
		...
		null1:AddNewModifier(caster, self, "modifier_unit_remove", {duration = t1})
		...
		null2:AddNewModifier(caster, self, "modifier_unit_remove", {duration = t1})
    end
...
function modifier_burrowstrike:OnCreated(tg)
    ...
    local fx = ParticleManager:SafeCreateParticle("particles/econ/items/sand_king/sandking_barren_crown/sandking_rubyspire_burrowstrike.vpcf", PATTACH_CUSTOMORIGIN, self.parent)
