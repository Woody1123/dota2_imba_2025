...
function modifier_caustic_finale_debuff:OnDeath(tg)
    ...
    if tg.unit == self.parent then
		...
		if result then
			local mod = self:GetCaster():FindModifierByName("modifier_caustic_finale")
			if mod then
				mod:SetStackCount(math.min(mod:GetStackCount()+1,self:GetCaster():GetLevel()*2))
			end
		end
            ...
            if #targets>0 then    
                for _, target in pairs(targets) do
                    local damage= {
                        ...
                        damage = (self.caustic_finale_damage_pct+(result and self.pct or 0 ))*0.01*self.parent:GetMaxHealth(),
                        damage_type = DAMAGE_TYPE_MAGICAL,
                        ability = self.ability,
                        }
                    ApplyDamage(damage)
