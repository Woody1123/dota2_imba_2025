...
function epicenter:OnSpellStart()
    print("[Epicenter] OnSpellStart triggered")
    local caster = self:GetCaster()
...
function modifier_epicenter:IsPurgable() return false end
function modifier_epicenter:IsPurgeException() return false end
function modifier_epicenter:IsHidden() return true end
...
function modifier_epicenter:OnIntervalThink()
    ...
    if #targets > 0 then
        for _, target in pairs(targets) do
            if not target:IsMagicImmune() or (target:IsMagicImmune() and (target:GetAbsOrigin() - casterPos):Length2D() <= self.epicenter_radius) then
                if not target:HasModifier("modifier_black_hole_debuff2") and target:IsAlive() then
                    target:AddNewModifier(self.caster, self.ability, "modifier_epicenter_m", {duration = 0.12, pos = casterPos})
                end
...
function modifier_epicenter_s:IsPurgable() return false end
function modifier_epicenter_s:IsPurgeException() return false end
function modifier_epicenter_s:IsHidden() return true end
function modifier_epicenter_s:AllowIllusionDuplicate() return false end
...
function modifier_epicenter_m:IsDebuff() return true end
function modifier_epicenter_m:IsHidden() return false end
function modifier_epicenter_m:IsPurgable() return false end
function modifier_epicenter_m:IsPurgeException() return false end
function modifier_epicenter_m:RemoveOnDeath() return false end
function modifier_epicenter_m:GetMotionPriority() return DOTA_MOTION_CONTROLLER_PRIORITY_HIGH end
