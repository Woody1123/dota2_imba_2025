...
function stampede:OnSpellStart()
	...
	if self:GetCooldownTimeRemaining() < 30 then
		self:EndCooldown()
...
function modifier_stampede_buff:OnCreated(tg)
    ...
    self.damageoutgoing=self:GetAbility():GetSpecialValueFor( "damageoutgoing" )*-1
...
function modifier_stampede_buff:OnIntervalThink()
    ...
    local heros = FindUnitsInRadius(
        ...
        for _, hero in pairs(heros) do
            if not hero:IsMagicImmune() and not hero:HasModifier("modifier_paralyzed") then
                    hero:AddNewModifier(self:GetParent(),self:GetAbility(), "modifier_paralyzed", {duration=self.dur})
            end
        end
    end
end
