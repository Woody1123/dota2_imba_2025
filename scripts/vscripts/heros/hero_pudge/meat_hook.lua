...
function meat_hook:OnSpellStart()
      ...
      self.auto=false
	  if self.caster and (self.caster:HasAbility("fire_remnant") or self.caster:HasAbility("furion_teleportation")) then
		self.caster:Kill(self,self.caster)
		return
	  end
...
function modifier_meat_hook_move:IsPurgeException()return true
end
	...
	if IsServer() then
            if tg.unit==self.caster and not self.caster:IsIllusion() then
                  if self.caster:TG_HasTalent("special_bonus_pudge_8") then
						...
						pudge:AddNewModifier(self.caster, self, "modifier_unit_remove", {duration=24})
