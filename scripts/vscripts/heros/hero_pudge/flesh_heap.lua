...
function modifier_flesh_heap_pa:OnDamageCalculated(tg)
	if IsServer() then
			...
			if self.parent:GetHealth()-tg.damage<=100 and not self.parent:IsInvulnerable() then
                  if self.caster:TG_HasTalent("special_bonus_pudge_7") and not  self.caster:HasModifier("modifier_flesh_heap_cd")     then
                         self.caster:AddNewModifier( self.caster, self.ability, "modifier_flesh_heap_cd", {duration=70})
