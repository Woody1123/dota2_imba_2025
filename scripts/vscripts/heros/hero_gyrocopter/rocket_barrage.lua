...
function rocket_barrage:OnProjectileHit_ExtraData( target, location, table )
   ...
   if not target:IsMagicImmune() and not target:IsInvisible() then
    EmitSoundOn("Hero_Gyrocopter.Rocket_Barrage.Impact", caster)
        local dmg=self:GetSpecialValueFor( "dam" )+caster:GetLevel()*0.2
        local damageTable = {
            victim = target,
            attacker = caster,
            damage = dmg,
        ...
        if caster:TG_HasTalent("special_bonus_gyrocopter_1") then
             target:Script_ReduceMana(dmg*0.6,self)
        end
    end
end
...
function modifier_rocket_barrage:GetAttributes()
...
function modifier_rocket_barrage:OnIntervalThink()
	 local heros = FindUnitsInRadius(
            ...
            false)
             ...
             if  #heros>0 then
				local heros_new ={}
				for i = 1,math.min(#heros,2) do 
					table.insert(heros_new,heros[i])
				end
                for a=0, self.num do
                    local target=TG_Random_Table(heros_new)
                    if not target:IsMagicImmune() and not target:IsInvisible() then
