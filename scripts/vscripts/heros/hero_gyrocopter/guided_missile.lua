...
function guided_missile:OnSpellStart()
    ...
    Timers:CreateTimer(0, function()
        ...
        local pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_gyrocopter/gyro_calldown_first.vpcf", PATTACH_CUSTOMORIGIN, nil)
...
function modifier_guided_missile:OnCreated(tg)
    ...
    Timers:CreateTimer(0.2, function()
        local particle= ParticleManager:SafeCreateParticle("particles/units/heroes/hero_gyrocopter/gyro_homing_missile_fuse.vpcf", PATTACH_ABSORIGIN_FOLLOW,self.missile)
        ParticleManager:SetParticleControl(particle, 0, Vector(0,0,100))
        ParticleManager:ReleaseParticleIndex( particle )
        return nil
    end)
    local particle2= ParticleManager:SafeCreateParticle("particles/econ/items/gyrocopter/hero_gyrocopter_gyrotechnics/gyro_guided_missile_target.vpcf", PATTACH_OVERHEAD_FOLLOW,self.target)
    self:AddParticle(particle2, false, false, 10, false, false)
    self:StartIntervalThink(3)
end

function modifier_guided_missile:OnIntervalThink()
        local particle3= ParticleManager:SafeCreateParticle("particles/econ/items/gyrocopter/hero_gyrocopter_gyrotechnics/gyro_guided_missile.vpcf", PATTACH_CUSTOMORIGIN,self.missile)
        ...
        Timers:CreateTimer(0, function()
            if  self.die then
             ...
             if TG_Distance( self.missile:GetAbsOrigin(),self.target:GetAbsOrigin())<=100 and  self.die then
                ...
                local particle4= ParticleManager:SafeCreateParticle("particles/econ/items/gyrocopter/hero_gyrocopter_gyrotechnics/gyro_guided_missile_explosion.vpcf", PATTACH_CUSTOMORIGIN,self.target)
