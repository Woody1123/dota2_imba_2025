...
function dragon_slave:OnSpellStart()
      ...
      local dragon_slave_distance=self:GetSpecialValueFor( "dragon_slave_distance" )+self:GetCaster():GetCastRangeBonus()
      if  dis<=0 then
	...
	if target==nil then
      ...
      if kv.array~=nil and kv.array==1 then
               if  target:IsHero() then
                  ...
                  local fx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_lina/lina_spell_light_strike_array_ray_team.vpcf", PATTACH_CUSTOMORIGIN, nil)
                  ParticleManager:SetParticleControl(fx, 0, tpos)
                  ParticleManager:SetParticleControl(fx, 1, Vector(250, 1,100))
                  ParticleManager:ReleaseParticleIndex(fx)
                  Timers:CreateTimer(0.3, function()
                  local fx1 = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_lina/lina_spell_light_strike_array.vpcf", PATTACH_CUSTOMORIGIN, nil)
                  ...
                  if #units>0 then
                              ...
                              for _,target in pairs(units) do
                                    if not target:IsMagicImmune() then
										  ...
										  local damage = self:GetCaster():GetIntellect(false)*self:GetSpecialValueFor("int_damage")
										  local damageTable =
												{
													  victim = target,
