...
function fiery_soul:OnSpellStart()
      ...
      local fx = ParticleManager:SafeCreateParticle("particles/tgp/lina/bird_m.vpcf", PATTACH_CUSTOMORIGIN, nil)
      ...
      local dmg=self.caster:GetIntellect(false)*2
      ...
      EmitSoundOn("Ability.PreLightStrikeArray", self.caster)
       Timers:CreateTimer(0.7, function()
                  local fx2 = ParticleManager:SafeCreateParticle( "particles/heros/axe/shake.vpcf", PATTACH_ABSORIGIN_FOLLOW ,self.caster)
...
function modifier_fiery_soul_pa:OnAttackLanded(tg)
            ...
            if tg.attacker == self.parent  and not tg.target:IsBuilding() and not self.parent:IsIllusion() and  self.parent:TG_HasTalent("special_bonus_lina_4")   then
                        ...
                        self.damageTable.damage = self.parent:GetIntellect(false)
                        ApplyDamage(self.damageTable)
            end
end
