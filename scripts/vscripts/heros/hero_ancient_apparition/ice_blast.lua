...
function ice_blast:OnProjectileHit_ExtraData(target, location,kv)
    ...
    local rd = self:GetSpecialValueFor( "rd" )
	local dam = self:GetSpecialValueFor( "dam" )*(1+math.min(self:GetCaster():GetSpellAmplification(false)*0.5,0.8))
	local spdur = self:GetSpecialValueFor( "dur_sp" )
	local px= ParticleManager:SafeCreateParticle("particles/units/heroes/hero_ancient_apparition/ancient_apparition_ice_blast_explode.vpcf", PATTACH_CUSTOMORIGIN,target)
    ...
    local heros = FindUnitsInRadius(
        ...
        for _, hero in pairs(heros) do
            ...
            caster:AddNewModifier(caster, self, "modifier_ice_blast_buff", {duration=6})
        end
    end
...
function modifier_ice_blast_debuff:OnDestroy()
	...
	if IsServer() and self:GetParent():IsAlive() then
		...
		if self:GetParent():GetHealth()/self:GetParent():GetMaxHealth() < 0.15 then
...
function modifier_ice_blast_debuff2:OnDestroy()
	if IsServer() and self:GetParent():IsAlive() then
		if self:GetParent():GetHealth()/self:GetParent():GetMaxHealth() < 0.15 then
			TG_Kill(self:GetCaster(), self:GetParent(), self:GetAbility())
		end
