...
function reverse_polarity:OnSpellStart()
            ...
            local heros = FindUnitsInRadius(
                  ...
                  if #heros>0 then
                        for _, hero in pairs(heros) do
                              if hero:GetModelScale()<self.caster:GetModelScale() and not hero:HasModifier("modifier_dfinv") and not hero:IsMagicImmune() then
		...
		ParticleManager:ReleaseParticleIndex(fx)
            local heros = FindUnitsInRadius(
                  ...
                  if #heros>0 then
                        for _, hero in pairs(heros) do
									if not hero:HasModifier("modifier_dfinv") then
...
function reverse_polarity:OnProjectileHit_ExtraData(target, location, kv)
	if target==nil then
       ...
       if not target:IsMagicImmune() then
            target:AddNewModifier_RS(self.caster, self, "modifier_imba_bashed", {duration=self.stun})
      end
end
