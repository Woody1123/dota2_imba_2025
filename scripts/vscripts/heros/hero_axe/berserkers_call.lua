...
function berserkers_call:GetCooldown(iLevel)
...
function berserkers_call:OnSpellStart()
    ...
    local units=FindUnitsInRadius(self.caster:GetTeamNumber(),self.caster:GetAbsOrigin(),nil,self.radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_ANY_ORDER, false)
     if #units>0 then
            ...
            for _,target in pairs(units) do
                target:AddNewModifier_RS(self.caster, self, "modifier_axe_berserkers_call", {duration = stun_time})
                    if #units>=num or self.caster:TG_HasTalent("special_bonus_axe_1")  then
                        self.caster:AddNewModifier(self.caster, self, "modifier_item_bkbs_buff", {duration = self:GetSpecialValueFor("buff_defense_time")})
                    end
                    if ab and self.caster:HasScepter() and not target:IsMagicImmune() then
  ...
  --防御buff
    ...
    function modifier_berserkers_call_axe_buff:OnCreated()
        ...
        self.armor=math.max(0,(self:GetAbility():GetSpecialValueFor( "armor") - self:GetCaster():GetPhysicalArmorValue(false)))
        self.magica=self:GetAbility():GetSpecialValueFor( "mr")
    end
