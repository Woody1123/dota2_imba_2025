...
function culling_blade:OnSpellStart()
    ...
    if curtar:GetHealth() <= self:GetSpecialValueFor("kill_t")+self:GetSpecialValueFor("max_hp")*0.01 * self.caster:GetMaxHealth()+self.caster:GetPhysicalArmorValue(false)*self:GetSpecialValueFor("armor_res") then
            ...
            local particle = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_axe/axe_culling_blade_kill.vpcf", PATTACH_CUSTOMORIGIN,  self.caster)
            ...
            local particle2 = ParticleManager:SafeCreateParticle("particles/heros/axe/axe_cb1.vpcf", PATTACH_CUSTOMORIGIN,  self.caster)
            ...
            ApplyDamage(damage)
            local particles = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_axe/axe_culling_blade.vpcf", PATTACH_ABSORIGIN_FOLLOW, curtar)
