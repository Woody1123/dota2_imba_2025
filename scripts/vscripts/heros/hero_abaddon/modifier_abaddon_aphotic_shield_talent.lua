...
function modifier_abaddon_aphotic_shield_talent:OnCreated(tg)
    ...
    if IsServer() then			
        ...
        if ab and ab:GetLevel()>0 then 
            AbilityChargeController:ChangeChargeAbilityConfig(ab, ab:GetCooldown(ab:GetLevel()), 1+self:GetParent():TG_GetTalentValue("special_bonus_unique_own_abaddon_6"), 1, true, true)
        end
    end
end