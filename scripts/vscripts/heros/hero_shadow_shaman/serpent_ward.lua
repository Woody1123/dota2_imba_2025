...
function serpent_ward:OnSpellStart()
   ...
   local NPC={}
    ...
    NPC[1].serpent_ward5=true
    for _,target in pairs(NPC) do
		target:FindAbilityByName("lone_druid_spirit_bear_entangle"):SetLevel( 1 )
        target:AddNewModifier(caster, self, "modifier_kill", {duration =dur})
		target:AddNewModifier(caster, self, "modifier_unit_remove", {duration =dur})
       ...
       -- target:FindAbilityByName("oldtroll_fervor"):SetLevel( 4 )
    end
	for i = 0,1 do
		local ward=CreateUnitByName("npc_dota_ward_s", GetRandomPosition2D(curpos,400),false,caster,caster,caster:GetTeamNumber())
		ward:FindAbilityByName("lone_druid_spirit_bear_entangle"):SetLevel( 1 )
		ward:AddNewModifier(caster, self, "modifier_kill", {duration = dur})
		ward:AddNewModifier(caster, self, "modifier_unit_remove", {duration = dur})
		ward:AddNewModifier(caster, self, "modifier_serpent_ward_base", {duration = dur})
	end
end
...
function modifier_serpent_ward_base:GetModifierProcAttack_BonusDamage_Magical(keys)
    if not keys.target:IsBuilding() then
		return self:GetAbility():GetSpecialValueFor("att")*1.5
	end
end
...
function modifier_serpent_ward_base:GetModifierAttackSpeedBonus_Constant()
    if self:GetParent():HasModifier("modifier_shock_dam") then
            return self:GetAbility():GetSpecialValueFor("attsp") + self:GetAbility():GetSpecialValueFor("att")/2
    end
    return  self:GetAbility():GetSpecialValueFor("att")/2
end
...
function modifier_serpent_ward_pos:UpdateHorizontalMotion( t, g )
    ...
    if not self.CASTER:IsNull() and  self.CASTER:IsAlive() then
...
function modifier_serpent_ward_a:OnDeath(tg)
    ...
    if  tg.attacker == self:GetParent() and tg.unit ~= self:GetParent() then
        ...
        if caster:HasScepter() then
			...
			ward:AddNewModifier(caster, self:GetAbility(), "modifier_unit_remove", {duration = self.dur})
