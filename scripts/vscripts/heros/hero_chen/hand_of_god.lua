...
function hand_of_god:OnSpellStart()
        ...
        if CDOTA_PlayerResource.TG_HERO[i] then
            ...
            if hero~= nil and  hero:IsAlive() then
                if  Is_Chinese_TG(hero,self) then
                    ...
                    local particle = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_chen/chen_hand_of_god.vpcf", PATTACH_POINT_FOLLOW, hero)
                    ...
                    if caster:TG_HasTalent("special_bonus_chen_7") then
                        hero:AddNewModifier(caster, self, "modifier_invulnerable", {duration=1})
                    end
	...
	if self:GetCooldownTimeRemaining() < 40 then
		self:EndCooldown()
...
function modifier_hand_of_god_buff:OnIntervalThink()
    ...
    local particle = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_chen/chen_hand_of_god.vpcf", PATTACH_POINT_FOLLOW, self.parent)
