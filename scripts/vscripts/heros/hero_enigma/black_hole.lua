...
function black_hole:OnSpellStart()
    ...
    --[[if ab and ab:GetAutoCastState() and ab:GetLevel()>0 and ab:IsOwnersManaEnough() and ab:IsCooldownReady() then
        ab:OnSpellStart()
        ab:UseResources(true, false, false, true)
    end]]
end
...
function black_hole:OnChannelFinish(bInterrupted)
    ...
    if  bInterrupted then
		...
		--[[
		local mod = caster:FindModifierByName("modifier_black_hole_singularity") 
		if mod then
			mod:SetStackCount(math.max(math.ceil(mod:GetStackCount()*2/3),0))
		end]]
	end
    caster:StopSound("Hero_Enigma.Black_Hole")
end
...
function modifier_black_hole_debuff:OnCreated(tg)
	...
	self.radius=tg.radius+self.mod:GetStackCount()*15
    if self.mod then  
    self.start_heros = FindUnitsInRadius(
        ...
        FIND_ANY_ORDER,
        false)
    if #self.start_heros >= 3 then
        self.mod:SetStackCount(self.mod:GetStackCount()+2)
    end
    ...
    if #heros>0 then
        for _, hero in pairs(heros) do
            if hero~=nil and hero:IsAlive() and not hero:HasModifier("modifier_fountain_aura_buff") then
					if not (hero:IsMagicImmune() and TG_Distance(hero:GetAbsOrigin(), self.pos) > self.ability:GetSpecialValueFor("pull_radius")) then
							local damageTable = {
					...
					if not hero:HasModifier("modifier_black_hole_debuff2") and hero:IsAlive()  then
						hero:AddNewModifier(self.caster, self.ability, "modifier_black_hole_debuff2", {pos=self.pos,sp=650})
					end
				end
            end
        end
...
function modifier_black_hole_debuff:OnDestroy()
    if  IsServer() then
		...
		if self:GetElapsedTime()>self.ability:GetSpecialValueFor("duration")-0.1 then
			if #self.start_heros>0 then
				self.mod:SetStackCount(self.mod:GetStackCount()+#self.start_heros)
			end
		end
    end
end
...
function modifier_black_hole_debuff2:UpdateHorizontalMotion(t, g)
	...
	self.dis=TG_Distance(self:GetParent():GetAbsOrigin(), self.CENTER)
    if  self:GetCaster().black_holestart==false or not self:GetParent():IsAlive() 
		or (self.dis>self.base_dis and self:GetParent():IsMagicImmune()) then
        self:Destroy()
    else
		if self.dis >600 then
			self.STUN = false
			else
			self.STUN = true
		end
		--print(self.STUN)
        if  self.dis>100 and self.ROT then
