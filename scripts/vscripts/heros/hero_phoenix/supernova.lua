...
function supernova:OnSpellStart()
	...
	null:AddNewModifier(caster, self, "modifier_unit_remove", {duration=dur })
...
function modifier_supernova_buff:OnCreated()
    ...
    for num=1,#self.pos do
        self.fx = ParticleManager:SafeCreateParticle( "particles/heros/phoenix/phoenix_sunray_solar_forge.vpcf", PATTACH_WORLDORIGIN, nil )
...
function modifier_supernova_buff2:OnCreated()
    ...
    local fx = ParticleManager:SafeCreateParticle( "particles/units/heroes/hero_phoenix/phoenix_supernova_egg.vpcf", PATTACH_ABSORIGIN_FOLLOW, self.parent)
...
function modifier_supernova_buff2:OnDestroy()
    if  IsServer() then
        if self.parent:GetHealth()>0 then
            ...
            local fx = ParticleManager:SafeCreateParticle( "particles/units/heroes/hero_phoenix/phoenix_supernova_reborn.vpcf", PATTACH_ABSORIGIN_FOLLOW, caster )
    ...
    if  tg.target == self.parent and not tg.attacker:IsBuilding() and not tg.attacker:IsCreep() and not tg.attacker:IsNeutralUnitType() then
        if self.parent:GetHealth()>1 then
            self.parent:SetHealth(self.parent:GetHealth() - 1)
					...
					--TG_Kill(tg.attacker, self:GetCaster(), self.ability)
...
function modifier_supernova_buff2:OnDeath(tg)
    ...
    if tg.unit==self.parent and not self.parent:IsIllusion() then
        local fx = ParticleManager:SafeCreateParticle( "particles/units/heroes/hero_phoenix/phoenix_supernova_death.vpcf", PATTACH_WORLDORIGIN, nil)
