...
function poison_nova:OnSpellStart()
	...
	local p1 = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_venomancer/venomancer_poison_nova_cast.vpcf", PATTACH_ABSORIGIN_FOLLOW, caster)
	ParticleManager:ReleaseParticleIndex(p1)
	local p2 = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_venomancer/venomancer_poison_nova.vpcf", PATTACH_ABSORIGIN, caster)
...
function modifier_poison_nova:OnIntervalThink()
    if not self:GetParent():IsMagicImmune()  or self:GetCaster():TG_HasTalent("special_bonus_venomancer_8") then
...
function modifier_poison_nova:GetModifierMoveSpeedBonus_Percentage()
    if  self:GetParent():IsMagicImmune() and not self:GetCaster():TG_HasTalent("special_bonus_venomancer_8") then
        return 0
    end
