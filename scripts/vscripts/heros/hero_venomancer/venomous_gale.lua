...
function venomous_gale:OnSpellStart()
	...
	local dis = 1200+caster:GetCastRangeBonus()
...
function venomous_gale:OnProjectileHit_ExtraData(target, location, kv)
	...
	local dur = self:GetSpecialValueFor("duration")+caster:TG_GetTalentValue("special_bonus_venomancer_2")
        ...
        if target:IsRealHero() and caster:HasAbility("plague_ward") then
            ...
            if AB~=nil and AB:GetLevel()>0 then
                if caster:TG_HasTalent("special_bonus_venomancer_1") then
                    ...
                    ward1:AddNewModifier(caster, AB, "modifier_plague_ward", {duration=dur+1})
                    ward1:AddNewModifier(caster, AB, "modifier_kill", {duration=dur+1})
					ward1:AddNewModifier(caster, AB, "modifier_unit_remove", {duration=dur+1})
                ...
                if caster:Has_Aghanims_Shard() then
                    ...
                    ward2:AddNewModifier(caster, AB, "modifier_plague_ward", {duration=dur+1})
                    ward2:AddNewModifier(caster, AB, "modifier_kill", {duration=dur+1})
					ward2:AddNewModifier(caster, AB, "modifier_unit_remove", {duration=dur+1})
                    ward2:SetControllableByPlayer(caster:GetPlayerOwnerID(), false)
                end
            end
        end
        target:AddNewModifier_RS(caster, self, "modifier_venomous_gale_debuff", {duration=dur})
        local damageTable = {
            victim = target,
            attacker = caster,
            damage = dam,
