...
function plague_ward:OnSpellStart()
    ...
    for i=1,self:GetSpecialValueFor("num") do
	   ...
	   ward:AddNewModifier(caster, self, "modifier_unit_remove", {duration=duration})
...
function modifier_plague_ward:GetModifierAttackSpeedBonus_Constant(tg)
    return self.asp
end
...
function modifier_plague_ward:OnCreated()
	...
	self.asp = self:GetCaster():GetLevel()*2
...
function modifier_plague_ward:OnAttackLanded(tg)
    ...
    if  tg.target == self:GetParent() then
		local dam = 1
		if tg.attacker:IsHero() then
			dam = 3
		end
        if self:GetParent():GetHealth()>0 then
		if (self:GetParent():GetHealth()-dam)<=0 then
			self:GetParent():Kill(self:GetAbility(), tg.attacker)
			return
        end
        self:GetParent():SetHealth(self:GetParent():GetHealth() - dam)
		end
    end
    ...
    if tg.attacker == self:GetParent() then
        ...
        if self:GetCaster():TG_HasTalent("special_bonus_venomancer_6") then
            local damage= {
                ...
                damage = self:GetCaster():GetLevel()*3,
