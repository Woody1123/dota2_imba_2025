...
function dual_breath:OnSpellStart()
	...
	local flydis = self:GetSpecialValueFor("flydis")+self:GetSpecialValueFor("talent_2")*caster:GetCastRangeBonus()
...
function dual_breath:OnProjectileHit_ExtraData(target, location, kv)
	...
	if not target:IsMagicImmune() then
		...
		if self:GetSpecialValueFor("talent_1")==1 then
			local Knockback ={
						should_stun = 1,
                ...
                target:AddNewModifier_RS(self:GetCaster(),self, "modifier_knockback", Knockback)
		end
