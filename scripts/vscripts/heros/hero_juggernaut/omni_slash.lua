...
function modifier_omni_slash_buff:OnDestroy()
    ...
    if self.kills and self.kills>0 then
        local pfx = ParticleManager:SafeCreateParticle("particles/econ/items/juggernaut/jugg_arcana/juggernaut_arcana_counter.vpcf", PATTACH_OVERHEAD_FOLLOW,self.parent)
...
function modifier_omni_slash_buff:OnDeath(tg)
    ...
    if tg.unit == self.target then
        if tg.unit:IS_TrueHero_TG() and tg.attacker==self.parent then
            ...
            local enemies = FindUnitsInRadius(tg.attacker:GetTeamNumber(), tg.unit:GetAbsOrigin(), nil,600 , DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_FARTHEST, false)
            if  #enemies>0 then
