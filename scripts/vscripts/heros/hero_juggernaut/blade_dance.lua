...
LinkLuaModifier("modifier_blade_dance_buff", "heros/hero_juggernaut/blade_dance.lua", LUA_MODIFIER_MOTION_HORIZONTAL)
...
function modifier_blade_dance_pa:RemoveOnDeath()
    return false
end
...
function modifier_blade_dance_pa:GetModifierPreAttack_CriticalStrike(tg)
    ...
    if tg.attacker == self:GetParent() and not tg.target:IsBuilding() and not self:GetParent():PassivesDisabled() then
        ...
        if RollPseudoRandomPercentage(ch,0,self:GetParent()) then
            ...
            return self:GetAbility():GetSpecialValueFor("crit_mult")+self:GetCaster():TG_GetTalentValue("special_bonus_juggernaut_3")+self.buff*self:GetAbility():GetSpecialValueFor("ex_mult")
		else
			return 0
		end
	end
...
function modifier_blade_dance_pa:OnAttackLanded(tg)
	...
	if tg.attacker ~= self:GetParent() or self:GetParent():PassivesDisabled()  or tg.target:IsBuilding() or not tg.target:IsAlive() or tg.target:IsBossCreature() then
		return
    end
	--[[
    local damageTable = {
...
modifier_blade_dance_buff=class({})


function modifier_blade_dance_buff:IsHidden()
    return false
end

function modifier_blade_dance_buff:IsPurgable()
    return false
end

function modifier_blade_dance_buff:IsPurgeException()
    return false
end
function modifier_blade_dance_buff:DeclareFunctions()
    return
    {
		MODIFIER_PROPERTY_DODGE_PROJECTILE,
	}
end
function modifier_blade_dance_buff:GetModifierDodgeProjectile()
    if RollPseudoRandomPercentage(10*self:GetStackCount(),0,self:GetParent()) then
        return 1
    else
        return 0
    end
end
function modifier_blade_dance_buff:OnCreated()
	if IsServer() then
		self.mod = self:GetParent():FindModifierByName("modifier_blade_dance_pa")
		self:SetStackCount(0)
		self:OnRefresh()
	end
end
function modifier_blade_dance_buff:OnRefresh()
	if IsServer() then
		self:SetStackCount(self:GetStackCount()+1)
		if self.mod~=nil then
			self.mod.buff = self:GetStackCount()
		end
	end
end
function modifier_blade_dance_buff:OnDestroy()
