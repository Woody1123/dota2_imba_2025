...
function stifling_dagger:OnSpellStart()
    ...
    if self:GetSpecialValueFor("talent_2")>0 then
        local P =
        {
            Target = target,
            Source = caster,
...
function stifling_dagger:OnProjectileHit_ExtraData( target, location,kv)
	...
	if not target:IsMagicImmune() then
        ...
        if self:GetSpecialValueFor("talent_1")>0 then
            target:AddNewModifier_RS(caster, self, "modifier_imba_stunned", {duration = self:GetSpecialValueFor("talent_1")})
        end
	end
end
