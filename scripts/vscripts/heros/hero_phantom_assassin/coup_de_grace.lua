			...
			if caster:HasScepter() then
				...
				for i = 0,5 do 
					if caster:GetItemInSlot(i) ~= nil then
						if not caster:GetItemInSlot(i):IsCooldownReady() then
							table.insert(item_tab,caster:GetItemInSlot(i))
						end
					end
				end
...
function modifier_coup_de_grace_pa:OnCreated()
    ...
    self.crit = {}
    self.crit_kill = 0.5--self:GetAbility():GetSpecialValueFor("crit_kill")
