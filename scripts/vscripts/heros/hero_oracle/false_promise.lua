...
LinkLuaModifier("modifier_false_promise_buff3_cd", "heros/hero_oracle/false_promise.lua", LUA_MODIFIER_MOTION_NONE)
...
function modifier_false_promise_buff:GetModifierAvoidDamage(tg)
    if not IsServer() then
        return 
    end

    self.DAM=self.DAM+tg.damage
    if (self:GetParent():GetHealth()-self.DAM)<=0 then
        self.ATT=tg.attacker
    end

    if self:GetParent():GetHealth()+ self.HEAL-self.DAM<=0 and self.fx3~=nil then
        ParticleManager:SetParticleControl(self.fx3, 1, Vector(5000,0,0))
    end
    return 1
end
...
function modifier_false_promise_buff3:CheckState()
    if  not self:GetParent():HasModifier("modifier_false_promise_buff3_cd") then
    return
        {
...
function modifier_false_promise_buff3:OnAttack(keys)
	...
	if keys.attacker == self:GetParent() and self:GetParent():IsRangedAttacker() then
		self:GetParent():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_false_promise_buff3_cd", {duration=0.15})
	end
end
...
modifier_false_promise_buff3_cd= class({})


function modifier_false_promise_buff3_cd:IsHidden()
    return true
end

function modifier_false_promise_buff3_cd:IsPurgable()
    return false
end

function modifier_false_promise_buff3_cd:IsPurgeException()
    return false
end