...
function modifier_purifying_flames_buff:IsPurgable() 		
    return Is_Chinese_TG(self:GetCaster(),self:GetParent())
end

function modifier_purifying_flames_buff:IsPurgeException() 
    return Is_Chinese_TG(self:GetCaster(),self:GetParent())
end
function modifier_purifying_flames_buff:DeclareFunctions() return {MODIFIER_PROPERTY_MAGICAL_RESISTANCE_BONUS} end

function modifier_purifying_flames_buff:GetModifierMagicalResistanceBonus( params )
	return self.mag
end
...
function modifier_purifying_flames_buff:OnCreated()
	...
	self.mag =self:GetAbility():GetSpecialValueFor("mag") * (Is_Chinese_TG(self:GetParent(),self:GetCaster()) and 1 or -1)
...
function modifier_purifying_flames_buff:OnIntervalThink() 
    if Is_Chinese_TG(self:GetParent(),self:GetCaster())  and self:GetCaster()~=self:GetParent() then
