...
LinkLuaModifier("modifier_fates_edict_buff_self_cd", "heros/hero_oracle/fates_edict.lua", LUA_MODIFIER_MOTION_NONE)
...
function modifier_fates_edict_buff_self:DeclareFunctions()
    return {
...
function modifier_fates_edict_buff_self:OnAttack()
    if not self:GetCaster():TG_HasTalent("special_bonus_oracle_1") then
        self:GetParent():AddNewModifier(caster, self, "modifier_fates_edict_buff_self_cd", {duration=1})
    end
end


function modifier_fates_edict_buff_self:GetModifierMagicalResistanceBonus()
    if self:GetParent():HasModifier("modifier_fates_edict_buff_self_cd")then
        return 0
    else
        return 100
    end
...
modifier_fates_edict_buff_self_cd= class({})

function modifier_fates_edict_buff_self_cd:IsDebuff()
    return false
end

function modifier_fates_edict_buff_self_cd:IsHidden()
    return true
end

function modifier_fates_edict_buff_self_cd:IsPurgable()
    return false
end

function modifier_fates_edict_buff_self_cd:IsPurgeException()
    return false
end
...
function modifier_fates_edict_debuff:CheckState()
    return
    {
