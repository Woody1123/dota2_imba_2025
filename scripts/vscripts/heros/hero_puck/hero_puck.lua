...
-- 	if IsServer() then
-- 		local caster = self:GetCaster()
-- 		local ability = caster:FindAbilityByName("imba_puck_ethereal_jaunt")
-- 		ability:SetLevel(1)
-- 	end
-- end
...
function imba_puck_ethereal_jaunt:Set_InitialUpgrade() return {LV=1} end
...
function modifier_imba_phase_shift:OnDestroy()
	if IsServer() then
		...
		if self.int == 0 then
			...
			local cd = (self:GetAbility():GetCooldown(-1) * self:GetCaster():GetCooldownReduction()) - self:GetElapsedTime()
			if cd<0.7 then 
				cd = 0.7
			end
			self:GetAbility():StartCooldown(math.max(FrameTime(), cd))
		end
	end
end
...
function modifier_imba_dream_coil_enemy:OnDestroy()
	if IsServer() then
		...
		if self:GetStackCount() == 1 then
			...
			target:AddNewModifier_RS(caster, ability, "modifier_imba_stunned", {duration = (caster:HasScepter() and ability:GetSpecialValueFor("coil_stun_duration_scepter") or ability:GetSpecialValueFor("coil_stun_duration"))})
		end
	end
end
