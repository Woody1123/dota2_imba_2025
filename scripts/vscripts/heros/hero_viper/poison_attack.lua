...
function modifier_poison_attack_att:GetModifierAttackRangeBonus()
    return self:GetAbility():GetSpecialValueFor("attr")
end
...
function modifier_poison_attack_att:OnAttackLanded(tg)
    ...
    if tg.attacker == self.parent then
        if self.parent:PassivesDisabled() or self.parent:IsIllusion()  or tg.target:IsMagicImmune() or tg.target:IsBuilding() or not self.ability:IsCooldownReady() then
            return
        end
        self.ability:UseResources(false,false,  false, true)
        tg.target:AddNewModifier_RS(self.parent, self.ability, "modifier_poison_attack_debuff", {duration=self.dur+self:GetCaster():TG_GetTalentValue("special_bonus_viper_2")})
...
function modifier_poison_attack_debuff:GetModifierMagicalResistanceBonus(tg)
    --return 0-self:GetStackCount()
    return -(self:GetAbility():GetSpecialValueFor("magic_resistance")+self:GetCaster():TG_GetTalentValue("special_bonus_viper_5"))*self:GetStackCount()
end

function modifier_poison_attack_debuff:GetModifierPhysicalArmorBonus()
    if self:GetCaster():Has_Aghanims_Shard() then
