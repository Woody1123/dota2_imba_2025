...
function nethertoxin:GetCooldown(iLevel)
        return self.BaseClass.GetCooldown(self,iLevel)-self:GetCaster():TG_GetTalentValue("special_bonus_viper_7")
end

function nethertoxin:OnSpellStart()
    local caster=self:GetCaster()
    EmitSoundOn("hero_viper.Nethertoxin.Cast",caster)
    CreateModifierThinker(self:GetCaster(), self, "modifier_nethertoxin_th", {duration=8}, self:GetCursorPosition(), caster:GetTeamNumber(), false)
end
...
function modifier_nethertoxin_th:OnIntervalThink()
    ...
    if #heros>0 then
        for _, target in pairs(heros) do
            if not target:IsMagicImmune() and target ~= CDOTA_PlayerResource.ROSHAN then
...
function modifier_nethertoxin_debuff:CheckState()
    ...
    return
    {
        ...
        [MODIFIER_STATE_SILENCED] = self:GetCaster():TG_HasTalent("special_bonus_viper_4"),
    }
    end
end
