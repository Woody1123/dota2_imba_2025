...
function corrosive_skin:OnSpellStart()
    ...
    caster:AddNewModifier(caster, self, "modifier_corrosive_skin_buff", {duration=self:GetSpecialValueFor("dursp")})
end
...
function modifier_corrosive_skin_pa:OnTakeDamage(tg)
    ...
    if tg.unit == self.parent and tg.attacker~=self.parent then
        ...
        tg.attacker:AddNewModifier_RS(self.parent, self.ability, "modifier_corrosive_skin_debuff", {duration=self.duration})
	end
end
