...
function psionic_trap:OnSpellStart()
    ...
    if caster.psionic_trapnum<=max then
        ...
        null:SetControllableByPlayer(id, true)
        ...
        local ab=null:AddAbility("Trigger_trap")
       -- print(ab:GetMaxLevel())
...
function modifier_psionic_trap_buff:OnIntervalThink()
    ...
    if parent:FindAbilityByName("Trigger_trap"):GetAutoCastState() then
        return
    end
    ...
    if  IsServer() then
        ...
        for _, hero in pairs(heros) do
            if not hero:IsMagicImmune() then
                ...
                hero:AddNewModifier(self:GetParent() , self:GetAbility(), "modifier_paralyzed", {duration=self:GetAbility():GetSpecialValueFor( "confuse" )})
                local damageTable = {
