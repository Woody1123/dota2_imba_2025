...
function meld:OnSpellStart()
    ...
    caster:AddNewModifier(caster, self, "modifier_meld_buff2", {duration=0.1})
...
function meld:OnProjectileHit_ExtraData(target, location,kv)
    ...
    if not target:IsBuilding() and not target:IsBossCreature() then
            if caster:TG_HasTalent("special_bonus_templar_assassin_8") then
                    ...
                    for _, tar  in pairs(illusions) do
						...
						--tar:AddNewModifier(caster, self, "modifier_unit_remove", {duration=2})
						...
						tar:SetBaseAgility(caster:GetAgility()*1.5)
                    end
            end
...
function modifier_meld_debuffheal:GetModifierPhysicalArmorBonus()
	return self.armor
end

function modifier_meld_debuffheal:OnCreated()
	if self:GetAbility() == nil then return end
    self.armor = -1*(self:GetParent():GetMaxHealth()/200)
end
