...
function modifier_essence_aura_buff2:OnAbilityExecuted(tg)
    if IsServer() then 
        if tg.unit==self.parent and  not self.parent:IsIllusion() and tg.ability~=nil and not tg.ability:IsToggle() and not tg.ability:IsItem() and tg.ability:GetName() ~= "imba_tiny_tree_grab" and tg.ability:GetCooldown(tg.ability:GetLevel())>1 then 
...
function modifier_essence_aura_buff3:OnCreated(tg)
    ...
    self.caster=self:GetCaster()
    if IsServer() then 
        self:SetStackCount(math.min(self:GetStackCount()+tg.num,400))
