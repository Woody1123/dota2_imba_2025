CreateTalents("npc_dota_hero_rattletrap", "heros/hero_rattletrap/hero_rattletrap.lua")

imba_rattletrap_battery_assault = class({})

LinkLuaModifier("modifier_imba_battery_assault", "heros/hero_rattletrap/hero_rattletrap.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_battery_assault_passive", "heros/hero_rattletrap/hero_rattletrap.lua", LUA_MODIFIER_MOTION_NONE)
...
function imba_rattletrap_battery_assault:OnSpellStart()
	...
	caster:AddNewModifier(caster, self, "modifier_imba_battery_assault", {duration = (self:GetSpecialValueFor("duration") + caster:TG_GetTalentValue("special_bonus_imba_rattletrap_1"))})
end
...
function modifier_imba_battery_assault:OnCreated()
	...
	if IsServer() then
		...
		local interval=self:GetAbility():GetSpecialValueFor("interval") + self.caster:TG_GetTalentValue("special_bonus_imba_rattletrap_2")
...
function modifier_imba_battery_assault:OnIntervalThink()
	...
	local pfx_origin = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_rattletrap/rattletrap_battery_assault.vpcf", PATTACH_CUSTOMORIGIN, nil)
	ParticleManager:SetParticleControlEnt(pfx_origin, 0, self.caster, PATTACH_POINT, attach, self.caster:GetAbsOrigin(), true)
	ParticleManager:ReleaseParticleIndex(pfx_origin)
	local enemy = FindUnitsInRadius(self.caster:GetTeamNumber(), self.caster:GetAbsOrigin(), nil, self.radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NO_INVIS, FIND_ANY_ORDER, false)
	for i=1, self.max_targets do
		local pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_rattletrap/rattletrap_battery_shrapnel.vpcf", PATTACH_CUSTOMORIGIN, nil)
		ParticleManager:SetParticleControlEnt(pfx, 0, self.caster, PATTACH_POINT, attach, self.caster:GetAbsOrigin(), true)
		if enemy[i] then
			ParticleManager:SetParticleControlEnt(pfx, 1, enemy[i], PATTACH_POINT, "attach_hitloc", enemy[i]:GetAbsOrigin(), true)
			enemy[i]:AddNewModifier(self.caster, self.ability, "modifier_imba_stunned", {duration = self.stun_duration})
			ApplyDamage({attacker = self.caster, victim = enemy[i], ability = self.ability, damage = self.damage, damage_type = self.ability:GetAbilityDamageType()})
			enemy[i]:EmitSound("Hero_Rattletrap.Battery_Assault_Impact")
		else
			ParticleManager:SetParticleControl(pfx, 1, GetRandomPosition2D(self.caster:GetAbsOrigin() + Vector(0, 0, 50), self.radius))
		end
...
function modifier_imba_battery_assault:OnDestroy()
	if IsServer() then
		self.caster:StopSound("Hero_Rattletrap.Battery_Assault")
	end
end
...
LinkLuaModifier("modifier_imba_power_cog", "heros/hero_rattletrap/hero_rattletrap.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_power_cog_block", "heros/hero_rattletrap/hero_rattletrap.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_power_cog_flying", "heros/hero_rattletrap/hero_rattletrap.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_power_cog_knocback", "heros/hero_rattletrap/hero_rattletrap.lua", LUA_MODIFIER_MOTION_NONE)
...
function imba_rattletrap_power_cogs:OnSpellStart()
	...
	for i=0, 7 do
		...
		local timer = cog:AddNewModifier(caster, self, "modifier_kill", {duration = self:GetSpecialValueFor("duration") + 0.1})
					  cog:AddNewModifier(caster, self, "modifier_unit_remove", {duration = self:GetSpecialValueFor("duration") + 0.1})
		local pfx_cog = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_rattletrap/rattletrap_cog_ambient.vpcf", PATTACH_CUSTOMORIGIN, cog)
	...
	local pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_rattletrap/rattletrap_cog_deploy.vpcf", PATTACH_CUSTOMORIGIN, nil)
...
function modifier_imba_power_cog_knocback:OnCreated(keys)
	...
	if IsServer() then
		...
		local pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_rattletrap/rattletrap_cog_attack.vpcf", PATTACH_CUSTOMORIGIN, nil)
...
LinkLuaModifier("modifier_imba_rocket_flare_vision", "heros/hero_rattletrap/hero_rattletrap.lua", LUA_MODIFIER_MOTION_NONE)
...
function imba_rattletrap_rocket_flare:OnSpellStart()
	...
	for i=1, (caster:TG_GetTalentValue("special_bonus_imba_rattletrap_4") + 1) do
		...
		local pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_rattletrap/rattletrap_rocket_flare.vpcf", PATTACH_CUSTOMORIGIN, nil)
...
function imba_rattletrap_rocket_flare:OnProjectileThink_ExtraData(pos, keys)
	...
	for i=1, #enemy do
		enemy[i]:AddNewModifier(caster, self, "modifier_imba_rocket_flare_vision", {duration = (self:GetSpecialValueFor("lock_duration") + caster:TG_GetTalentValue("special_bonus_imba_rattletrap_3"))})
	end
end
...
--	end
	local rd=self:GetSpecialValueFor("radius")
	local dur=self:GetSpecialValueFor("duration")+caster:TG_GetTalentValue("special_bonus_imba_rattletrap_3")
	local pfx = ParticleManager:SafeCreateParticle(pfx_name, PATTACH_CUSTOMORIGIN, nil)
	ParticleManager:SetParticleControl(pfx, 0, pos)
	ParticleManager:SetParticleControl(pfx, 1, Vector(dur, 0, 0))
	ParticleManager:ReleaseParticleIndex(pfx)
	AddFOWViewer(caster:GetTeamNumber(), pos, rd, dur, false)
	local truesight = CreateUnitByName("npc_dummy_unit", pos, false, caster, caster, caster:GetTeamNumber())
	truesight:AddNewModifier(caster, self, "modifier_kill", {duration = dur})
	truesight:AddNewModifier(caster, self, "modifier_unit_remove", {duration = dur})
	CreateModifierThinker(caster, self, "modifier_truesight_f", {duration = dur}, pos,caster:GetTeamNumber(), false)
	local enemy = FindUnitsInRadius(caster:GetTeamNumber(), pos, nil, rd, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NONE, FIND_ANY_ORDER, false)
	for i=1, #enemy do
...
function modifier_imba_rocket_flare_vision:OnCreated()
	if self:GetAbility() == nil then return end
	if IsServer() then
		local pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_rattletrap/rattletrap_rocket_flare_sparks.vpcf", PATTACH_CUSTOMORIGIN, self:GetParent())
...
LinkLuaModifier("modifier_imba_hookshot_hookcheck", "heros/hero_rattletrap/hero_rattletrap.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_hookshot_motion", "heros/hero_rattletrap/hero_rattletrap.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_hookshot_stunned", "heros/hero_rattletrap/hero_rattletrap.lua", LUA_MODIFIER_MOTION_NONE)
...
function imba_rattletrap_hookshot:OnProjectileHit_ExtraData(target, pos, keys)
	...
	if target and caster:IsAlive() and not caster:HasModifier("modifier_imba_hookshot_motion") then
		...
		if IsEnemy(caster, target) then
			target:AddNewModifier(caster, self, "modifier_imba_hookshot_stunned", {duration=10})
		end
		caster:AddNewModifier(caster, self, "modifier_imba_hookshot_motion", {pfx = keys.pfx, target = target:entindex(), thinker = 0,duration=10})
		return true
	else
		ParticleManager:SetParticleControlEnt(keys.pfx, 1, caster, PATTACH_POINT_FOLLOW, "attach_attack1", caster:GetAbsOrigin(), true)
		local thinker = CreateModifierThinker(caster, self, "modifier_imba_hookshot_motion", {pfx = keys.pfx, target = caster:entindex(), thinker = 1,duration=10}, pos, caster:GetTeamNumber(), false)
		--ParticleManager:DestroyParticle(keys.pfx, false)
		--ParticleManager:ReleaseParticleIndex(keys.pfx)
		return true
	end
...
function modifier_imba_hookshot_motion:CheckState() return {[MODIFIER_STATE_INVULNERABLE]=true,[MODIFIER_STATE_NO_UNIT_COLLISION] = true, [MODIFIER_STATE_STUNNED] = true, [MODIFIER_STATE_FLYING_FOR_PATHING_PURPOSES_ONLY] = true} end
