...
function modifier_wukongs_command_motion:OnDestroy()
    if  IsServer() then
        ...
        if self:GetParent():HasScepter() then
			...
			illusions[1]:AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_unit_remove", {duration=dur})
            local mod =self:GetParent():FindModifierByName("modifier_item_attsp_book")
            if mod then
                illusions[1]:AddNewModifier(self:GetParent(), nil, "modifier_item_attsp_book", {num=mod:GetStackCount()})
            end]]
        end
        self.dur=6
...
function modifier_wukongs_command_buff4:GetModifierPhysicalArmorBonus() return 60 end
function modifier_wukongs_command_buff4:GetModifierAttackRangeBonus()
    return 100
end

function modifier_wukongs_command_buff4:OnAttackLanded(tg)
    if not IsServer() then
		return
	end
    ...
    if tg.target:IS_TrueHero_TG() and RollPseudoRandomPercentage(2,0,self:GetParent()) and not tg.target:HasModifier("modifier_droiyan_cbuff")  then
        ...
        Notifications:TopToAll({text = "灰飞烟灭", duration = 3.0, style = {["font-size"] = "50px", color = "#CD2626"}})
    end
end
...
function modifier_wukongs_command_buff5:DeclareFunctions()
    return
     {
...
function modifier_wukongs_command_buff5:GetModifierIncomingDamageConstant(keys)	 
	if IsServer() then
		if keys.target:HasScepter() and keys.target:GetHealth() - keys.damage <=0 then
				keys.target:AddNewModifier( keys.target, nil, "modifier_wukongs_command_buff6", {duration=3.5})
				self:Destroy()
    ...
    if #heros > 0 then
        for _, hero in pairs(heros) do
            ...
            local fx =ParticleManager:CreateParticle('particles/tgp/mk/att_m.vpcf',PATTACH_CUSTOMORIGIN, nil)
				...
				if PseudoRandom:RollPseudoRandom(self:GetAbility(), 50) then 
					if self:GetCaster():HasModifier("modifier_item_monkey_king_bar_v2_pa") then
						...
						if mod then
							mod.miss = true
							self:GetCaster():PerformAttack(hero, true, true, true, false, false, false, true)
							mod.miss = false
						end
					else
						self:GetCaster():PerformAttack(hero, true, true, true, false, false, false, true)
