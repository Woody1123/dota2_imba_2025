...
function mischief:OnSpellStart()
    ...
    local modifier=
    {
		...
		--illusions[1]:AddNewModifier(caster, self, "modifier_unit_remove", {duration=5})
        ...
        if mod then
            illusions[1]:AddNewModifier(caster, nil, "modifier_item_attsp_book", {num=mod:GetStackCount()})
        end
