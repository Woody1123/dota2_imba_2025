...
function boundless_strike:OnSpellStart()
    ...
    local end_pos=caster_pos+dir*(strike_cast_range+self.caster:Script_GetAttackRange())
    ...
    local modifier=
    {
        ...
        local mod =self.caster:FindModifierByName("modifier_item_attsp_book")
        ...
        for i=1,#illusions do
			...
			--illusions[i]:AddNewModifier(self.caster, self, "modifier_unit_remove", {duration=12})
            FindClearSpaceForUnit(illusions[i], cur_pos, true)
            if mod then
                illusions[i]:AddNewModifier(self.caster, nil, "modifier_item_attsp_book", {num=mod:GetStackCount()})
            end
        end
    end
end
