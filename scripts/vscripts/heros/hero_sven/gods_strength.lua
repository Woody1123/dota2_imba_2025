...
function gods_strength:GetCooldown(level)
	local cooldown = self.BaseClass.GetCooldown(self, level)
	local caster = self:GetCaster()
	local Talent = caster:TG_GetTalentValue("special_bonus_sven_8")
	local  Getcd = cooldown - Talent
	if caster:TG_HasTalent("special_bonus_sven_8") then
		return (Getcd)
	end
	return cooldown
end
...
function modifier_gods_strength2:OnCreated( tg )
	if self:GetAbility() == nil then return end
	self.att = self:GetAbility():GetSpecialValueFor( "att" )-50
end
