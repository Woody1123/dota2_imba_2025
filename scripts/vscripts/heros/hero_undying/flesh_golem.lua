...
function modifier_flesh_golem_buff:RemoveOnDeath()
    return false
end
...
function modifier_flesh_golem_buff:GetModifierIncomingDamage_Percentage()
	return	self.dam_in
end
...
function modifier_flesh_golem_buff:OnCreated(tg)
   if self:GetAbility() == nil then return end
   self.hp = self:GetAbility():GetSpecialValueFor("hp")
   self.ar = self:GetAbility():GetSpecialValueFor("ar")
   self.dam_in = self:GetCaster():TG_GetTalentValue("special_bonus_undying_7")*-1
end
function modifier_flesh_golem_buff:OnRefresh(tg)
	self:OnCreated()
end
