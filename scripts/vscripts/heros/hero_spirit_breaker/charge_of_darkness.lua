...
function charge_of_darkness:CastFilterResultTarget(target)
    local caster=self:GetCaster()
	if (caster:GetTeamNumber() == target:GetTeamNumber()) or (target:IsMagicImmune() and not self:GetCaster():HasScepter()) or  target:IsBuilding() or not target:IsUnit()then
		return UF_FAIL_CUSTOM
	end
	return UF_SUCCESS
end

function charge_of_darkness:GetCastPoint()
...
function charge_of_darkness:GetCustomCastErrorTarget(target)
    if target:IsMagicImmune() and target:IsUnit() then
        return "无法对魔免单位使用"
    else
        return "无法对该目标使用"
    end
...
function modifier_charge_of_darkness:OnCreated(tg)
    ...
    self.dmg=self.ability:GetSpecialValueFor( "dmg" )+self.parent:TG_GetTalentValue("special_bonus_spirit_breaker_5")
    ...
    local particle = ParticleManager:SafeCreateParticle("particles/heros/spirit_breaker/spirit_breaker_charge_mist_change0.vpcf", PATTACH_ABSORIGIN_FOLLOW, self.parent)
...
function modifier_charge_of_darkness:UpdateHorizontalMotion( t, g )
    ...
    if  not self.parent:IsAlive() or self.parent:IsStunned() or self.parent:IsOutOfGame() or self.parent:IsRooted() or self.parent:IsHexed() or  dis<=150 then
        self:Destroy()
        return
    end
    ...
    if  self.parent and self.parent:IsAlive() then
        ...
        if #heros>0 then
            ...
            if not Is_DATA_TG( self.TARTB,hero) then
                    ...
                    local particle = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_spirit_breaker/spirit_breaker_greater_bash.vpcf" , PATTACH_CENTER_FOLLOW,hero)
                ...
                if hero:IsRealHero() then
                    ...
                    self.damageTable.damage = sp*self.dmg*0.015
                end
...
function modifier_charge_of_darkness:OnDestroy()
    ...
    if  self.target and  self.target:IsAlive() then
        ...
        if self.ability:GetAutoCastState()==true then
            ...
            if dis<=150 then
                self.target:AddNewModifier(self.parent, self.ability, "modifier_stunned", {duration=self.stun_duration})
...
function modifier_charge_of_darkness:OnOrder(tg)
    ...
    if tg.unit == self.parent then
		if --tg.order_type == DOTA_UNIT_ORDER_MOVE_TO_POSITION or tg.order_type == DOTA_UNIT_ORDER_MOVE_TO_TARGET or
           ...
           -- tg.order_type == DOTA_UNIT_ORDER_HOLD_POSITION or

			tg.order_type == DOTA_UNIT_ORDER_HOLD_POSITION or tg.order_type == DOTA_UNIT_ORDER_STOP then
            self.parent:ClearActivityModifiers()
            self:Destroy()
        end
	end
...
function modifier_charge_of_darkness_target:OnCreated()
    ...
    self.ability=self:GetAbility()
    if IsServer() then
        local particle = ParticleManager:SafeCreateParticleForPlayer("particles/units/heroes/hero_spirit_breaker/spirit_breaker_charge_target.vpcf" , PATTACH_OVERHEAD_FOLLOW, self.parent,self.caster:GetPlayerOwner())
        ParticleManager:SetParticleControl(particle, 0, self.parent:GetAbsOrigin())
