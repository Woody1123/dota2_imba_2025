...
function mortal_strike:OnSpellStart()
        ...
        local dust = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_earthshaker/earthshaker_totem_leap_impact_dust.vpcf", PATTACH_CUSTOMORIGIN,nil)
    ...
    if caster:TG_HasTalent("special_bonus_skeleton_king_5") then
        for a=1,6 do
            ...
            local unit = CreateUnitByName("npc_wraith_king_skeleton_warrior",epos, true, caster, caster, caster:GetTeamNumber())
					...
					unit:AddNewModifier(caster, self, "modifier_unit_remove", {duration=10})
...
function modifier_mortal_strike_pa:RemoveOnDeath()
    return false
end
...
function modifier_mortal_strike_buff:RemoveOnDeath()
    return	false
end
