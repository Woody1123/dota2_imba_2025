...
function modifier_item_siege_pa:GetModifierMoveSpeedBonus_Percentage()
    return self.speed
end
...
function modifier_item_siege_debuff:OnCreated()
    ...
    self.auraattsp=self:GetAbility():GetSpecialValueFor("auraattsp")
    self.aurasp=self:GetAbility():GetSpecialValueFor("aurasp")
...
--攻速光环
function modifier_item_siege_debuff:GetModifierAttackSpeedBonus_Constant()
    if IsValidEntity(self.caster) and IsValidEntity(self.parent) then
        if self.caster:GetTeamNumber()==self.parent:GetTeamNumber() then
                return self.auraattsp
        end
    end
        return 0
end
...
--移速光环
function modifier_item_siege_debuff:GetModifierMoveSpeedBonus_Constant()
    if IsValidEntity(self.caster) and IsValidEntity(self.parent) then
        if self.caster:GetTeamNumber()==self.parent:GetTeamNumber() then
                return self.aurasp
        end
