...
function item_abyssal_blade_v2:OnSpellStart()
       -- local t1 = GetSystemTimeMS()
        ...
        target:AddNewModifier(self.caster, self, "modifier_item_abyssal_blade_v2_debuff2", {duration=self.pa_dur})
...
function modifier_item_abyssal_blade_v2_pa:GetModifierProcAttack_BonusDamage_Physical(keys)
	if not self:GetParent():IsIllusion() and not keys.target:IsBuilding() then
        ...
        if  RollPseudoRandomPercentage(ch,0,self.ability) then
            if self.stunned==true then
                ...
                keys.target:AddNewModifier_RS(self.parent, self.ability, "modifier_imba_stunned", {duration=self.stun})
				keys.target:AddNewModifier(self.parent, self.ability, "modifier_item_abyssal_blade_v2_debuff2", {duration=self.stun})
            end
			return 200
		end
	end
