    ...
    for i=1, #CDOTA_PlayerResource.TG_HERO do
        if CDOTA_PlayerResource.TG_HERO[i] then
            ...
            if hero~= nil and  hero:IsAlive() then
                if  Is_Chinese_TG(hero,self.caster) then
                    hero:AddNewModifier(hero, self, "modifier_item_aegis_v2_pa", {duration=300})
                end
            end
        end
    end
...
function modifier_item_aegis_v2_pa:OnDeath(tg)
    ...
    if tg.unit == self.parent and not self.parent:IsIllusion() then
        ...
        self.parent:EmitSound("Aegis.Expire")
        local fx = ParticleManager:SafeCreateParticle("particles/items_fx/aegis_timer.vpcf", PATTACH_CUSTOMORIGIN, nil)
        ...
        Timers:CreateTimer(3, function()
            local fx2 = ParticleManager:SafeCreateParticle("particles/items_fx/aegis_respawn.vpcf", PATTACH_ABSORIGIN_FOLLOW, self.parent)
