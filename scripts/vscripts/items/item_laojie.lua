...
function modifier_item_laojie_pa:OnAttackLanded(tg)
	...
	tg.target:AddNewModifier(tg.attacker,self:GetAbility(),"modifier_item_laojie_debuff",{duration = 7})
...
function modifier_item_laojie_debuff:OnRefresh()
	if IsServer() then
		if self:GetParent():IsBuilding() then return end
		if self:GetParent():GetPhysicalArmorValue(false)>30 then
		
			if self:GetParent()==CDOTA_PlayerResource.ROSHAN or self:GetCaster():IsRangedAttacker() then
