...
function item_pool_blink:OnSpellStart()
	...
	ParticleManager:SafeCreateParticle("particles/items_fx/blink_dagger_start.vpcf", PATTACH_ABSORIGIN, self:GetCaster())

	self:GetCaster():SetAbsOrigin(target_point)
	FindClearSpaceForUnit(self:GetCaster(), target_point, false)
    ParticleManager:SafeCreateParticle("particles/items_fx/blink_dagger_end.vpcf", PATTACH_ABSORIGIN, self:GetCaster())
