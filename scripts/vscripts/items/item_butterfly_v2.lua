...
LinkLuaModifier("modifier_item_butterfly_v2_cd", "items/item_butterfly_v2.lua", LUA_MODIFIER_MOTION_NONE)
...
function item_butterfly_v2:OnSpellStart()
    ...
    if self.caster:IsRangedAttacker() then
        duration = self:GetSpecialValueFor("dur_range")
    end
    self.caster:AddNewModifier(self.caster, self, "modifier_item_butterfly_v2_buff", {duration=duration})
	Timers:CreateTimer(0.1, function()
		if self:GetCooldownTimeRemaining() < 8 then
			self:EndCooldown()
...
function modifier_item_butterfly_v2_pa:GetModifierDodgeProjectile()
...
function modifier_item_butterfly_v2_buff:CheckState()return {[MODIFIER_STATE_UNSLOWABLE ]=true} end
...
function modifier_item_butterfly_v2_buff:GetModifierAvoidDamage(tg)
        if RollPseudoRandomPercentage(self.ch,0,self.parent) then
            return 1
        else
            return 0
