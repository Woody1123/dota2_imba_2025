...
function item_greaves_v2:OnSpellStart()
	...
	local fx = ParticleManager:SafeCreateParticle("particles/items3_fx/warmage.vpcf", PATTACH_ABSORIGIN, caster)
    ...
    for _, hero in pairs(heros) do
       -- if not hero:HasModifier("modifier_item_greaves_v2_buffcd") then
            ...
            local fx2 = ParticleManager:SafeCreateParticle("particles/items3_fx/warmage_recipient.vpcf", PATTACH_ABSORIGIN_FOLLOW, hero)
			
            ParticleManager:ReleaseParticleIndex(fx2)
            --hero:AddNewModifier(hero, self, "modifier_item_greaves_v2_buffcd", {duration=20})
            hero:Purge(false, true, false, true, true)
			hero:AddNewModifier(caster,self,"modifier_item_greaves_v2_buff",{duration = duration})
       -- end
	end
end
...
function modifier_item_greaves_v2_buff:GetModifierHealthRegenPercentage()
    return self.AHP
end
