...
function modifier_enchanted_edge_pa:OnIntervalThink()
	if IsServer() then
		self.buff = false
...
function modifier_enchanted_edge_pa:GetModifierBaseAttackTimeConstant()
	if self.buff == true then
		return 1.5
	end
end

function modifier_enchanted_edge_pa:GetModifierProcAttack_BonusDamage_Magical(keys)
	if not self:GetParent():IsIllusion() and not keys.target:IsBuilding() and self.buff == true then
		keys.target:AddNewModifier(self:GetParent(),self:GetAbility(),"modifier_enchanted_edge_debuff",{duration = 1})
		return 100
	end
end	
...
function modifier_enchanted_edge_pa:GetModifierAttackSpeedBonus_Constant()
    return self.buff == true and 1000 or 20
end
...
function modifier_enchanted_edge_buff:GetModifierBaseAttackTimeConstant()
	return 1.5
end
