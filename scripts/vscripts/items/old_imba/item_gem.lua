...
LinkLuaModifier("modifier_item_imba_soul_of_truth", "items/old_imba/item_gem", LUA_MODIFIER_MOTION_NONE)
function item_imba_soul_of_truth:OnSpellStart()
	local caster = self:GetCaster()
	caster:EmitSound("Item.DropGemWorld")
	caster:AddNewModifier(caster, self, "modifier_truesight_f", {duration = self:GetSpecialValueFor("duration"),num =1200})
	--[[
...
function modifier_item_imba_soul_of_truth:GetTexture() return "item_imba_soul_of_truth" end
...
function modifier_item_imba_soul_of_truth:IsPermanent() 		return true end
...
function modifier_item_imba_soul_of_truth:GetModifierPhysicalArmorBonus() return 6 end
function modifier_item_imba_soul_of_truth:GetModifierConstantHealthRegen() return 4 end
