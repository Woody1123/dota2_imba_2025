...
function Precache( context )
	...
	PrecacheResource("particle", "particles/econ/items/mirana/mirana_persona/mirana_persona_base_attack.vpcf", context )
	...
	PrecacheResource( "model", "models/heroes/bristleback/bristleback.vmdl", context )
	PrecacheResource( "model", "models/heroes/bristleback/bristleback_weapon.vmdl", context )
	PrecacheResource( "model", "models/heroes/bristleback/bristleback_head.vmdl", context )
	PrecacheResource( "model", "models/heroes/bristleback/bristleback_back.vmdl", context )
	PrecacheResource( "model", "models/heroes/bristleback/bristleback_necklace.vmdl", context )
	PrecacheResource( "model", "models/heroes/bristleback/bristleback_bracer.vmdl", context )
	PrecacheResource( "model", "models/heroes/mirana_persona/mirana_persona_armor.vmdl", context )
	PrecacheResource( "model", "models/heroes/mirana_persona/mirana_persona_back.vmdl", context )
	PrecacheResource( "model", "models/heroes/mirana_persona/mirana_persona_head.vmdl", context )
	PrecacheResource( "model", "models/heroes/mirana_persona/mirana_persona_mount.vmdl", context )
	PrecacheResource( "model", "models/heroes/mirana_persona/mirana_persona_weapon.vmdl", context )
	PrecacheResource( "model", "models/heroes/mirana_persona/mirana_persona_base.vmdl", context )
	...
	PrecacheResource( "model", "models/heroes/dragon_knight_persona/dk_persona_shoulder_cape.vmdl.vmdl", context )
	...
	PrecacheModel("models/courier/juggernaut_dog/juggernaut_dog.vmdl", context)
	--[[
...
function L_TG:InitGameMode()
	...
	-- local weeks= 604800
	-- local days = 86400
	-- local timelocal = GetSystemTime()
	-- print (timelocal)
	-- local datenow=GetSystemDate()
	-- print(datenow)
	-- print(GetSystemTimeMS())
	-- local day4 = (timenow)%weeks
	-- print(day4)
	-- if day4 < days then
	-- 	print("day4444")
	-- end
	...
	PrintTable(GameRules.Abilities_KV["special_bonus_unique_own_abaddon_8"])
	...
	--mode:SetCustomHeroMaxLevel(30)					--30
	...
	--设置喷泉恢复魔力的恒定速率。（默认为-1）
	mode:SetFountainConstantManaRegen(15)

	--设置喷泉恢复健康的百分比。（默认为-1）
	mode:SetFountainPercentageHealthRegen(15)
	...
	--mode:SetMinimumAttackSpeed(300)
	--设置单位的最低攻击速度。
	...
	if map =="6v6v6" then
		...
		GameRules:SetStartingGold(2000)--2000
		GameRules:SetCreepSpawningEnabled(true)
	...
	ListenToGameEvent("dota_rune_activated_serve", Dynamic_Wrap(L_TG, 'OnRunePickedUp'), self)
...
function L_TG:DamageFilter(tg)
	...
	--]]
	--[[
	if attacker and  unit and unit:HasAbility("reincarnation") and unit:IsRealHero() and unit:GetHealth()<=500  and  unit:GetHealth()<=tg.damage then
		local ab=unit:FindAbilityByName("reincarnation")
		if ab then
			if ab~=nil and ab:GetLevel()>0 and not  ab:IsCooldownReady()  and   unit:HasScepter() and not  unit:HasModifier("modifier_reincarnation_ghost") and not unit:HasModifier("modifier_reincarnation_ghost_cd") then
				unit:AddNewModifier( attacker, ab, "modifier_reincarnation_ghost", {duration=7})
				unit:AddNewModifier( attacker, ab, "modifier_reincarnation_attsp", {duration=7})
				tg.damage=0
			end
		end
	end]]
	...
	if attacker and  unit and unit:HasModifier("modifier_flame_guard_buff")then
		...
		if type==DAMAGE_TYPE_MAGICAL or unit:TG_HasTalent("special_bonus_ember_spirit_5") then
			if  stack>=dam then
				...
				tg.damage=dam-stack
				mod:SetStackCount(0).khhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh
				2j323wukongs_command
	...
	if unit and attacker and attacker:HasItemInInventory("item_bloodthorn_v2")  then
		if attacker~=unit and not unit:IsBuilding() then
	...
	--]]
	--[[
	if attacker and ability and (unit:GetAbsOrigin() - attacker:GetAbsOrigin()):Length2D() > 4000 then
