...
function modifier_imba_the_swarm_debuff:OnCreated(keys)
	...
	if IsServer() then
		...
		self.bug:AddNewModifier(self.caster, self.ability, "modifier_unit_remove", {duration = duration + 1})
...
function imba_weaver_time_lapse:OnSpellStart()
	...
	for i=1, #caster.illusions do
		...
		--caster.illusions[i]:AddNewModifier(caster, self, "modifier_unit_remove", {duration = duration+1})
...
function modifier_imba_time_lapse_new:CheckState() 
	if not self.parent:IsNull() and self.parent:IsIllusion() then
		return 
		{
			[MODIFIER_STATE_INVULNERABLE] = true,
			[MODIFIER_STATE_NO_UNIT_COLLISION] = true,
...
function modifier_imba_time_lapse_passive:OnIntervalThink(keys)
	...
	if self.hp > hp and self.parent:IsAlive()  and not self.parent:PassivesDisabled() then
