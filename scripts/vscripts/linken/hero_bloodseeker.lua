...
function imba_bloodseeker_rupture:OnSpellStart(scepter)
	...
	local duration = self:GetSpecialValueFor("duration")
	target:AddNewModifier(caster, self, "modifier_imba_rupture", {duration = duration})	
	if not caster:HasScepter()  then
		AbilityChargeController:AbilityChargeInitialize(self, self:GetCooldown(self:GetLevel()),1, 1, true, true)
		AbilityChargeController:GetChargeModifier(self):SetStackCount(0)
    end
...
function modifier_imba_rupture:OnCreated(keys)	
	...
	if IsServer() then
		...
		local damageTable = {
			...
			damage = self:GetParent():GetMaxHealth() * self:GetAbility():GetSpecialValueFor("hp_pct") * 0.01, 
			ability = self:GetAbility(), 
			damage_type = self:GetAbility():GetAbilityDamageType(),
...
function modifier_imba_rupture_kill:OnDestroy()
	...
	local damageTable = {
		attacker = self:GetCaster(), 
		victim = self:GetParent(), 
		damage = self:GetParent():GetMaxHealth() / 2, 
		ability = self:GetAbility(), 
		damage_type = self:GetAbility():GetAbilityDamageType(),
