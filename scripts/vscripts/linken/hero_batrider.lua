...
function imba_batrider_sticky_napalm:OnSpellStart(scepter)
	...
	self.pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_batrider/batrider_stickynapalm_impact.vpcf", PATTACH_POINT, caster)
...
self.duration = self:GetAbility():GetSpecialValueFor("duration")
	if IsServer() then
		...
		local caster = self:GetCaster()
		self.pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_alchemist/alchemist_acid_spray.vpcf", PATTACH_POINT, self:GetParent())
...
self.damage = self:GetAbility():GetSpecialValueFor("napalm_damage")
	if IsServer() then
		...
		--self.pfx = ParticleManager:SafeCreateParticle("particles/heros/batrider/imba_batrider_sticky_napalm.vpcf", PATTACH_POINT, self:GetParent())
		--[[self.pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_dragon_knight/dragon_knight_shard_fireball.vpcf", PATTACH_POINT, self:GetParent())
		ParticleManager:SetParticleControl(self.pfx, 0, self:GetParent():GetAbsOrigin())
		...
		self:AddParticle(self.pfx, false, false, 1, false, false)]]
		self.pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_shredder/shredder_flame_thrower.vpcf", PATTACH_POINT_FOLLOW, self:GetParent())
		ParticleManager:SetParticleControl(self.pfx, 0, self:GetParent():GetAbsOrigin())
		ParticleManager:SetParticleControl(self.pfx, 3, self:GetParent():GetAbsOrigin())
		self:AddParticle(self.pfx, false, false, -1, false, false)				
	end	
...
function modifier_imba_sticky_napalm_debuff:OnCreated()
	...
	if IsServer() then
		...
		self.pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_batrider/batrider_stickynapalm_stack.vpcf", PATTACH_OVERHEAD_FOLLOW, self:GetParent())
		ParticleManager:SetParticleControl(self.pfx, 0, self:GetParent():GetAbsOrigin())
...
function imba_batrider_flamebreak:OnSpellStart()
	...
	dummy:AddNewModifier(self:GetCaster(), nil, "modifier_unit_remove", {duration = self.duration})
	--if ability then
	...
	local flamebreak_particle = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_batrider/batrider_flamebreak.vpcf", PATTACH_WORLDORIGIN, nil)
...
function modifier_imba_batrider_firefly:OnCreated()
	...
	if not IsServer() then return end
		self.pfx = ParticleManager:SafeCreateParticle("particles/heros/batrider/imba_batrider_sticky_napalm.vpcf", PATTACH_POINT_FOLLOW, self:GetParent())
		ParticleManager:SetParticleControl(self.pfx, 0, self:GetParent():GetAbsOrigin())
		ParticleManager:SetParticleControl(self.pfx, 1, self:GetParent():GetAbsOrigin())
...
function imba_batrider_flaming_lasso:OnSpellStart()
	...
	if not self:GetAutoCastState() then
		...
		--	illusion:AddNewModifier(caster, self, "modifier_unit_remove", {duration=30})
...
function modifier_imba_batrider_flaming_lasso:OnCreated(params)
	...
	self.lasso_particle = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_batrider/batrider_flaming_lasso.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
