...
function imba_snapfire_scatterblast:OnSpellStart()
	...
	local projectile_start_radius = self:GetSpecialValueFor( "blast_width_initial" )/2+caster:GetCastRangeBonus()*0.4
	local projectile_end_radius = self:GetSpecialValueFor( "blast_width_end" )/2+caster:GetCastRangeBonus()*0.4
		...
		if self~=nil then
			...
			if lil_abi and lil_abi:GetLevel() > 0 then
				if caster:HasModifier("modifier_imba_snapfire_lil_shredder_buff") then
					caster:SetModifierStackCount("modifier_imba_snapfire_lil_shredder_buff", nil, caster:GetModifierStackCount("modifier_imba_snapfire_lil_shredder_buff", nil) + self.stack*2)
				else
					-- addd buff
					...
					lil_modifier:SetStackCount(self.stack*2)
				end
			end
		end
		return nil
...
function imba_snapfire_scatterblast:OnProjectileHit_ExtraData( target, location, extraData )
	...
	local point_blank_range = self:GetSpecialValueFor( "point_blank_range" )+caster:GetCastRangeBonus()*0.4
	local point_blank_mult = self:GetSpecialValueFor( "point_blank_dmg_bonus_pct" )/100
	local damage = self:GetSpecialValueFor( "damage" )+caster:TG_GetTalentValue("special_bonus_imba_snapfire_7")*caster:GetAttackDamage()*0.01
	...
	local knockback_table =
	{
		...
		knockback_distance = math.max(0,caster:Script_GetAttackRange()-100-(location-caster:GetOrigin()):Length2D()),
		knockback_height = 0,
	}
...
function imba_snapfire_firesnap_cookie:OnProjectileHit( target, location )
	...
	if not IsEnemy(self:GetCaster(),target) and self:GetCaster():TG_HasTalent("special_bonus_imba_snapfire_5") then
		local heal_num = self:GetCaster():TG_GetTalentValue("special_bonus_imba_snapfire_5")*target:GetMaxHealth()*0.01
...
-----------------------------
	...
	self.end_attacks = self.attacks
	-- will be changed dynamically for talents
	self.use_modifier = true

	if not IsServer() then return end
	self.end_attacks = math.ceil(self:GetParent():GetIncreasedAttackSpeed(false)/1.5)+self.end_attacks
	self:SetStackCount( self.end_attacks )
...
--[[	if params.attacker:HasScepter() then
	...
	if self:GetParent():TG_HasTalent("special_bonus_imba_snapfire_8") and self:GetStackCount()>(self.end_attacks-self.attacks) then
...
function modifier_imba_snapfire_lil_shredder_buff:OnAttackLanded( params )
	if self.records[params.record] and not params.target:IsBuilding() then
		-- add modifier
		params.target:AddNewModifier(
...
function modifier_imba_snapfire_lil_shredder_buff:GetModifierOverrideAttackDamage()
	if self:GetStackCount()<=0 then return end
	if self:GetCaster():TG_HasTalent("special_bonus_imba_snapfire_6") then
		return self.parent:GetAttackDamage() + self.damage
	end	
	return self.damage
end
