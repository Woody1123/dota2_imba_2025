...
function modifier_imba_antimage_mana_break:OnAttackLanded(keys)
	...
	if keys.target:IsMagicImmune() and not self:GetCaster():TG_HasTalent("special_bonus_imba_antimage_4")then
		return
	end
	...
	local nowMane = keys.target:GetMana()
	if keys.target:GetMaxMana() <= 0 then
		mana = self:GetAbility():GetSpecialValueFor("nilmana_max")
	end
	if nowMane<=self:GetAbility():GetSpecialValueFor("mana_stun") and not  keys.target:IsMagicImmune()  then
		keys.target:EmitSound("Hero_TrollWarlord.BerserkersRage.Stun")
        keys.target:AddNewModifier(keys.attacker,self:GetAbility(), "modifier_imba_stunned", {duration = self:GetAbility():GetSpecialValueFor("stun")})
	end		
...
function imba_antimage_blink:OnSpellStart()
   ...
   local modifier=
	...
	caster.illusions = CreateIllusions(caster, caster, modifier, 1, 0, true, true)
    ...
    for i=1, #caster.illusions do 
		...
		--caster.illusions[i]:AddNewModifier(caster, self, "modifier_unit_remove", {duration = self:GetSpecialValueFor("duration")})
        caster.illusions[i]:AddNewModifier(caster, self, "modifier_imba_illusion_no_order", {duration = self:GetSpecialValueFor("duration")})
		if mod then
            caster.illusions[i]:AddNewModifier(caster, nil, "modifier_item_attsp_book", {num=mod:GetStackCount()})
        end
    end	
   ...
   local modifier=
    ...
    for i=1, #caster.illusions do
		...
		Timers:CreateTimer(0.1, function()
			caster.illusions[i]:MoveToPositionAggressive(caster.illusions[i]:GetAbsOrigin())
		return nil
		end
		)
    end	
...
function imba_antimage_mana_void:OnSpellStart()
	...
	local dmg = 0
	target:Script_ReduceMana((math.max(0, ((mana * (self:GetSpecialValueFor("mana_void_mana_burn_pct") / 100))))),self)
...
function modifier_imba_mana_void_debuff:OnIntervalThink()
	if self:GetAbility() == nil  or self:GetAbility():IsNull() then
		self:Destroy()
		return
	end
