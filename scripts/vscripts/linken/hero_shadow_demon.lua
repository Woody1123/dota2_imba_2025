...
function imba_shadow_demon_disruption:OnSpellStart()
	...
	if not Is_Chinese_TG(caster,target) and target:HasModifier("modifier_black_king_bar_immune") then
		return 
	end
	...
	if not self:GetCaster():TG_HasTalent("special_bonus_imba_shadow_demon_4") then
		...
		for i=1, #caster.illusions do
			...
			--caster.illusions[i]:AddNewModifier(target, self, "modifier_unit_remove", {duration =duration})
...
function modifier_imba_disruption:OnCreated()
	...
	if IsServer() then
		...
		EmitSoundOn("Hero_ShadowDemon.Disruption", self:GetParent())
	   	self.pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_shadow_demon/shadow_demon_disruption.vpcf", PATTACH_CUSTOMORIGIN, nil)
...
function imba_shadow_demon_soul_catcher:OnSpellStart()
	...
	for _, enemy in pairs(enemies) do
		if enemy:IsInvulnerable()  then
			if enemy:HasModifier("modifier_imba_disruption") then
	...
	local particle_ground_fx = ParticleManager:SafeCreateParticle(particle_ground, PATTACH_WORLDORIGIN, nil)
...
function modifier_imba_soul_catcher:OnCreated(keys)
	...
	if IsServer() then
		...
		ApplyDamage(damageTable)
	   	self.pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_shadow_demon/shadow_demon_soul_catcher_debuff.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
	    ParticleManager:SetParticleControl(self.pfx, 0, self:GetParent():GetAbsOrigin())
	    ParticleManager:SetParticleControl(self.pfx, 1, self:GetParent():GetAbsOrigin())
	    self:AddParticle(self.pfx, false, false, -1, false, false)

	   	self.pfx_1 = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_shadow_demon/shadow_demon_soul_catcher.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
...
function modifier_imba_shadow_poison:IsPurgable() 			return false end
function modifier_imba_shadow_poison:IsPurgeException() 	return false end
...
function modifier_imba_shadow_poison:OnCreated(keys)
	...
	if IsServer() then
		...
		self.pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_shadow_demon/shadow_demon_shadow_poison_stackui.vpcf", PATTACH_OVERHEAD_FOLLOW, self:GetParent())
		ParticleManager:SetParticleControl(self.pfx, 0, self:GetParent():GetAbsOrigin())
...
function modifier_imba_shadow_poison:OnDestroy(keys)
	if IsServer() then
		...
		self.particle_kill_fx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_shadow_demon/shadow_demon_shadow_poison_kill.vpcf", PATTACH_ABSORIGIN, self:GetParent())
...
function modifier_imba_shadow_energy_thinker:OnCreated(keys)
	...
	if IsServer() then
		...
		self.dummy:AddNewModifier(self:GetCaster(), nil, "modifier_unit_remove", {duration = self.duration})
	   	self.pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_shadow_demon/shadow_demon_disruption.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
	    ParticleManager:SetParticleControl(self.pfx, 0, GetGroundPosition(self:GetParent():GetAbsOrigin(),self:GetParent())+Vector(0,0,150))
	    ParticleManager:SetParticleControl(self.pfx, 4, GetGroundPosition(self:GetParent():GetAbsOrigin(),self:GetParent())+Vector(0,0,150))
	    self:AddParticle(self.pfx, false, false, -1, false, false)
	    EmitSoundOn("Imba.Hero_shadow_demon.shadow_energy", self:GetParent())

		--[[self.pfx2 = ParticleManager:SafeCreateParticle("particles/basic_ambient/generic_range_display.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
...
function imba_shadow_demon_demonic_purge:CastFilterResultTarget(hTarget)
...
function modifier_imba_demonic_purge_debuff:OnCreated()
	...
	if IsServer() then
		...
		self.pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_shadow_demon/shadow_demon_demonic_purge.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
		ParticleManager:SetParticleControl(self.pfx, 0, self:GetParent():GetAbsOrigin())
		ParticleManager:SetParticleControl(self.pfx, 1, self:GetParent():GetAbsOrigin())
