...
function imba_void_spirit_aether_remnant:OnSpellStart()
	...
	dummy:AddNewModifier(self:GetCaster(), nil, "modifier_unit_remove", {duration = self:GetSpecialValueFor("duration")})
...
function modifier_imba_resonant_pulse:OnCreated()
	...
	if IsServer() then
		...
		if self:GetParent():HasModifier("modifier_imba_voidification") then 
			...
			local int_caster = self:GetParent():GetIntellect(false)
...
function modifier_imba_voidification:OnCreated()
	if self:GetAbility() == nil then return end
	if IsServer() then
		self:SetStackCount(self:GetParent():GetIntellect(false) * (self:GetAbility():GetSpecialValueFor("bouns_voidification") / 100))
	end
end
--[[
...
function imba_void_spirit_void_cut:Spell_VoidCut(vLocation,vRadius,vEffect)
	...
	for i = 1 , multiple do 
		Timers:CreateTimer(i * 0.3, function()
			...
			local pure_dmg = (self:GetCaster():GetPrimaryStatValue() * (self:GetSpecialValueFor("cut_damage") + self:GetCaster():TG_GetTalentValue("special_bonus_imba_void_spirit_2")))/100
