...
function imba_hoodwink_bushwhack:GetCastRange() return self:GetCaster():Script_GetAttackRange() + self:GetSpecialValueFor("bonus_range")-self:GetCaster():GetCastRangeBonus() end
...
function modifier_imba_hoodwink_scurry_passive:OnCreated(params)
	if self:GetAbility() == nil then return end
		...
		self.invisible_dtime = 1--ability:GetSpecialValueFor("invisible_dtime")
	if IsServer() then
		self.trees = false
...
function modifier_imba_hoodwink_scurry_passive:OnRefresh(params)
		...
		self.evasion = ability:GetSpecialValueFor("evasion_s")
		self.attack_range = ability:GetSpecialValueFor("attack_range_s")
		self.invisible_dtime = 2--ability:GetSpecialValueFor("invisible_dtime")
	if IsServer() then
		self.trees = false
		self.modifier_bool = false
		self.invisible = false
...
function modifier_imba_hoodwink_scurry_passive:OnIntervalThink(params)
	local caster = self:GetCaster()
	local ability = self:GetAbility()
	local trees = {1}--GridNav:GetAllTreesAroundPoint( self:GetParent():GetAbsOrigin(), self.radius, false )
	...
	if dummy ~= nil then
		trees_fa = true--(dummy:GetAbsOrigin()-caster:GetAbsOrigin()):Length2D() <= ability2:GetSpecialValueFor("special_range")
	end
	if #trees ~= 0 or trees_fa then
		self.trees = true
		self:SetStackCount(5)
		self.invisible_int = self.invisible_int + 1--FrameTime()
...
function modifier_imba_hoodwink_scurry:CheckState()
	return 	{	
...
function imba_hoodwink_decoy:OnSpellStart()
	...
	for i=1, #caster.illusions do
		...
		--caster.illusions[i]:AddNewModifier(caster, self, "modifier_unit_remove", {duration = duration})
