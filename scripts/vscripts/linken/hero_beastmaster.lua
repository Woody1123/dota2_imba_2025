...
function imba_beastmaster_wild_axes:OnSpellStart(int,tar)
	...
	dummy_pfx:AddNewModifier(self.caster, self, "modifier_unit_remove", {duration = 20})
	local info =
	{
		Target = dummy_end,
		Source = self.caster,
...
function imba_beastmaster_call_of_the_wild:OnSpellStart()
	...
	--self.boar = {}
	local num=1+self.caster:TG_GetTalentValue("special_bonus_imba_beastmaster_1")
	for a=1,num do
...
function modifier_imba_beastmaster_inner_beast_buff:IsDebuff()			return IsEnemy(self:GetParent(),self:GetCaster()) end
...
function modifier_imba_beastmaster_inner_beast_buff:IsHidden() 			return false end
...
function modifier_imba_beastmaster_inner_beast_buff:OnAttackLanded(keys)
	...
	if keys.attacker ~= self.parent and self:GetParent() then
		return
	end
	if (keys.attacker:IsRealHero() or keys.attacker:GetPlayerOwnerID() == self:GetCaster():GetPlayerOwnerID() ) and not keys.attacker:HasAbility("imba_beastmaster_inner_beast")and keys.attacker:HasModifier("modifier_imba_beastmaster_inner_beast_buff") and keys.target:IsAlive() and keys.target:IsUnit() and IsEnemy(keys.target,self:GetCaster()) and not IsEnemy(keys.attacker,self:GetCaster()) then
		local chl=self:GetAbility():GetSpecialValueFor("bash_chance")
		if self:GetAbility():Hasmodif() and self:GetCaster():HasScepter()  then
			chl=chl*2
		end
		if PseudoRandom:RollPseudoRandom(self:GetAbility(), chl) then
			self:GetCaster():PerformAttack(keys.target, false, true, true, false, true, false, true)
		end
	end
...
function modifier_imba_beastmaster_inner_beast_buff:GetModifierAttackSpeedBonus_Constant()
	...
	--
	--end
	local as_bonus=self.as_bonus
	if self:GetAbility():Hasmodif() then
		as_bonus=2*self.as_bonus
	end
