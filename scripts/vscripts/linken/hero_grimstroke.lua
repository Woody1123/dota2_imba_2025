...
function imba_grimstroke_ink_creature:OnSpellStart()
	...
	npc:AddNewModifier(caster, self, "modifier_unit_remove", {duration = 30})
...
function modifier_imba_ink_creature_movecontroller:OnAttackLanded(keys)
	...
	if dmg >= self.parent:GetHealth() then
		...
		--self.parent:RemoveAllModifiers()
		return
	end
...
function imba_grimstroke_dark_portrait:OnSpellStart()
	...
	local images_do_damage_percent = self:GetSpecialValueFor("images_do_damage_percent")+caster:GetLevel()*3
	...
	for i=1, #caster.illusions do
		...
		caster.illusions[i]:Set_HP(target:GetMaxHealth(),false)
		caster.illusions[i]:SetBaseStrength(target:GetStrength())
		caster.illusions[i]:SetBaseIntellect(target:GetIntellect(false))
		caster.illusions[i]:SetBaseAgility(target:GetAgility()+100)
	...
	--	caster.illusions[i]:AddNewModifier(caster, self, "modifier_unit_remove", {duration = illusion_duration})
		...
		caster.illusions[i]:AddNewModifier(caster, self, "modifier_imba_dark_portrait_mov", {duration = illusion_duration})
			...
			caster.illusions[i]:MoveToTargetToAttack(target)
			return nil
		end)
	end
end
