...
function modifier_imba_stand_we_divided:OnCreated()
	...
	if IsServer() then
		...
		local pfx = ParticleManager:SafeCreateParticleForPlayer("particles/basic_ambient/generic_range_display.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent(), self:GetParent():GetPlayerOwner())
...
function imba_meepo_earthbind:OnSpellStart()
	...
	self.pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_meepo/meepo_earthbind_projectile_fx.vpcf", PATTACH_CUSTOMORIGIN, nil)
...
function modifier_imba_meepo_earthbind_root:OnCreated()
	...
	if IsServer() then
		local caster = self:GetCaster()
		self.root 		= 	true
		self.pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_meepo/meepo_earthbind.vpcf", PATTACH_CUSTOMORIGIN, self:GetParent())
...
function imba_meepo_Poof:OnSpellStart()
	...
	for i=1, #clone do
		if clone[i]:GetName() == "npc_dota_hero_meepo" or clone[i]:HasModifier("modifier_imba_meepo_earthbind_root") then
			...
			local pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_meepo/meepo_poof_end.vpcf", PATTACH_CUSTOMORIGIN, self:GetCaster())
...
function modifier_imba_meepo_Poof_self:OnAbilityStart(keys) 
	...
	if keys.ability:GetAbilityName() == "imba_meepo_Poof" and keys.unit == self:GetCaster() then
		...
		local pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_meepo/meepo_poof_start.vpcf", PATTACH_CUSTOMORIGIN, self:GetCaster())
		ParticleManager:SetParticleControl(pfx, 0, self:GetCaster():GetAbsOrigin())
		ParticleManager:SetParticleControl(pfx, 1, self:GetCaster():GetAbsOrigin())
		ParticleManager:SetParticleControl(pfx, 2, self:GetCaster():GetAbsOrigin())
		ParticleManager:ReleaseParticleIndex(pfx)
...
function imba_meepo_ransack:OnSpellStart()
	...
	for i=1, #clone do
		...
		local pfx = ParticleManager:SafeCreateParticle("particles/econ/items/crystal_maiden/ti9_immortal_staff/cm_ti9_golden_staff_lvlup_globe.vpcf", PATTACH_POINT_FOLLOW, clone[i])
...
function modifier_imba_meepo_ransack_self:OnAttackLanded(keys)
	...
	for i=1, #clone do
		if clone[i]:GetName() == "npc_dota_hero_meepo" then
			local pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_meepo/meepo_ransack.vpcf", PATTACH_CUSTOMORIGIN, clone[i])
