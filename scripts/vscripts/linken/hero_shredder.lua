...
self.stat_loss_pct = self:GetAbility():GetSpecialValueFor( "stat_loss_pct" ) + self:GetCaster():TG_GetTalentValue("special_bonus_shredder_1")
	...
	self.stat_loss_st = -self.parent:GetBaseStrength() * self.stat_loss_pct / 100
	self.stat_loss_st2 = -self.parent:GetBaseStrength() * self.stat_loss_pct / 50
end
...
if IsServer() then
	...
	function modifier_imba_shredder_whirling_death:GetModifierBonusStats_Strength()
		...
		return self.stat_loss_st2 or 0
	end
end
...
function modifier_imba_shredder_timber_chain:OnDestroy()
	...
	if loc >= self:GetAbility():GetSpecialValueFor( "shard_range" ) and self:GetAbility().bool then
