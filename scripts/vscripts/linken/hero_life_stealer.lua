...
function imba_life_stealer_rage:OnSpellStart()
  ...
  local dur_tar = duration - self:GetCaster():TG_GetTalentValue("special_bonus_imba_life_stealer_5")
  --定位目标
...
function modifier_imba_life_stealer_rage_self:GetModifierMagicalResistanceBonus() return self.mag  end
...
function modifier_imba_life_stealer_feast_damage:OnAttack(keys)
  if not keys.attacker:PassivesDisabled() and keys.attacker == self:GetParent() then
    ...
    if keys.target and keys.target:IsBossCreature() or keys.target:IsBuilding() then
	  shuzhi2 = shuzhi2 * int * 0.5
    end
...
function modifier_imba_life_stealer_buff:OnCreated( params )
  ...
  if self:GetCaster():HasScepter() then
	self.item_15_cd = false
	self.item_15_cd_next = false
	if self:GetParent():GetItemInSlot(15) ~= nil then
		self.item_15_cd = self:GetParent():GetItemInSlot(15):GetCooldownTimeRemaining()
	end
    for i=0,4 do
...
function modifier_imba_life_stealer_buff:OnIntervalThink()
  ...
  if self.item_15_cd~=false then
	if self:GetParent():GetItemInSlot(15)~=nil then
		if self:GetParent():GetItemInSlot(15):GetCooldownTimeRemaining()>self.item_15_cd then
			self.order = true
		end
  end
