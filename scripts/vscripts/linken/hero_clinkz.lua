...
function modifier_imba_strafe_active:DeclareFunctions()
	return {
		 MODIFIER_PROPERTY_DODGE_PROJECTILE,}
end
function modifier_imba_strafe_active:GetModifierDodgeProjectile()
        return 1
end
...
function imba_clinkz_burning_army:OnSpellStart(keys)
	...
	self.illusion:AddNewModifier(caster, self, "modifier_unit_remove", {duration = self.duration})
...
function imba_clinkz_wind_walk:OnSpellStart()
	...
	local pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_clinkz/clinkz_windwalk.vpcf", PATTACH_ABSORIGIN_FOLLOW, caster)
...
function modifier_imba_skeleton_walk:DeclareFunctions()
	return {MODIFIER_PROPERTY_INCOMING_DAMAGE_CONSTANT,MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE, MODIFIER_PROPERTY_DISABLE_AUTOATTACK, MODIFIER_EVENT_ON_ATTACK_LANDED, MODIFIER_EVENT_ON_ATTACK, MODIFIER_EVENT_ON_ABILITY_EXECUTED, MODIFIER_PROPERTY_INVISIBILITY_LEVEL}
end
...
function modifier_imba_skeleton_walk:OnCreated()
	...
	if IsServer() then
		local pfx = "particles/econ/events/ti10/high_five/towers/dire_tower_2021/high_five_dire_tower_2021_travel_fire.vpcf"
		
		--local pfx = "particles/econ/items/warlock/warlock_golem_dark_curator/golem_ambient_fire_hand_r_dark_curator.vpcf"
		local pos = self.parent:GetAbsOrigin()
		pos.z = pos.z+100
		self.pfx = ParticleManager:SafeCreateParticle(pfx, PATTACH_CUSTOMORIGIN, nil)
        ParticleManager:SetParticleControl(self.pfx, 0, pos)
        self:AddParticle(self.pfx, false, false, -1, false, false)
		self.pos = self.parent:GetAbsOrigin()
	end
end
function modifier_imba_skeleton_walk:OnRefresh()
	if IsServer() then
...
function modifier_imba_skeleton_walk:GetModifierIncomingDamageConstant(keys)
	if IsServer() then
		if keys.target:GetHealth() - keys.damage <=0 then
				keys.target:SetHealth(keys.target:GetMaxHealth())
				keys.target:SetAbsOrigin(self.pos)
				
				local pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_clinkz/clinkz_death.vpcf", PATTACH_ABSORIGIN, self.parent)
				ParticleManager:ReleaseParticleIndex(pfx)
				EmitSoundOnLocationWithCaster(self.parent:GetAbsOrigin(), "Hero_Clinkz.WindWalk", self.parent)
				self:Destroy()
...
function modifier_imba_skeleton_walk:OnDestroy()
	if IsServer() then
		...
		if ability and ability:IsTrained() and self.caster:Has_Aghanims_Shard() then
			ability.duration = self.shard+self.caster:TG_GetTalentValue("special_bonus_imba_clinkz_2")
...
function imba_clinkz_death_pact:OnSpellStart()
	...
	if target:TG_TriggerSpellAbsorb(self) or target == CDOTA_PlayerResource.ROSHAN then
		return
	end
	...
	local death_pact_pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_clinkz/clinkz_death_pact.vpcf", PATTACH_ABSORIGIN, caster)
