...
function imba_phantom_lancer_spirit_lance:OnProjectileHit_ExtraData(target, pos, keys)
	...
	if target:IsAlive() and target and caster:IsAlive() and not caster:IsIllusion() then
		...
		--	caster.illusions[i]:AddNewModifier(caster, self, "modifier_unit_remove", {duration = 15})
...
function modifier_imba_juxtapose_passive:OnAttackStart( keys )
	if not IsServer() then return end
	if self:GetParent():PassivesDisabled() or self:GetParent():IsHexed() or self:GetParent():IsStunned() then
		return
	end
...
function modifier_imba_juxtapose_passive:OnAbilityStart( keys )
	if not IsServer() then return end
	if self:GetParent():PassivesDisabled() and self:GetParent():IsStunned() or self:GetParent():IsHexed() then
		return
	end
