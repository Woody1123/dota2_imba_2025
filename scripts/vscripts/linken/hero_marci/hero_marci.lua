...
CreateTalents("npc_dota_hero_marci", "linken/hero_marci/hero_marci")
...
function modifier_imba_marci_armor:OnCreated(keys)
	...
	if IsServer() then
		self.armor_int = math.abs(self:GetParent():GetPhysicalArmorValue(false)*0.7)
...
function imba_marci_guardian:AddMeAndOtherModif()
	local caster = self:GetCaster()		
	...
	caster:AddNewModifier(
				...
				if not caster:HasModifier("modifier_marci_guardian_buff") then
					caster:AddNewModifier(
					caster, 
					self, 
	...
	local enemies = FindUnitsInRadius(
			...
			false)
		for _,enemy in pairs(enemies) do
			if enemy ~= caster and not enemy:IsRangedAttacker() then
				enemy:AddNewModifier(
				caster, 
				self, 
				"modifier_imba_grapple_buff1", 
				...
				if not enemy:HasModifier("modifier_marci_guardian_buff") then
					enemy:AddNewModifier(
					caster, 
					self, 
					"modifier_marci_guardian_buff", 
		...
		if self.Mirana and self.Mirana:IsAlive() then
			self.Mirana:AddNewModifier(caster, self, "modifier_imba_grapple_buff1", {duration = -1})
			if not self.Mirana:HasModifier("modifier_marci_guardian_buff") then
...
function modifier_imba_grapple_passive:OnAttackLanded(keys)
	...
	if PseudoRandom:RollPseudoRandom(self.ability, self.ability:GetSpecialValueFor("chance_pct")) then
		self.ability:AddMeAndOtherModif()
