...
function modifier_imba_splitting_dagger_debuff:OnAbilityStart(keys)
	...
	if self:GetStackCount() == -1 then	
		...
		if keys.target == self:GetParent() and not IsEnemy(ability_caster,self:GetParent()) then
			...
			local cast_pfx = ParticleManager:SafeCreateParticle("particles/heros/spe/spe_sphere_rings.vpcf", PATTACH_ABSORIGIN_FOLLOW, keys.target)
...
function modifier_imba_splitting_dagger_debuff:OnCreated(keys)
	...
	if IsServer() then
		...
		self.pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_void_spirit/planeshift/void_spirit_planeshift_spells_stack.vpcf", PATTACH_OVERHEAD_FOLLOW, self:GetParent())
...
function modifier_imba_splitting_dagger_debuff:OnStackCountChanged(iStack)
	if IsServer() and self.pfx then
		...
		elseif self:GetStackCount() == -1 and not self.cast_pfx then
			ParticleManager:DestroyParticle(self.pfx, false)
			ParticleManager:ReleaseParticleIndex(self.pfx)
			local cast_pfx = ParticleManager:SafeCreateParticle("particles/econ/items/spectre/spectre_arcana/spectre_arcana_minigame_v2_death_target.vpcf", PATTACH_POINT_FOLLOW, self:GetParent())
			...
			self.cast_pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_spectre/spectre_desolate_debuff.vpcf", PATTACH_OVERHEAD_FOLLOW, self:GetParent())
...
function imba_spectre_spectral_dagger:OnProjectileHit_ExtraData(target, location, keys)
	if target and keys.sound then
		...
		if not target:IsHero() then
			local pfx = ParticleManager:SafeCreateParticle("particles/creatures/spectre/spectre_active_dispersion.vpcf", PATTACH_ABSORIGIN_FOLLOW, target)
...
function modifier_imba_dispersion_passive:OnTakeDamage(keys)
	...
	if keys.damage > self.caster:GetMaxHealth() * (self.hp_int / 100) then	
		...
		local pfx = ParticleManager:SafeCreateParticle("particles/creatures/spectre/spectre_active_dispersion.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetCaster())
