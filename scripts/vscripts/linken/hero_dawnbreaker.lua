...
function imba_dawnbreaker_fire_wreath:OnAbilityPhaseStart()
	...
	local int = math.min(math.max(caster:GetAttacksPerSecond(false)*0.5,1), 2)
...
function modifier_imba_dawnbreaker_fire_wreath:OnCreated()
	...
	if IsServer() then
		local caster = self:GetCaster()
		self.cishu = 1
		local pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_dawnbreaker/dawnbreaker_fire_wreath_sweep_cast.vpcf", PATTACH_CUSTOMORIGIN, caster)
		ParticleManager:SetParticleControl(pfx, 0, self:GetCaster():GetAbsOrigin())
		ParticleManager:ReleaseParticleIndex(pfx)
		local pfx2 = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_dawnbreaker/dawnbreaker_fire_wreath_sweep.vpcf", PATTACH_CUSTOMORIGIN, caster)
		...
		for i=1, #enemy do
			...
			--local pfx3 = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_dawnbreaker/dawnbreaker_fire_wreath_impact.vpcf", PATTACH_CUSTOMORIGIN, enemy[i])
...
function modifier_imba_dawnbreaker_fire_wreath:OnIntervalThink()
	...
	if 	self.cishu == self.total_attacks then
		...
		local pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_dawnbreaker/dawnbreaker_fire_wreath_sweep_overhead.vpcf", PATTACH_CUSTOMORIGIN, caster)
		...
		local pfx2 = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_dawnbreaker/dawnbreaker_fire_wreath_smash.vpcf", PATTACH_CUSTOMORIGIN, caster)
...
function imba_dawnbreaker_celestial_hammer:OnSpellStart()
	...
	if caster:HasModifier("modifier_imba_dawnbreaker_hammer") then
		...
		local pfx_fly = ParticleManager:SafeCreateParticle("particles/econ/items/sven/sven_warcry_ti5/sven_spell_warcry_ti_5.vpcf", PATTACH_CUSTOMORIGIN, sound)
		...
		local pfx_fly2 = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_dawnbreaker/dawnbreaker_celestial_hammer_grounded.vpcf", PATTACH_CUSTOMORIGIN, sound)
...
function modifier_imba_dawnbreaker_hammer_thinker:OnCreated()
	...
	if IsServer() then
		...
		self.pfx = ParticleManager:SafeCreateParticle("particles/basic_ambient/generic_range_display.vpcf", PATTACH_ABSORIGIN_FOLLOW, parent)
...
function modifier_imba_dawnbreaker_hammer_thinker:OnIntervalThink()
	...
	if not self:GetCaster():HasModifier("modifier_imba_dawnbreaker_hammer") then
		--Timers:CreateTimer(0.2, function()
			local pfx_fly3 = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_dawnbreaker/dawnbreaker_solar_guardian_landing.vpcf", PATTACH_CUSTOMORIGIN, nil)
	...
	if self:GetStackCount() >= self:GetAbility():GetSpecialValueFor("attack_count") then
		...
		for i=1, #enemy do
			if (not self:GetAbility():GetToggleState() and not IsEnemy(keys.attacker, enemy[i])) or (keys.attacker:HasScepter() and not IsEnemy(keys.attacker, enemy[i])) then
				...
				local pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_dawnbreaker/dawnbreaker_luminosity.vpcf", PATTACH_CUSTOMORIGIN, keys.attacker)
...
local caster = self:GetCaster()
	if IsServer() then
		if keys.attacker == caster and caster:HasModifier("modifier_imba_luminosity_effect") then
			local int = keys.attacker:GetAttacksPerSecond(false) + 1
...
function modifier_imba_luminosity_effect:OnCreated()
	if self:GetAbility() == nil then return end
	if IsServer() then
		self.pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_dawnbreaker/dawnbreaker_luminosity_attack_buff.vpcf", PATTACH_CUSTOMORIGIN, self:GetParent())
...
function modifier_imba_dawnbreaker_solar_guardian:OnIntervalThink()
	...
	local pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_phoenix/phoenix_supernova_death.vpcf", PATTACH_CUSTOMORIGIN, dummy)
...
function modifier_imba_dawnbreaker_solar_guardian:OnDestroy()
	if IsServer() then
		...
		caster:RemoveNoDraw()
		FindClearSpaceForUnit(caster, self.pos, false)
		local pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_phoenix/phoenix_supernova_reborn.vpcf", PATTACH_CUSTOMORIGIN, caster)
		ParticleManager:SetParticleControl(pfx, 0, self:GetCaster():GetAbsOrigin())
...
function modifier_imba_solar_guardian_thinker:OnCreated(keys)
	...
	if IsServer() then		
		if keys.int == 1 then
			...
			local pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_dawnbreaker/dawnbreaker_solar_guardian_aoe.vpcf", PATTACH_CUSTOMORIGIN, self.dummy)
