...
function modifier_imba_spider_strikes_immune:GetModifierModelScale() return self.scale end
...
function modifier_imba_spin_web_caster_aura:OnIntervalThink()
	if self.caster:HasModifier("modifier_imba_insatiable_hunger") then
		...
		if (self.caster:GetAbsOrigin() - self.parent:GetAbsOrigin()):Length2D() <= (ability:GetSpecialValueFor("web_radius") + self.caster:TG_GetTalentValue("special_bonus_imba_broodmother_6")) then
			AddFOWViewer(self.caster:GetTeamNumber(), self.parent:GetAbsOrigin(), self.radius, 1, false)
		end
	end
end
...
function modifier_imba_spin_web_buff:CheckState()
	if (0 - self:GetStackCount() / 10) >= self.fade_time then
		return {[MODIFIER_STATE_FLYING_FOR_PATHING_PURPOSES_ONLY] = true, [MODIFIER_STATE_INVISIBLE] = true,[MODIFIER_STATE_NO_HEALTH_BAR] = true,}
	else
		return {[MODIFIER_STATE_FLYING_FOR_PATHING_PURPOSES_ONLY] = true ,[MODIFIER_STATE_NO_HEALTH_BAR] = true,}
	end
end
...
function modifier_imba_spin_web_buff:OnIntervalThink()
	self:SetStackCount(math.max(self:GetStackCount() - 5, 0 - self.fade_time * 10))
	AddFOWViewer(self.parent:GetTeamNumber(), self.parent:GetAbsOrigin(), self.vision_radius, 0.5, false)
end
...
function modifier_imba_spin_web_debuff:OnIntervalThink()
	...
	self:SetStackCount(self:GetStackCount() + 20)
