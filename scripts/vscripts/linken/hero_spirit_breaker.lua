...
function modifier_imba_spirit_breaker_charge_of_darkness_target:OnCreated(keys)
	...
	if IsServer() then
		...
		self:StartIntervalThink(FrameTime())
	   	self.pfx = ParticleManager:SafeCreateParticleForTeam("particles/units/heroes/hero_spirit_breaker/spirit_breaker_charge_target.vpcf", PATTACH_OVERHEAD_FOLLOW, self:GetParent(), self:GetCaster():GetTeamNumber())
...
function modifier_imba_spirit_breaker_charge_of_darkness_caster:OnCreated(keys)
	...
	if IsServer() then	
		...
		self:GetAbility():EndCooldown()
	   	self.pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_spirit_breaker/spirit_breaker_charge.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
...
function modifier_imba_spirit_breaker_bulldoze:OnCreated(keys)
	...
	if IsServer() then
		...
		for _, enemy in pairs(enemies) do
	   	...
	   	self.pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_spirit_breaker/spirit_breaker_haste_owner.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
	    ParticleManager:SetParticleControlEnt(self.pfx, 0, self:GetCaster(), PATTACH_ABSORIGIN_FOLLOW, "attach_hitloc", self:GetCaster():GetAbsOrigin(), true)
	    self:AddParticle(self.pfx, false, false, -1, true, false)		
	end
end
...
function modifier_imba_spirit_breaker_greater_bash_passive:Bash(target, parent, bool, bool2)
	...
	target:SetForwardVector(Vector(caster:GetForwardVector()[1], caster:GetForwardVector()[2], 0))					
   	self.pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_spirit_breaker/spirit_breaker_greater_bash.vpcf", PATTACH_CENTER_FOLLOW, target)
...
function modifier_spirit_breaker_nether_strike_caster:OnCreated(keys)
	if self:GetAbility() == nil then return end
	if IsServer() then	
	   	self.pfx = ParticleManager:SafeCreateParticle("particles/econ/items/spirit_breaker/spirit_breaker_iron_surge/spirit_breaker_charge_iron.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
	    ParticleManager:SetParticleControlEnt(self.pfx, 0, self:GetCaster(), PATTACH_ABSORIGIN_FOLLOW, "attach_hitloc", self:GetCaster():GetAbsOrigin(), true)
	    self:AddParticle(self.pfx, false, false, -1, true, false)
