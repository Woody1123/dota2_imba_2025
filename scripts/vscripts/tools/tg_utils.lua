...
function TG_Kill(caster, tar, ab)
  ...
  local husk = false --不斩杀用
  if (tar:HasModifier("modifier_imba_berserkers_blood_passive") and tar:HasModifier("modifier_item_aghanims_shard"))
	or tar:HasModifier("modifier_imba_faceless_void_chronosphere") 
	or tar:HasModifier("modifier_reincarnation_ghost") 
	or (tar:HasModifier("modifier_reincarnation") and not tar:FindModifierByName("modifier_reincarnation"):GetAbility():IsCooldownReady())
	or (tar:TG_HasTalent("special_bonus_pudge_7") and not tar:HasModifier("modifier_flesh_heap_cd"))
	or tar:HasModifier("modifier_flesh_heap_buff") then
		husk = true
		return false
  end
  if not husk then
...
function TG_AbilityBook_Get_Normal(hero)
  if not hero then
    return nil
  end
  local name=hero:GetUnitName()
  local rda=RandomAbility_normal
  if TableContainsValue(RandomAbilityHero,name) then
      rda=RandomAbility2_normal
  end
  if name=="npc_dota_hero_tinker" then
      rda=TK_RD_normal
  end
...
function TG_AbilityBook_Get_funny(hero)
  if not hero then
    return nil
  end
  local name=hero:GetUnitName()
  local rda=RandomAbility_funny
  if TableContainsValue(RandomAbilityHero,name) then
      rda=RandomAbility_funny
  end
  if name=="npc_dota_hero_tinker" then
      rda=TK_RD_funny
  end
...
function CDOTA_BaseNPC:TG_Find_Tower(func)
  ...
  for aa=1,#CDOTAGameRules.TOWER do
      if not CDOTAGameRules.TOWER[aa]:IsNull() and Is_Chinese_TG(CDOTAGameRules.TOWER[aa],self) then
          if func then
