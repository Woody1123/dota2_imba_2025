...
function GetRandomAbilityNormal()
	...
	--print("start get random ability ---",hero)
	...
	local loop_count = 0
	while (loop_count <= count and IsInTable(ability,Random_Abilities_Table)) do
	--while not ability do
		ability = Random_Abilities_Normal[hero]['Ability'..RandomInt(1, count)]
		loop_count = loop_count + 1
		--print("same ability",loop_count)
	end
	--标记已经使用的随机技能 05-28
	--print("hero add random ability",hero,ability)
	if not IsInTable(ability,Random_Abilities_Table) then 
		table.insert(Random_Abilities_Table,ability)
	end
	return {hero, ability}
end
...
function GetRandomAKAbility_Event()
	...
	for i=1, 3 do
		...
		rand_tab[2 + (i - 1) * 6] = {"npc_dota_hero_lion", "imba_lion_finger_of_death"}
		rand_tab[3 + (i - 1) * 6] = {"npc_dota_hero_pudge", "imba_pudge_flesh_heap"}
		rand_tab[4 + (i - 1) * 6] = {"npc_dota_hero_troll_warlord", "imba_troll_warlord_fervor"}
		rand_tab[5 + (i - 1) * 6] = {"npc_dota_hero_dazzle", "dazzle_bad_juju"}
		rand_tab[6 + (i - 1) * 6] = {"npc_dota_hero_storm_spirit", "imba_storm_spirit_ball_lightning"}
	end
...
function IsRefreshableByAbility(str)
	local NO_REFRESH = {
		...
		["item_black_king_bar_tg"] = true,
		["item_xin_black_king_bar"] = true,
		["item_xin_white_queen_cape"] = true,
		["item_tg_black_king_bar"] = true,
		["item_meteor_hammer"] = true,
		["item_tg_qt_s"] = true,
		["item_tg_tortoise"] = true,
		["item_tg_drums"] = true,
		["item_tg_siege"] = true,
		["item_tg_skadi"] = true,
		["item_tg_bkb"] = true,
