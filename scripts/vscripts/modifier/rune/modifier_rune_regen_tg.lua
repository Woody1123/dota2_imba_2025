...
function modifier_rune_regen_tg:OnIntervalThink()
    ...
    self:GetParent():Heal( 300,  self:GetParent() )
    SendOverheadEventMessage(self:GetParent(), OVERHEAD_ALERT_HEAL, self:GetParent(),300, nil)
    self:GetParent():GiveMana(100)
    SendOverheadEventMessage(self:GetParent(), OVERHEAD_ALERT_MANA_ADD, self:GetParent(),100, nil)
end