...
function OnStartTouch(trigger)
	...
	local npc_pos = npc:GetOrigin()
	if npc ~= nil then		
		local meat_hook = npc:FindAbilityByName("trap_hook")
		meat_hook:SetLevel(1)
		npc:SetCursorPosition(target_pos)
		meat_hook:OnSpellStart(team)
		
		npc:SetCursorPosition(Vector(0,0,0))
		npc:SetContextThink( "ResetButtonModel", function() ResetButtonModel() end, 10 )
		--npc:CastAbilityOnPosition(target:GetOrigin(), meat_hook, trigger.activator:GetPlayerOwnerID() )
	...
	DoEntFire( button, "SetAnimation", "ancient_trigger001_up", 9.5, self, self )
	DoEntFire( button, "SetAnimation", "ancient_trigger001_idle", 10.5, self, self )
...
function OnStartTouch(trigger)
	...
	if npc1 ~= nil then
		...
		DoEntFire( model1, "SetAnimation", "fang_attack", 0, self, self )
	end
	...
	if npc2 ~= nil then
		...
		DoEntFire( model2, "SetAnimation", "fang_attack", 0, self, self )
	end
