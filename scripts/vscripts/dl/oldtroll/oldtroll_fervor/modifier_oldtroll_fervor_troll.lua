...
function modifier_oldtroll_fervor_troll:OnAttackLanded(params)
    ...
    if params.target==CDOTA_PlayerResource.ROSHAN then return end --如果攻击肉山那没事了
    ...
    if PseudoRandom:RollPseudoRandom(ability, ability:GetSpecialValueFor("fervor_chance")) then
        ...
        params.target:AddNewModifier_RS(caster,ability, "modifier_imba_stunned", {duration = stundur})
    end
