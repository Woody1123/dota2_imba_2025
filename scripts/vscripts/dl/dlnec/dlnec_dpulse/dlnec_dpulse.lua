...
function dlnec_dpulse:OnProjectileHit(target, location)
    ...
    local heal = self:GetSpecialValueFor("dpulse_heal") + caster:GetIntellect(false)*intco
    local damage = self:GetSpecialValueFor("dpulse_damage") + caster:GetIntellect(false)*intco + caster:GetIntellect(false)*caster:TG_GetTalentValue("special_bonus_dlnec_20r")   --基础伤害+智力系数+天赋智力系数
...
function dlnec_dpulse:OnUpgrade()
	local ab = self:GetCaster():FindAbilityByName("necrolyte_death_pulse")
	if ab~=nil then
