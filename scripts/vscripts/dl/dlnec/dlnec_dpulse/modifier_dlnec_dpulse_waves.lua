...
function modifier_dlnec_dpulse_waves:OnIntervalThink()
    ...
    local min_manap = ability:GetSpecialValueFor("dpulse_minmana_percent")*caster:GetMaxMana()*0.01
    if now_manan < min_manap or now_manan < 200 then self:Destroy() end
	...
	print((caster:GetMana()-spendmana))
	print(caster:GetMaxMana())
	print(min_manap)
	if (caster:GetMana()-spendmana)<min_manap then
		self:Destroy()
	end
end
