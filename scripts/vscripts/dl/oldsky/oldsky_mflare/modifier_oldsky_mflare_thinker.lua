...
function modifier_oldsky_mflare_thinker:OnCreated()
	...
	self.damage = self:GetAbility():GetSpecialValueFor("damage_base")
	self.damageTable = {
...
function modifier_oldsky_mflare_thinker:OnIntervalThink()
	local damage = (self.damage+self:GetCaster():GetIntellect(false)*self:GetAbility():GetSpecialValueFor("int_damage"))/(self:GetDuration() / 0.1)  
	local pos = GetRandomPosition2D(self:GetParent():GetAbsOrigin(), self:GetAbility():GetSpecialValueFor("mflare_radius") * 0.8)      --特效用      --每次打击伤害
...
function modifier_oldsky_mflare_thinker:OnRemoved()
    ...
    local dmg = self:GetCaster():GetIntellect(false)*self:GetAbility():GetSpecialValueFor("mflare_intco_exdam")      --爆炸智力伤害
    local radius = self:GetCaster():GetIntellect(false)*self:GetAbility():GetSpecialValueFor("mflare_intco_exrad") + self:GetAbility():GetSpecialValueFor("mflare_radius")   --爆炸半径扩大
