...
function oldsky_abolt:OnProjectileHit(target, location)
    ...
    if target:IsMagicImmune() or target:TG_TriggerSpellAbsorb(self) or not target:IsAlive() then return end
	...
	local dmg = self:GetSpecialValueFor("abolt_damage") + caster:GetIntellect(false) * intco
	local damageTable = {
						victim = target,
						attacker = caster,
						damage = dmg,
