...
function dlzuus_ts:OnSpellStart()
	...
	local projectile_distance = self:GetSpecialValueFor("ts_range")+caster:GetCastRangeBonus()+caster:Script_GetAttackRange()-caster:GetBaseAttackRange()
...
function dlzuus_ts:OnProjectileHitHandle(t, pos, ipro)
	...
	if t:IsHero() or t:IsBoss() then
		...
		local aoedam = self:GetSpecialValueFor("ts_aoedam")+self:GetSpecialValueFor("ts_dam_att")*self:GetCaster():GetBaseDamageMax()*0.01
