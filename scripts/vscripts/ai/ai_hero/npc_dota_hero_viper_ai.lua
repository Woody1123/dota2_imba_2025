if npc_dota_hero_viper_ai == nil then
	npc_dota_hero_viper_ai = ({})
end

function npc_dota_hero_viper_ai:IsHidden() return false end
function npc_dota_hero_viper_ai:IsDebuff() return false end
function npc_dota_hero_viper_ai:IsPurgable() 		return false end
function npc_dota_hero_viper_ai:IsPurgeException() 	return false end
function npc_dota_hero_viper_ai:RemoveOnDeath() 	return false end
function npc_dota_hero_viper_ai:OnCreated()
	...
	self.talent_table = {
		...
		"special_bonus_custom_value_asp50",
		"special_bonus_viper_3",
		"special_bonus_viper_4",
		"special_bonus_viper_5",
		"special_bonus_viper_7",
		"special_bonus_custom_value_str40",
	...
	if IsServer() then
		...
		self.ability_1 = self.parent:FindAbilityByName("poison_attack")
		self.ability_2 = self.parent:FindAbilityByName("nethertoxin")	
		self.ability_3 = self.parent:FindAbilityByName("corrosive_skin")	
		self.ability_4 = self.parent:FindAbilityByName("viper_strike")
...
function npc_dota_hero_viper_ai:think()
	if IsServer() then
		--等级相关数据
...
--[[
function npc_dota_hero_viper_ai:use_combo(hero)
...
function npc_dota_hero_viper_ai:use_item(hero,table_ab_1,table_ab_2)
	if IsServer() then
		if hero.stat ~= 2 then
...
function npc_dota_hero_viper_ai:use_ability(hero,table_ab_1,table_ab_2)
	if not IsServer() then return end
	...
	if self.parent:GetMana() > 250 and self.ability_4 and self.ability_4:IsCooldownReady() and #table_ab_1 > 0 then
...
--[[
function npc_dota_hero_viper_ai:learn_ability(lv)
...
function npc_dota_hero_viper_ai:add_item(lv)
	if lv <= 6 then
...
function npc_dota_hero_viper_ai:leart_talent(lv)   
	for i = (lv-1)*4+1, lv*4 do
		self.parent:AddNewModifier(self.parent,nil,self.talent_table[i],{})
	end
end

function npc_dota_hero_viper_ai:learn_veteran_talent(lv)
