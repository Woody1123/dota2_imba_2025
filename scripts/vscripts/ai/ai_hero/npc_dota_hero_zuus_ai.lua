if npc_dota_hero_zuus_ai == nil then
	npc_dota_hero_zuus_ai = ({})
end

function npc_dota_hero_zuus_ai:IsHidden() return false end
function npc_dota_hero_zuus_ai:IsDebuff() return false end
function npc_dota_hero_zuus_ai:IsPurgable() 		return false end
function npc_dota_hero_zuus_ai:IsPurgeException() 	return false end
function npc_dota_hero_zuus_ai:RemoveOnDeath() 	return false end
function npc_dota_hero_zuus_ai:OnCreated()
	self.parent = self:GetParent()
	self.hero_lv = 7
	self.unit_table = {}
	self.item_table = {"item_premium_power_treads","item_revenants_brooch","item_imba_gungnir","item_bkb","item_sheepstick_v2","item_skadi_v2"}
	...
	if IsServer() then
		...
		self.parent.move = falses
		self.ability_1 = self.parent:FindAbilityByName("dlzuus_al")
		self.ability_2 = self.parent:FindAbilityByName("dlzuus_ts")	
		self.ability_3 = self.parent:FindAbilityByName("dlzuus_sf")	
		self.ability_4 = self.parent:FindAbilityByName("zuus_cloud")
		self.ability_5 = self.parent:FindAbilityByName("zuus_thundergods_wrath")
		self.ability_6 = self.parent:FindAbilityByName("zuus_lightning_bolt")
...
function npc_dota_hero_zuus_ai:think()
	if IsServer() then
		--等级相关数据
		...
		--
		--物品
		self.gung = self.parent:GetItemInSlot(2)
		self.bkb = self.parent:GetItemInSlot(3)
		self.brooch = self.parent:GetItemInSlot(1)
		self.sheep = self.parent:GetItemInSlot(4)
...
--[[
function npc_dota_hero_zuus_ai:use_combo(hero)
...
function npc_dota_hero_zuus_ai:use_item(hero,table_ab_1,table_ab_2)
	if IsServer() then
		if hero.stat ~= 2 then
			if self.sheep and self.sheep:IsCooldownReady() and #table_ab_1>0 and not table_ab_1[1]:IsMagicImmune() then
				hero:CastAbilityOnTarget(table_ab_1[1],self.sheep,self.id)
			end
			if self.bkb and self.bkb:IsCooldownReady() and #table_ab_2 > 0  then
				hero:CastAbilityNoTarget(self.bkb,self.id)
			end
			if self.brooch and self.brooch:IsCooldownReady() and #table_ab_2 >0 then
...
function npc_dota_hero_zuus_ai:use_ability(hero,table_ab_1,table_ab_2)
	if not IsServer() then return end
		
	if self.parent:GetMana() > 145  and self.ability_2 and self.ability_2:IsCooldownReady() and #table_ab_1 > 0 then
		hero:CastAbilityOnPosition(table_ab_1[1]:GetAbsOrigin(),self.ability_2,self.id)
	end
	if self.parent:GetMana() > 500 and self.ability_4 and self.ability_4:IsCooldownReady() and #table_ab_1 > 4 then
		hero:CastAbilityOnTarget(table_ab_1[#table_ab_1],self.ability_4,self.id)
	end
	if self.parent:GetHealth()/self.parent:GetMaxHealth() < 0.5  and self.ability_3 and self.ability_3:IsCooldownReady() and #table_ab_2 > 0 then
		hero:CastAbilityOnPosition(self.parent:GetTeam() == 2 and GOODGUYS_HOME or BADGUYS_HOME,self.ability_3,self.id)
	end
end


--[[
function npc_dota_hero_zuus_ai:learn_ability(lv)
...
function npc_dota_hero_zuus_ai:add_item(lv)
	if lv <= 6 then
...
function npc_dota_hero_zuus_ai:leart_talent(lv)   
	for i = (lv-1)*4+1, lv*4 do
		self.parent:AddNewModifier(self.parent,nil,self.talent_table[i],{})
	end
end

function npc_dota_hero_zuus_ai:learn_veteran_talent(lv)
