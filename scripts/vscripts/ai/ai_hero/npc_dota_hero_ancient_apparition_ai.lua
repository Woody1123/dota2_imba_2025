if npc_dota_hero_ancient_apparition_ai == nil then
	npc_dota_hero_ancient_apparition_ai = ({})
end

function npc_dota_hero_ancient_apparition_ai:IsHidden() return false end
function npc_dota_hero_ancient_apparition_ai:IsDebuff() return false end
function npc_dota_hero_ancient_apparition_ai:IsPurgable() 		return false end
function npc_dota_hero_ancient_apparition_ai:IsPurgeException() 	return false end
function npc_dota_hero_ancient_apparition_ai:RemoveOnDeath() 	return false end
function npc_dota_hero_ancient_apparition_ai:OnCreated()
	self.parent = self:GetParent()
	self.hero_lv = 7
	self.unit_table = {}
	self.item_table = {"item_premium_power_treads","item_imba_gungnir","item_bkb","item_imba_thirst","item_greater_crit2","item_skadi_v2"}
	self.veteran_talent_table = {"modifier_veteran_talent_3","modifier_veteran_talent_1"}
	self.ability_table = {""}
	...
	if IsServer() then
		...
		self.ability_1 = self.parent:FindAbilityByName("cold_feet")
		self.ability_2 = self.parent:FindAbilityByName("ice_vortex")	
		self.ability_3 = self.parent:FindAbilityByName("chilling_touch")	
		self.ability_4 = self.parent:FindAbilityByName("ice_blast")
...
function npc_dota_hero_ancient_apparition_ai:think()
	if IsServer() then
		--等级相关数据
...
--[[
function npc_dota_hero_ancient_apparition_ai:use_combo(hero)
...
function npc_dota_hero_ancient_apparition_ai:use_item(hero,table_ab_1,table_ab_2)
	if IsServer() then
		if hero.stat ~= 2 then
			if self.bkb and self.bkb:IsCooldownReady() and #table_ab_2 > 0 then
				hero:CastAbilityNoTarget(self.bkb,self.id)
			end
			if self.stan and self.stan:IsCooldownReady() and #table_ab_2 > 0 
...
function npc_dota_hero_ancient_apparition_ai:use_ability(hero,table_ab_1,table_ab_2)
	if not IsServer() then return end
	if self.parent:GetMana() > 400 and  self.ability_4:IsCooldownReady() and #table_ab_1 > 0 then
		hero:CastAbilityOnPosition(table_ab_1[#table_ab_1]:GetAbsOrigin(),self.ability_4,self.id)
	end
	if self.parent:GetMana() > 100 and self.ability_1 and self.ability_1:IsCooldownReady() and #table_ab_1 > 0 then
		hero:CastAbilityOnTarget(table_ab_1[1],self.ability_1,self.id)
	end
	if self.parent:GetMana() > 100 and self.ability_2 and self.ability_2:IsCooldownReady() and #table_ab_1 > 0 then
...
--[[
function npc_dota_hero_ancient_apparition_ai:learn_ability(lv)
...
function npc_dota_hero_ancient_apparition_ai:add_item(lv)
	if lv <= 6 then
...
function npc_dota_hero_ancient_apparition_ai:leart_talent(lv)   
	for i = (lv-1)*4+1, lv*4 do
		self.parent:AddNewModifier(self.parent,nil,self.talent_table[i],{})
	end
end

function npc_dota_hero_ancient_apparition_ai:learn_veteran_talent(lv)
