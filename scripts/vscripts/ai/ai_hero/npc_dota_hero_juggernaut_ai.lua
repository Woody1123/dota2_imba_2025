if npc_dota_hero_juggernaut_ai == nil then
	npc_dota_hero_juggernaut_ai = ({})
end

function npc_dota_hero_juggernaut_ai:IsHidden() return false end
function npc_dota_hero_juggernaut_ai:IsDebuff() return false end
function npc_dota_hero_juggernaut_ai:IsPurgable() 		return false end
function npc_dota_hero_juggernaut_ai:IsPurgeException() 	return false end
function npc_dota_hero_juggernaut_ai:RemoveOnDeath() 	return false end
function npc_dota_hero_juggernaut_ai:OnCreated()
	self.parent = self:GetParent()
	self.hero_lv = 7
	self.unit_table = {}
	self.item_table = {"item_premium_power_treads","item_imba_harpoon","item_imba_thirst","item_butterfly_ex","item_greater_crit2","item_battle_fury"}
	...
	if IsServer() then
		...
		self.ability_1 = self.parent:FindAbilityByName("blade_fury")
		self.ability_2 = self.parent:FindAbilityByName("healing_ward")	
		self.ability_3 = self.parent:FindAbilityByName("blade_dance")	
		self.ability_4 = self.parent:FindAbilityByName("omni_slash")
		self.ability_5 = self.parent:FindAbilityByName("swift_slash2")
...
function npc_dota_hero_juggernaut_ai:think()
	if IsServer() then
		--等级相关数据
...
--[[
function npc_dota_hero_juggernaut_ai:use_combo(hero)
...
function npc_dota_hero_juggernaut_ai:use_item(hero,table_ab_1,table_ab_2)
	if IsServer() then
		if hero.stat ~= 2 then
...
function npc_dota_hero_juggernaut_ai:use_ability(hero,table_ab_1,table_ab_2)
	if not IsServer() then return end
		
	if self.parent:GetMana() > 100 and self.ability_2 and self.ability_2:IsCooldownReady() and #table_ab_1 > 0 then	
		hero:CastAbilityOnPosition(self.parent:GetAbsOrigin(),self.ability_2,self.id)
	end
	if self.parent:GetMana() > 100 and self.ability_5 and self.ability_5:IsCooldownReady() and #table_ab_1 > 0 then		
		hero:CastAbilityOnTarget(table_ab_1[#table_ab_1],self.ability_5,self.id)
	end
	if self.parent:GetMana() > 300 and self.ability_4 and self.ability_4:IsCooldownReady() and #table_ab_2 > 0 then
		hero:CastAbilityOnTarget(table_ab_2[1],self.ability_4,self.id)
	end
...
--[[
function npc_dota_hero_juggernaut_ai:learn_ability(lv)
...
function npc_dota_hero_juggernaut_ai:add_item(lv)
	if lv <= 6 then
...
function npc_dota_hero_juggernaut_ai:leart_talent(lv)   
	for i = (lv-1)*4+1, lv*4 do
		self.parent:AddNewModifier(self.parent,nil,self.talent_table[i],{})
	end
end

function npc_dota_hero_juggernaut_ai:learn_veteran_talent(lv)
