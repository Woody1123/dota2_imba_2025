if npc_dota_hero_dragon_knight_ai == nil then
	npc_dota_hero_dragon_knight_ai = ({})
end

function npc_dota_hero_dragon_knight_ai:IsHidden() return false end
function npc_dota_hero_dragon_knight_ai:IsDebuff() return false end
function npc_dota_hero_dragon_knight_ai:IsPurgable() 		return false end
function npc_dota_hero_dragon_knight_ai:IsPurgeException() 	return false end
function npc_dota_hero_dragon_knight_ai:RemoveOnDeath() 	return false end
function npc_dota_hero_dragon_knight_ai:OnCreated()
	self.parent = self:GetParent()
	self.hero_lv = 7
	self.unit_table = {}
	self.item_table = {"item_three_knives","item_skadi_v2","item_bkb","item_imba_aegis_heart","item_imba_greatwyrm_plate","item_veil_of_evil"}
	...
	if IsServer() then
		...
		self.ability_1 = self.parent:FindAbilityByName("breathe_fire")
		self.ability_2 = self.parent:FindAbilityByName("dragon_tail")	
		self.ability_3 = self.parent:FindAbilityByName("dragon_blood")	
		self.ability_4 = self.parent:FindAbilityByName("elder_dragon_form")
...
function npc_dota_hero_dragon_knight_ai:think()
	if IsServer() then
		--等级相关数据
...
--[[
function npc_dota_hero_dragon_knight_ai:use_combo(hero)
...
function npc_dota_hero_dragon_knight_ai:use_item(hero,table_ab_1,table_ab_2)
	if IsServer() then
		if hero.stat ~= 2 then
...
function npc_dota_hero_dragon_knight_ai:use_ability(hero,table_ab_1,table_ab_2)
	if not IsServer() then return end
		
	if self.parent:GetMana() > 50 and self.ability_1 and self.ability_1:IsCooldownReady() and #table_ab_1 > 0 then
		hero:CastAbilityOnPosition(table_ab_1[1]:GetAbsOrigin(),self.ability_1,self.id)
	end
	if self.parent:GetMana() > 80 and self.ability_2 and self.ability_2:IsCooldownReady() and #table_ab_2 > 0 then
		hero:CastAbilityOnTarget(table_ab_2[#table_ab_2],self.ability_2,self.id)
	end
	if self.parent:GetMana() > 50 and self.ability_3 and self.ability_3:IsCooldownReady() and #table_ab_1 > 0 then
		hero:CastAbilityOnPosition(table_ab_1[1]:GetAbsOrigin(),self.ability_3,self.id)
	end
	
	if self.ability_4 and self.ability_4:IsCooldownReady() and self.parent.stat == 1  and #table_ab_1 >0 then
...
--[[
function npc_dota_hero_dragon_knight_ai:learn_ability(lv)
...
function npc_dota_hero_dragon_knight_ai:add_item(lv)
	if lv <= 6 then
...
function npc_dota_hero_dragon_knight_ai:leart_talent(lv)   
	for i = (lv-1)*4+1, lv*4 do
		self.parent:AddNewModifier(self.parent,nil,self.talent_table[i],{})
	end
end

function npc_dota_hero_dragon_knight_ai:learn_veteran_talent(lv)
