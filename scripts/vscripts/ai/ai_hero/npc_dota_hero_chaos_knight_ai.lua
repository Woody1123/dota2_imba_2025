if npc_dota_hero_chaos_knight_ai == nil then
	npc_dota_hero_chaos_knight_ai = ({})
end

function npc_dota_hero_chaos_knight_ai:IsHidden() return false end
function npc_dota_hero_chaos_knight_ai:IsDebuff() return false end
function npc_dota_hero_chaos_knight_ai:IsPurgable() 		return false end
function npc_dota_hero_chaos_knight_ai:IsPurgeException() 	return false end
function npc_dota_hero_chaos_knight_ai:RemoveOnDeath() 	return false end
function npc_dota_hero_chaos_knight_ai:OnCreated()
	...
	self.talent_table = {
		...
		"special_bonus_custom_value_msp60",
		"special_bonus_custom_value_asp75",
		"special_bonus_custom_value_castrange200",
		"special_bonus_imba_chaos_knight_4",
		"special_bonus_imba_chaos_knight_3",
		"special_bonus_imba_chaos_knight_2",
		"special_bonus_custom_value_cd20",		
	}
	...
	if IsServer() then
		...
		self.ability_1 = self.parent:FindAbilityByName("imba_chaos_knight_chaos_bolt")
		self.ability_2 = self.parent:FindAbilityByName("imba_chaos_knight_reality_rift")	
		self.ability_3 = self.parent:FindAbilityByName("imba_chaos_knight_chaos_strike")	
		self.ability_4 = self.parent:FindAbilityByName("imba_chaos_knight_phantasm")
...
function npc_dota_hero_chaos_knight_ai:think()
	if IsServer() then
		--等级相关数据
...
--[[
function npc_dota_hero_chaos_knight_ai:use_combo(hero)
...
function npc_dota_hero_chaos_knight_ai:use_item(hero,table_ab_1,table_ab_2)
	if IsServer() then
		if hero.stat ~= 2 then
...
function npc_dota_hero_chaos_knight_ai:use_ability(hero,table_ab_1,table_ab_2)
	if not IsServer() then return end
		
	if self.parent:GetMana() > 50 and self.ability_2 and self.ability_2:IsCooldownReady() and #table_ab_2 > 0 then
		hero:CastAbilityOnTarget(table_ab_2[#table_ab_2],self.ability_2,self.id)
	end
	if self.parent:GetMana() > 140 and self.ability_1 and self.ability_1:IsCooldownReady() and #table_ab_2 > 0 then
		hero:CastAbilityOnTarget(table_ab_2[1],self.ability_1,self.id)
	end
	
	if self.ability_4 and self.ability_4:IsCooldownReady() and self.parent.stat == 1 and self.parent:GetMana() > 275 and #table_ab_2 > 0 then
...
--[[
function npc_dota_hero_chaos_knight_ai:learn_ability(lv)
...
function npc_dota_hero_chaos_knight_ai:add_item(lv)
	if lv <= 6 then
...
function npc_dota_hero_chaos_knight_ai:leart_talent(lv)   
	for i = (lv-1)*4+1, lv*4 do
		self.parent:AddNewModifier(self.parent,nil,self.talent_table[i],{})
	end
end

function npc_dota_hero_chaos_knight_ai:learn_veteran_talent(lv)
