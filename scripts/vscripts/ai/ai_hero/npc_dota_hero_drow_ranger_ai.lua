if npc_dota_hero_drow_ranger_ai == nil then
	npc_dota_hero_drow_ranger_ai = ({})
end

function npc_dota_hero_drow_ranger_ai:IsHidden() return false end
function npc_dota_hero_drow_ranger_ai:IsDebuff() return false end
function npc_dota_hero_drow_ranger_ai:IsPurgable() 		return false end
function npc_dota_hero_drow_ranger_ai:IsPurgeException() 	return false end
function npc_dota_hero_drow_ranger_ai:RemoveOnDeath() 	return false end
function npc_dota_hero_drow_ranger_ai:OnCreated()
	...
	if IsServer() then
		...
		self.ability_1 = self.parent:FindAbilityByName("frost_arrows")
		self.ability_2 = self.parent:FindAbilityByName("wave_of_silence")	
		self.ability_3 = self.parent:FindAbilityByName("multishot")	
		self.ability_4 = self.parent:FindAbilityByName("marksmanship")
...
function npc_dota_hero_drow_ranger_ai:think()
	if IsServer() then
		--等级相关数据
...
--[[
function npc_dota_hero_drow_ranger_ai:use_combo(hero)
...
function npc_dota_hero_drow_ranger_ai:use_item(hero,table_ab_1,table_ab_2)
	if IsServer() then
		if hero.stat ~= 2 then
...
function npc_dota_hero_drow_ranger_ai:use_ability(hero,table_ab_1,table_ab_2)
	if not IsServer() then return end
		
	if self.parent:GetMana() > 90 and self.ability_2 and self.ability_2:IsCooldownReady() and #table_ab_2 > 0 then
		hero:CastAbilityOnPosition(table_ab_2[1]:GetAbsOrigin(),self.ability_2,self.id)
	end
	...
	if self.ability_4 and self.ability_4:IsCooldownReady() and #table_ab_1 > 0 then
		hero:CastAbilityOnPosition(table_ab_1[1]:GetAbsOrigin(),self.ability_4,self.id)
	end
end


--[[
function npc_dota_hero_drow_ranger_ai:learn_ability(lv)
...
function npc_dota_hero_drow_ranger_ai:add_item(lv)
	if lv <= 6 then
...
function npc_dota_hero_drow_ranger_ai:leart_talent(lv)   
	for i = (lv-1)*4+1, lv*4 do
		self.parent:AddNewModifier(self.parent,nil,self.talent_table[i],{})
	end
end

function npc_dota_hero_drow_ranger_ai:learn_veteran_talent(lv)
