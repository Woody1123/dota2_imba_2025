...
LinkLuaModifier("modifier_imba_faceless_void_chronosphere", "mb/hero_faceless_void/faceless_void_chronosphere", LUA_MODIFIER_MOTION_NONE)
...
function imba_faceless_void_chronosphere:OnSpellStart()
	...
	local time_2 = self:GetSpecialValueFor("base_duration")-1
	...
	if #enemies >= 4 then
		radius = max(2000,radius)
		caster:SetModifierStackCount("modifier_imba_faceless_void_chronosphere_stamps", nil, caster:GetModifierStackCount("modifier_imba_faceless_void_chronosphere_stamps", nil) + 1)
	end
	...
	caster:SetMana(150)
	if caster:TG_HasTalent("special_bonus_imba_faceless_void_6") then	
		caster:AddNewModifier(caster,self,"modifier_imba_faceless_void_chronosphere",{duration = time_2})
	end
end
...
modifier_imba_faceless_void_chronosphere = class({})

function modifier_imba_faceless_void_chronosphere:IsDebuff()				return false end
function modifier_imba_faceless_void_chronosphere:IsHidden() 			return false end
function modifier_imba_faceless_void_chronosphere:IsPurgable() 			return false end
function modifier_imba_faceless_void_chronosphere:IsPurgeException() 	return false end
function modifier_imba_faceless_void_chronosphere:DeclareFunctions()
	return {MODIFIER_PROPERTY_INCOMING_DAMAGE_CONSTANT,MODIFIER_PROPERTY_MIN_HEALTH}
end
function modifier_imba_faceless_void_chronosphere:GetModifierIncomingDamageConstant(keys)
	if IsServer() then
		if keys.target:GetHealth() - keys.damage <=1000 then
...
function modifier_imba_faceless_void_chronosphere:GetMinHealth()return 999
end
...
function modifier_imba_faceless_void_chronosphere_thinker:OnCreated(keys)
	if IsServer() then
		...
		if self.radius < 3000 then
			pfx_name = "particles/units/heroes/hero_faceless_void/faceless_void_chronosphere.vpcf"
			-- pfx_name  = "particles/units/heroes/hero_faceless_void/faceless_void_chronosphere.vpcf"
		end
...
function modifier_imba_faceless_void_chronosphere_debuff:GetModifierAttackSpeedBonus_Constant()
	if self.buff_type == Chronosphere_Caster then
		local caster=self:GetCaster()
		--local manacost=caster:GetMaxMana()-caster:GetMana()-100-caster:GetAbility():GetSpecialValueFor("AbilityManaCost")
		local manacost=caster:GetMaxMana()-caster:GetMana()
		return (caster:GetModifierStackCount("modifier_imba_faceless_void_chronosphere_stamps", nil) * self:GetAbility():GetSpecialValueFor("bonus_as")+manacost/15)
	else
		return nil
	end
end
