...
function imba_riki_blink_strike:OnSpellStart()
	...
	for i=1, #caster.illusions do
		...
		--caster.illusions[i]:AddNewModifier(caster, self, "modifier_unit_remove", {duration = illusion_duration+1})
	...
	if modifier == nil or modifier:GetStackCount() == 1 then
		...
		--Talent 6
		if caster:TG_HasTalent("special_bonus_imba_riki_6") and not Is_Chinese_TG(caster,target) then 
...
function modifier_imba_riki_blink_strike_illusion:OnCreated()
	if IsServer() then
		self.move = false
	end
end
function modifier_imba_riki_blink_strike_illusion:OnOrder( params )
	if params.unit~=self:GetCaster() or params.target~=self:GetParent() or self.move then return end
...
function modifier_imba_riki_blink_strike_illusion:SetValidTarget( location )
	...
	Timers:CreateTimer(0.3, function()
		if self~=nil then
			if self:GetCaster():IsStunned() or self:GetCaster():IsRooted() or self:GetCaster():HasModifier("modifier_item_nullifier_v2_debuff") or self:GetCaster():IsHexed() then
...
function modifier_imba_riki_blink_strike_mark:SetValidTarget( location )
	...
	Timers:CreateTimer(0.2, function()
		print("123")
		if self~=nil then
			if self:GetCaster():IsStunned() or self:GetCaster():IsRoot() or self:GetCaster():HasModifier("modifier_item_nullifier_v2_debuff") or self:GetCaster():IsHexed() then
				self:Destroy()
				else
