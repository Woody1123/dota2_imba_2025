...
function imba_lion_earth_spike:OnProjectileHit_ExtraData(target, location, keys)
	...
	if target then
		...
		if target:HasModifier("modifier_imba_soul_chain") and keys.extra == 1 then
			--
		elseif target:TG_TriggerSpellAbsorb(self) then
			return
		end
...
function imba_lion_earth_spike:OnSpellStart(dir1,dir2)
	...
	local start_pos = caster:GetAbsOrigin()
	local dir_tab = {0}
	if self:GetCaster():TG_HasTalent("special_bonus_imba_lion_2") then
		dir_tab = {-15,0,15}
	end
	...
	for _,angel in pairs(dir_tab) do
		local new_dir = TG_Direction(RotatePosition(start_pos, QAngle(0, angel, 0), start_pos + direction * 200),caster:GetAbsOrigin())
		local new_end_Pos = start_pos + new_dir * (self:GetCastRange(end_pos, caster) + caster:GetCastRangeBonus())
		local marker = CreateModifierThinker(caster, self, "modifier_dummy_thinker", {duration = 20.0}, start_pos, caster:GetTeamNumber(), false):entindex()
		...
		local info = 
		{
			...
			EffectName = "particles/econ/items/lion/lion_ti9/lion_spell_impale_ti9.vpcf",
			vSpawnOrigin = start_pos,
			fDistance = (start_pos - new_end_Pos):Length2D(),
			fStartRadius = self:GetSpecialValueFor("spikes_radius"),
			fEndRadius = self:GetSpecialValueFor("spikes_radius"),
			Source = caster,
			bHasFrontalCone = false,
			...
			fExpireTime = GameRules:GetGameTime() + 10.0,
			bDeleteOnHit = true,
			vVelocity = new_dir * self:GetSpecialValueFor("spike_speed"),
			bProvidesVision = false,
...
function imba_lion_hex:OnSpellStart()
	...
	if self:GetCaster():TG_HasTalent("special_bonus_imba_lion_6") then
		local enemies=FindUnitsInRadius(
				self:GetCaster():GetTeamNumber(),
				target:GetAbsOrigin(),
				nil,
				self:GetCaster():TG_GetTalentValue("special_bonus_imba_lion_6"),
		...
		local tars = 0
		for _, enemy in pairs(enemies) do
			enemy:AddNewModifier_RS(self:GetCaster(), self, "modifier_imba_lion_hex", {duration = self:GetSpecialValueFor("duration")})
		end
	else
...
function modifier_imba_lion_hex:CheckState() return {[MODIFIER_STATE_SILENCED] = true, [MODIFIER_STATE_MUTED] = true, [MODIFIER_STATE_DISARMED] = true, [MODIFIER_STATE_HEXED] = true,[MODIFIER_STATE_PASSIVES_DISABLED] = true} end
...
function modifier_imba_lion_hex:GetModifierMagicalResistanceBonus()
	return -33
end
function modifier_imba_lion_hex:GetModifierPhysicalArmorBonus()
...
function modifier_imba_mana_drain_thinker:OnCreated(keys)
	if self:GetAbility() == nil then return end
	self.movement_slow = self:GetAbility():GetSpecialValueFor("move_slow")
...
function modifier_imba_mana_drain_thinker:OnIntervalThink()
	...
	local max_distance=self:GetAbility():GetSpecialValueFor("break_distance")+(self:GetCaster():Has_Aghanims_Shard() and 300 or 0)+self:GetCaster():GetCastRangeBonus()
	if distance > max_distance or self:GetParent():IsInvulnerable() or self:GetParent():IsMagicImmune() or not self:GetCaster():CanEntityBeSeenByMyTeam(self:GetParent()) then
		self:Destroy()
		return
	end
	-- 主目标没有被吸蓝也结束
...
function imba_lion_finger_of_death:OnSpellStart()
	...
	local stack_damage = ability:GetSpecialValueFor("stack_damage")+self:GetCaster():TG_GetTalentValue("special_bonus_imba_lion_1")
...
function modifier_imba_finger_of_death_kill:OnDestroy()
	if IsServer() and self:GetAbility() ~= nil then 
		-- self:GetAbility():StartCooldown(self:GetAbility():GetCooldown(-1) * self:GetCaster():GetCooldownReduction())
...
function modifier_imba_finger_of_death:OnTooltip() return self:GetStackCount() * (self:GetAbility():GetSpecialValueFor("stack_damage")+self:GetParent():TG_GetTalentValue("special_bonus_imba_lion_1")) end
function modifier_imba_finger_of_death:GetModifierExtraHealthBonus() return self:GetParent():TG_HasTalent("special_bonus_imba_lion_4") and self:GetStackCount() * self:GetParent():TG_GetTalentValue("special_bonus_imba_lion_4") end
