...
-- Arrangement: MysticBug 05/13/2021

CreateTalents("npc_dota_hero_pugna", "mb/hero_pugna")


imba_pugna_nether_blast = class({})

LinkLuaModifier("modifier_imba_nether_blast_debuff", "mb/hero_pugna.lua", LUA_MODIFIER_MOTION_NONE)
...
function imba_pugna_nether_blast:OnSpellStart()
	...
	local dmg = self:GetSpecialValueFor("damage") + self:GetCaster():TG_GetTalentValue("special_bonus_imba_pugna_5")
	...
	--end
	local pfx_main = ParticleManager:SafeCreateParticle(pfx_main_name, PATTACH_CUSTOMORIGIN, nil)
	...
	for _, ememy_main in pairs(enemies_main) do
		...
		if not ememy_main:IsBuilding() then
			ememy_main:AddNewModifier_RS(caster, self, "modifier_imba_nether_blast_debuff", {duration = self:GetSpecialValueFor("duration")})
		end
	end
	...
	for i=1, blasts do
		...
		local pfx_min = ParticleManager:SafeCreateParticle(pfx_pre_name, PATTACH_CUSTOMORIGIN, nil)
		...
		Timers:CreateTimer(delay, function()
			local pfx_balst = ParticleManager:SafeCreateParticle(pfx_main_name, PATTACH_CUSTOMORIGIN, nil)
			...
			for _, ememy_balst in pairs(enemies_balst) do
				...
				if ememy_balst:IsBuilding() then
					secondary_damage = secondary_damage * (self:GetSpecialValueFor("building_pct") / 100)
				end
				local damageTable = {
									victim = ememy_balst,
									attacker = self:GetCaster(),
				...
				if not ememy_balst:IsBuilding() then
					ememy_balst:AddNewModifier_RS(caster, self, "modifier_imba_nether_blast_debuff", {duration = self:GetSpecialValueFor("duration")})
				end
			end
			return nil
		end
...
function modifier_imba_nether_blast_debuff:GetEffectName() return "particles/econ/items/pugna/pugna_ward_golden_nether_lord/pugna_gold_ambient.vpcf" end
...
LinkLuaModifier("modifier_imba_decrepify_ally", "mb/hero_pugna.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_decrepify_enemy", "mb/hero_pugna.lua", LUA_MODIFIER_MOTION_NONE)
...
function imba_pugna_decrepify:OnSpellStart()
	...
	local duration = self:GetSpecialValueFor("duration") + self:GetCaster():TG_GetTalentValue("special_bonus_imba_pugna_4")
	...
	if IsEnemy(target, caster) then
		target:AddNewModifier_RS(caster, self, buff_name, {duration = duration})
	else
		--禁用帮助
		if PlayerResource:IsDisableHelpSetForPlayerID(target:GetPlayerOwnerID(), caster:GetPlayerOwnerID()) then
			...
			caster:AddNewModifier(caster, self, "modifier_stunned", {duration = 10.0})
			return
		end
		target:AddNewModifier(caster, self, buff_name, {duration = duration})
	end
	target:EmitSound("Hero_Pugna.Decrepify")
	--Talent 
	if caster:Has_Aghanims_Shard() then 
		caster:AddNewModifier(caster, self, "modifier_imba_decrepify_ally", {duration = duration})
		...
		if #enemies then 
			for i=1, #enemies do
				if enemies[i] ~= target then
					--buff_name = IsEnemy(caster, enemies[i]) and "modifier_imba_decrepify_enemy" or "modifier_imba_decrepify_ally"
					if IsEnemy(caster, enemies[i]) then
						enemies[i]:AddNewModifier_RS(caster, self, "modifier_imba_decrepify_enemy", {duration = duration})
					else
						enemies[i]:AddNewModifier(caster, self, "modifier_imba_decrepify_ally", {duration = duration})
					end
...
function modifier_imba_decrepify_ally:OnDestroy()
	if IsServer() then
		if self:GetCaster():HasAbility("imba_pugna_nether_blast") and self:GetCaster():FindAbilityByName("imba_pugna_nether_blast"):GetLevel() > 0 then
			...
			local pfx_min = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_pugna/pugna_netherblast_pre.vpcf", PATTACH_CUSTOMORIGIN, nil)
			ParticleManager:SetParticleControl(pfx_min, 0, Vector(blast_pos.x, blast_pos.y, blast_pos.z + 128))
			ParticleManager:SetParticleControl(pfx_min, 1, Vector(radius, 1, 1))
			ParticleManager:ReleaseParticleIndex(pfx_min)
			...
			Timers:CreateTimer(ability:GetSpecialValueFor("secondary_delay"), function()
				local pfx_balst = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_pugna/pugna_netherblast.vpcf", PATTACH_CUSTOMORIGIN, nil)
				ParticleManager:SetParticleControl(pfx_balst, 0, Vector(blast_pos.x, blast_pos.y, blast_pos.z + 128))
				ParticleManager:SetParticleControl(pfx_balst, 1, Vector(radius, 1, 1))
				ParticleManager:ReleaseParticleIndex(pfx_balst)
				...
				for _, ememy_balst in pairs(enemies_balst) do
					...
					ApplyDamage(damageTable)
					ememy_balst:AddNewModifier_RS(caster, ability, "modifier_imba_nether_blast_debuff", {duration = ability:GetSpecialValueFor("duration")})
				end
				return nil
			end
			)
...
LinkLuaModifier("modifier_imba_nether_ward", "mb/hero_pugna.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_nether_ward_debuff", "mb/hero_pugna.lua", LUA_MODIFIER_MOTION_NONE)
...
function imba_pugna_nether_ward:OnSpellStart()
	local caster = self:GetCaster()
	local pos = self:GetCursorPosition()
	local ward = CreateUnitByName("npc_dota_pugna_nether_ward_"..self:GetLevel(), pos, true, caster, caster, caster:GetTeamNumber())
	...
	ward:AddNewModifier(caster, self, "modifier_unit_remove", {duration = self:GetSpecialValueFor("duration") + 1})
...
LinkLuaModifier("modifier_imba_life_drain_enemy", "mb/hero_pugna.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_life_drain_friend", "mb/hero_pugna.lua", LUA_MODIFIER_MOTION_NONE)
...
function modifier_imba_life_drain_enemy:OnCreated(keys)
	...
	if IsServer() then
		...
		self.ally = keys.ally
		local pfx_name = "particles/units/heroes/hero_pugna/pugna_life_drain.vpcf"
		--if HeroItems:UnitHasItem(self:GetCaster(), "pugna_ti10_immortal_head") then
			--particles/econ/items/pugna/pugna_ti10_immortal/pugna_ti10_immortal_life_drain_gold.vpcf  gold 
...
function modifier_imba_life_drain_enemy:OnIntervalThink()
	...
	if not caster:CanEntityBeSeenByMyTeam(target) and not caster:TG_HasTalent("special_bonus_imba_pugna_1") then
		self:Destroy()
		return
	end
	...
	if caster:GetHealth() ~= caster:GetMaxHealth() then
		ParticleManager:SetParticleControl(self.pfx, 11, Vector(0,0,0))
		--Talent
		dmg_done = dmg_done + math.floor(dmg_done * self:GetCaster():TG_GetTalentValue("special_bonus_imba_pugna_3")/100)
