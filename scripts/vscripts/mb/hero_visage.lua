...
function imba_visage_soul_assumption:GetCastRange(location , target)
	return self.BaseClass.GetCastRange(self,location,target) + self:GetCaster():GetIntellect(false) + self:GetCaster():GetCastRangeBonus()
end
...
function imba_visage_soul_assumption:OnProjectileHit_ExtraData(target, pos, keys)
	--黄泉超度伤害 无法被躲避
	if not target or target:TG_TriggerSpellAbsorb(self) then
		return
	end
	local caster = self:GetCaster()	
...
function modifier_imba_visage_summon_familiars_caster:CheckState()
	local state = {
		...
		[MODIFIER_STATE_CANNOT_MISS] = true, --必中
	}
	return state
end
...
function modifier_imba_visage_summon_familiars_caster:OnAttackLanded(keys)
	...
	if self:GetCaster():TG_HasTalent("special_bonus_imba_visage_5") then 
			local talent_dmg = self:GetCaster():GetIntellect(false) * self:GetCaster():TG_GetTalentValue("special_bonus_imba_visage_5")/100/2
		ApplyDamage({
				victim = keys.target,
...
function modifier_imba_visage_summon_familiars_controller:OnAttackLanded(keys)
	...
	if self:GetCaster():TG_HasTalent("special_bonus_imba_visage_5") then 
			local talent_dmg = self:GetCaster():GetIntellect(false) * self:GetCaster():TG_GetTalentValue("special_bonus_imba_visage_5")/100
		ApplyDamage({
				victim = keys.target,
