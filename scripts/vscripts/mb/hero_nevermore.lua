...
function CastPointsShadoWraze(caster,ability,lord_pos)
	...
	ParticleManager:SetParticleControl(pfx, 3, Vector(pfx_radius, 1, 1))
	ParticleManager:ReleaseParticleIndex(pfx)
	local enemies = FindUnitsInRadius(caster:GetTeamNumber(), pos, nil, pfx_radius+caster:GetAttackRangeBuffer()/2, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NONE,FIND_ANY_ORDER,false)
...
function modifier_imba_necromastery_counter:OnTakeDamage(keys)
	if not IsServer() or keys.attacker ~= self:GetParent() then
		...
		if not keys.inflictor  or self:GetParent():PassivesDisabled() or keys.unit:GetTeamNumber() == self:GetParent():GetTeamNumber() or
...
function modifier_imba_dark_lord_debuff:DeclareFunctions() return {MODIFIER_PROPERTY_PHYSICAL_ARMOR_BONUS,MODIFIER_EVENT_ON_DEATH} end
...
function modifier_imba_dark_lord_debuff:OnDeath()
...
LinkLuaModifier("modifier_nevermore_requiem_fear_cd", "mb/hero_nevermore", LUA_MODIFIER_MOTION_NONE)
...
function imba_nevermore_requiem:OnSpellStart(talent,talent_pos)
	...
	if talent then
		...
		cast_pos = talent_pos
		if caster:HasModifier("modifier_nevermore_requiem_fear_cd") then
			fear = 0
		end
		caster:AddNewModifier(caster,self,"modifier_nevermore_requiem_fear_cd",{duration = 15})
	end
	...
	local line_width_start=self:GetSpecialValueFor("line_width_start")
	local line_width_end=self:GetSpecialValueFor("line_width_end")
	for i=0, lines-1 do
...
function imba_nevermore_requiem:OnProjectileHit_ExtraData(target, location, keys)
	if keys.go == 1 then
		...
		if self:GetCaster():IsAlive() and not target:IsMagicImmune() then
			target:AddNewModifier(self:GetCaster(), self, "modifier_nevermore_requiem_fear", {duration = 1.7*keys.fear})
		end
		local damageTable = {
							victim = target,
...
function modifier_nevermore_requiem_fear_cd:IsDebuff()			return false end
function modifier_nevermore_requiem_fear_cd:IsHidden() 			return true end
function modifier_nevermore_requiem_fear_cd:IsPurgable() 			return true end
function modifier_nevermore_requiem_fear_cd:IsPurgeException() 	return true end
