	...
	if caster:HasScepter() then
		...
		for _,unit in pairs(units) do
			if not unit:IsMagicImmune() then
				...
				if (self:GetCaster():GetAbsOrigin() - unit:GetAbsOrigin()):Length2D()<radius then
					--眩晕
					unit:AddNewModifier_RS(self:GetCaster(), self, "modifier_stunned", {duration = stun_duration})
					--减攻速和减移速
					unit:AddNewModifier_RS(self:GetCaster(), self, debuff_modifier, {duration = allbuff_duration + stun_duration})
				end
				-----------------------------
				--Value Shard
				if self:GetCaster():Has_Aghanims_Shard() then
					...
					--叠满
					--unit:SetModifierStackCount("modifier_imba_slardar_bash_amplify_damage", nil, unit:GetModifierStackCount("modifier_imba_slardar_bash_amplify_damage", nil) + 10)
				end
				--造成伤害
		...
		for _,enemy in pairs(enemies) do
			if not enemy:IsMagicImmune() then
				...
				if self:GetCaster():Has_Aghanims_Shard() then
					...
					--叠满 :GetSpecialValueFor("armor_reduction_max")
					--enemy:SetModifierStackCount("modifier_imba_slardar_bash_amplify_damage", nil, enemy:GetModifierStackCount("modifier_imba_slardar_bash_amplify_damage", nil) + 10)
					--if  self:GetCaster():HasAbility("imba_slardar_bash") then
					--	enemy:SetModifierStackCount("modifier_imba_slardar_bash_amplify_damage", nil, self:GetCaster():FindAbilityByName("imba_slardar_bash"):GetSpecialValueFor("armor_reduction_max"))
					--end
				end
				--造成伤害
				ApplyDamage(damage_table)
...
function modifier_imba_slardar_trap_caster:OnIntervalThink()
	--移动替身
	if self:GetAbility().thinker and self:GetAbility().thinker:IsNull() then
