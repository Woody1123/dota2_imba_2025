...
function imba_tidehunter_gush:OnSpellStart()
	...
	local info = {
		...
		bDeleteOnHit		= true,
		vVelocity			= direction * (self:GetSpecialValueFor("speed")+caster:GetCastRangeBonus()*3),
...
function modifier_imba_tidehunter_kraken_shell_passive:GetModifierIncomingDamage_Percentage(keys)
	...
	if parent:HasModifier("modifier_imba_tidehunter_kraken_shell_time") then 
		--天赋 吸收伤害治愈自己
		local damage_absorb_pct = passive:GetSpecialValueFor("damage_absorb_pct") + parent:GetLevel()/2
		if parent:TG_HasTalent("special_bonus_imba_tidehunter_2") then 
			if parent:IsAlive() then
				local heal = damage_absorb_pct * keys.damage/200
				parent:Heal(heal,parent)
				SendOverheadEventMessage(nil, OVERHEAD_ALERT_HEAL , parent, heal, nil)
				--parent:SetHealth(math.max(parent:GetHealth() + math.floor(damage_absorb_pct * keys.damage/100),1))
			end
		end
...
LinkLuaModifier("modifier_anchor_smash_passive","mb/hero_tidehunter.lua", LUA_MODIFIER_MOTION_NONE)
...
function imba_tidehunter_anchor_smash:GetIntrinsicModifierName() return "modifier_anchor_smash_passive" end
		...
		if not enemy:IsBoss() then
			...
			--if not enemy:IsMagicImmune() then
				...
				if buff:GetStackCount() >= self:GetSpecialValueFor("proc_count") then 
					...
					--造成晕眩
					enemy:AddNewModifier_RS(caster, self, "modifier_stunned", {duration = 1.0})
				end
			--end
		end
	end
...
function modifier_anchor_smash_passive:IsHidden()			return true end
function modifier_anchor_smash_passive:IsDebuff()			return false end
function modifier_anchor_smash_passive:IsPurgable() 		return false end
function modifier_anchor_smash_passive:IsPurgeException() 	return false end
function modifier_anchor_smash_passive:RemoveOnDeath() 		return false end
function modifier_anchor_smash_passive:DeclareFunctions()  return {MODIFIER_EVENT_ON_ATTACK_START,MODIFIER_EVENT_ON_ATTACK_LANDED} end
function modifier_anchor_smash_passive:OnAttackStart(tg)
    if not IsServer() then
        return
    end
    if tg.attacker == self:GetParent() and self:GetParent():TG_HasTalent("special_bonus_imba_tidehunter_8") then
        self.miss=true
    end
end
function modifier_anchor_smash_passive:OnAttackLanded(keys)
	if not IsServer() then
		return
	end
	if keys.attacker ~= self:GetParent() or keys.target:IsOther() or self:GetParent():PassivesDisabled()  or (not keys.target:IsAlive() ) or (not self.miss) or not self:GetParent():TG_HasTalent("special_bonus_imba_tidehunter_8") then
		return
	end
	self.miss = false
	if  RollPseudoRandomPercentage(self:GetParent():TG_GetTalentValue("special_bonus_imba_tidehunter_8"),0,self) then
...
function imba_tidehunter_ravage:OnSpellStart(mini)
	local caster = self:GetCaster()
	...
	local knock_table = {}
	local duration = self:GetSpecialValueFor("duration")*mini_ravage
	--环参数
	local radius = (self:GetSpecialValueFor("radius") + caster:TG_GetTalentValue("special_bonus_imba_tidehunter_4"))*mini_ravage
	local ring_count = math.ceil(self:GetSpecialValueFor("ring_count")*mini_ravage)
	...
	if not self:GetAutoCastState() then
		...
		if self:GetCooldownTimeRemaining() < 35 then
			self:EndCooldown()
			self:StartCooldown(35)
		end	
	else 
		--自动施法 环形巨浪
		local gush_count = math.ceil((self:GetSpecialValueFor("gush_count") + caster:TG_GetTalentValue("special_bonus_imba_tidehunter_4","value_autocast"))*mini_ravage)
...
imba_tidehunter_coming = class({})
LinkLuaModifier("modifier_tidehunter_coming_motion","mb/hero_tidehunter.lua", LUA_MODIFIER_MOTION_NONE)
function imba_tidehunter_coming:IsHiddenWhenStolen() 		return false end
function imba_tidehunter_coming:IsRefreshable() 			return true end
function imba_tidehunter_coming:IsStealable() 				return true end
function imba_tidehunter_coming:IsNetherWardStealable()		return true end
function imba_tidehunter_coming:GetAOERadius() return self:GetSpecialValueFor("radius") end

function imba_tidehunter_coming:OnHeroLevelUp()
	local level = self:GetCaster():GetLevel()
	local ability = self
	local level_to_set = math.min((math.floor((level + 1) / 2) - 1), ability:GetMaxLevel())
	if ability:GetLevel() ~= level_to_set then
...
function imba_tidehunter_coming:OnInventoryContentsChanged()
	--魔晶技能
	---------------------------------------------------------------
	local caster=self:GetCaster()
	if self:GetCaster():Has_Aghanims_Shard() then 
        TG_Set_Scepter(caster,false,1,"imba_tidehunter_coming")
    else
        TG_Set_Scepter(caster,true,1,"imba_tidehunter_coming")
    end
	---------------------------------------------------------------
end

function imba_tidehunter_coming:OnSpellStart()
	...
	local tralve_duration = 0.9
		caster:AddNewModifier(caster, self, "modifier_tidehunter_coming_motion", {duration = tralve_duration, pos_x = pos.x, pos_y = pos.y, pos_z = pos.z, dis = distance,height = height})
	local cooldown = self:GetCooldownTimeRemaining() 
	self:EndCooldown()
	self:StartCooldown(cooldown+1)
end
modifier_tidehunter_coming_motion = class({})

function modifier_tidehunter_coming_motion:IsDebuff()			return false end
function modifier_tidehunter_coming_motion:IsHidden() 			return true end
function modifier_tidehunter_coming_motion:IsPurgable() 		return false end
function modifier_tidehunter_coming_motion:IsPurgeException() 	return false end

function modifier_tidehunter_coming_motion:GetEffectName()
	return "particles/units/heroes/hero_tiny/tiny_toss_blur.vpcf"
end


function modifier_tidehunter_coming_motion:IsMotionController() return true end
function modifier_tidehunter_coming_motion:GetMotionControllerPriority() return DOTA_MOTION_CONTROLLER_PRIORITY_MEDIUM end
function modifier_tidehunter_coming_motion:CheckState() return {[MODIFIER_STATE_INVULNERABLE] = true, [MODIFIER_STATE_NO_HEALTH_BAR] = true,[MODIFIER_STATE_IGNORING_MOVE_ORDERS]=true} end
function modifier_tidehunter_coming_motion:OnCreated(keys)
	if IsServer() then
		...
		if self:CheckMotionControllers() then
			...
			self.stun_duraion = self:GetAbility():GetSpecialValueFor("duration")
			self.dis = keys.dis
...
function modifier_tidehunter_coming_motion:OnIntervalThink()
...
function modifier_tidehunter_coming_motion:OnDestroy()
	if IsServer() then
		self:GetCaster():ClearActivityModifiers()
		FindClearSpaceForUnit(self:GetParent(), self:GetParent():GetAbsOrigin(), true)
		local ab = self:GetCaster():FindAbilityByName("imba_tidehunter_ravage")
