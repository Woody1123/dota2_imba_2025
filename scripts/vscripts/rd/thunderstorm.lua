...
function thunderstorm:OnSpellStart()
    ...
    local p = ParticleManager:SafeCreateParticle( "particles/units/heroes/hero_zuus/zuus_lightning_bolt.vpcf", PATTACH_CUSTOMORIGIN, nil )
...
function thunderstorm:OnProjectileHit_ExtraData(target, location, kv)
    ...
    if not caster:IsMagicImmune() then
             local damageTable = {
								victim = target,
								attacker = caster,
								damage = self:GetSpecialValueFor("base_dam")+caster:GetIntellect(false)*self:GetSpecialValueFor("int_per")*0.01,
