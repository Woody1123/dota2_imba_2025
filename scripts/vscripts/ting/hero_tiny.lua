...
function modifier_imba_tiny_avalanche_thinker:OnCreated( kv )
	if IsServer() then
		...
		self.radius = self:GetAbility():GetSpecialValueFor("radius")*math.max(math.min(2,self:GetCaster():GetModelScale()),1)
...
function imba_tiny_tree_grab:CastFilterResultTarget(target)
	...
	if not self:GetCaster():HasModifier("modifier_imba_tiny_grabtree") and not self:GetCaster():HasModifier("modifier_imba_tiny_tree_grab_hero_flag") then
		...
		if target~=nil then
			...
			if string.find(target:GetUnitName(), "npc_dota_creep") or string.find(target:GetUnitName(), "siege") then
				return UF_FAIL_CUSTOM
			end
...
function imba_tiny_tree_grab:OnSpellStart()
	...
	if caster:HasModifier("modifier_imba_tiny_tree_grab_hero_flag") then
		...
		local damage = (self:GetCaster():GetAverageTrueAttackDamage(caster)+en:GetAverageTrueAttackDamage(en))*2
...
function modifier_imba_tiny_grabtree:OnCreated()
	...
	self.damage_builiding = 10--self:GetAbility():GetSpecialValueFor("bonus_damage_buildings")
	if IsServer() then
...
function imba_tiny_tree_channel:OnSpellStart()
	...
	caster:AddNewModifier(caster,self,"modifier_imba_tiny_tree_channel",{duration = self:GetSpecialValueFor("dur"),tar_pos_x =tar_pos.x,tar_pos_y =tar_pos.y,tar_pos_z =tar_pos.z })
end
...
function imba_tiny_tree_channel:OnProjectileHit_ExtraData(target, pos, keys)
	local caster = self:GetCaster()
	local radius = self:GetSpecialValueFor("splash_radius")*math.min(self:GetCaster():GetModelScale()*0.8,1)
