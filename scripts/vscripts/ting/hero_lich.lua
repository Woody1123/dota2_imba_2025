...
function Lich_Frost_Nova_Blast(caster, target, ability)
	...
	local per = math.max((target:GetBaseMoveSpeed() - target:GetIdealSpeed())/(target:GetBaseMoveSpeed() - 100),0)
		...
		damageTable.damage = damage+caster:GetIntellect(false)*ability:GetSpecialValueFor("damage_per")*per
...
function modifier_imba_frost_nova_slow:GetAttributes() return MODIFIER_ATTRIBUTE_MULTIPLE end 
...
function modifier_imba_frost_armor:OnTakeDamage(keys)
	...
	if keys.unit == self.parent and keys.attacker:IsHero()  and self.t and (self.parent:GetAbsOrigin() - keys.attacker:GetAbsOrigin()):Length2D() < self.radius then
		...
		local pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_lich/lich_ice_age.vpcf", PATTACH_ABSORIGIN_FOLLOW, self.parent)
...
function modifier_imba_frost_armor:OnIntervalThink()
	...
	local pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_lich/lich_ice_age_dmg.vpcf", PATTACH_ABSORIGIN_FOLLOW, self.parent)
	ParticleManager:SetParticleControlEnt(pfx, 1, self.parent, PATTACH_ABSORIGIN_FOLLOW, nil, self.parent:GetAbsOrigin(), true)
	ParticleManager:SetParticleControl(pfx, 2, Vector(self.radius,self.radius,self.radius))
...
function modifier_imba_sinister_gaze_target:OnCreated()
	if self:GetAbility() == nil then return end		
		...
		--end
		local pfx = ParticleManager:SafeCreateParticle(pfx_name, PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
...
function imba_lich_chain_frost:OnSpellStart()
	...
	--end
	if self.caster:TG_HasTalent("special_bonus_imba_lich_1")   then 
		self.radius = self.radius + self.caster:TG_GetTalentValue("special_bonus_imba_lich_1")	
	end
	self.damageTable = {
...
function imba_lich_chain_frost:OnProjectileHit_ExtraData(target, location, keys)
	...
	if speed >2400  then
		...
		self.damageTable.damage = (string.find(target:GetUnitName(), "npc_dota_lich_ice_spire") or string.find(target:GetUnitName(), "roshan") ) and 0 or dmg
	...
	if self.caster:TG_HasTalent("special_bonus_imba_lich_2") then
	 ...
	 --结束
	if self.bounces ~= -1  and  bounces > self.bounces  then
		return false
	end
...
-- 			summon:AddNewModifier(self:GetCaster(), self, "modifier_unit_remove", {duration = self:GetCaster():TG_GetTalentValue("special_bonus_imba_lich_1") + 1})
-- 		end
-- 	end
-- end
...
function modifier_paralyzed:GetModifierMoveSpeed_Absolute() return 150 end
function modifier_paralyzed:GetModifierMoveSpeed_Limit() return 150 end
function modifier_paralyzed:GetModifierMoveSpeed_Max() return 150 end
function modifier_paralyzed:GetModifierBaseAttackTimeConstant() return 2.2 end
...
function imba_lich_ice_spire:OnSpellStart()
	...
	local ice = CreateUnitByName("npc_dota_lich_ice_spire", pos, true, caster, caster, caster:GetTeamNumber())
		  ice:SetOwner(caster)
		  ice:SetBaseMaxHealth(50)
		  ice:SetMaxHealth(50)
		  ice:SetHealth(50)
		  ice:SetPhysicalArmorBaseValue(armor)
		  ice:SetMaximumGoldBounty(300)
		  ice:SetMinimumGoldBounty(200)
		  ice:AddNewModifier(caster, self, "modifier_kill", {duration = self:GetSpecialValueFor("duration")})
		  ice:AddNewModifier(caster, self, "modifier_lich_ice_spire", {duration = self:GetSpecialValueFor("duration")})
		  ice:AddNewModifier(caster, self, "modifier_unit_remove", {duration = self:GetSpecialValueFor("duration")+1})
	...
	if ab and ab:GetLevel() > 0 and caster:TG_HasTalent("special_bonus_imba_lich_8") then
		local mod = ice:AddNewModifier(caster, ab, "modifier_imba_frost_armor", {duration = ab:GetSpecialValueFor("duration")})
		if mod then	
			mod:OnIntervalThink()
