...
function imba_witch_doctor_paralyzing_cask:OnSpellStart()
	...
	local next_pos = Vector(pos.x,pos.y,pos.z+200)
...
function imba_witch_doctor_paralyzing_cask:OnProjectileHit_ExtraData(target, location, keys)
	...
	local P1= {
		...
		Timers:CreateTimer(0.1, function() --弹跳延迟
			 ProjectileManager:CreateTrackingProjectile(P1)
		end)
	end
...
function modifier_imba_witch_doctor_voodoo_restoration:OnIntervalThink()
	if not IsServer() then return end
	if self:GetAbility() == nil   then return end
	local interval	= 1 --self:GetAbility():GetSpecialValueFor("heal_interval")
	...
	local heal = (self:GetAbility():GetSpecialValueFor("heal") + self:GetCaster():GetIntellect(false))*interval
...
function imba_witch_doctor_voodoo_switcheroo:OnSpellStart()
	...
	local num = 1 --caster:Has_Aghanims_Shard() and self:GetSpecialValueFor("num_shard") or  self:GetSpecialValueFor("num")
	self.ward = {}	--守卫集合
	for i = 1,num,1 do
...
function modifier_death_voodoo_handling:OnCreated()
	...
	self.intv = self.ab:GetSpecialValueFor("intv")
...
function modifier_death_voodoo_handling:OnIntervalThink()
	if not self.ab then return end
	if IsServer() then
		local units = FindUnitsInRadius(self.caster:GetTeamNumber(), self.parent:GetAbsOrigin(), nil, self.ab:GetSpecialValueFor("radius"), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_NOT_ATTACK_IMMUNE + DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE + DOTA_UNIT_TARGET_FLAG_NO_INVIS, 1, false)
		local bounces = 0
		--if self.caster:Has_Aghanims_Shard() then bounces = 3 end
		for _, unit in pairs(units) do
			...
			EmitSoundOn("Hero_WitchDoctor_Ward.Attack", self.parent)
			ProjectileManager:CreateTrackingProjectile(projectile)
			break
		end
	end
...
function modifier_death_voodoo_handling:OnAttackLanded(keys)
	...
	if self.caster ~= self.parent or self.caster:IsChanneling() then return  end
	if keys.attacker ~= self.parent or keys.target:IsOther()  or not keys.target:IsAlive() then
		return
	end
		local units = FindUnitsInRadius(self.caster:GetTeamNumber(), keys.target:GetAbsOrigin(), nil, 800, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_NOT_ATTACK_IMMUNE + DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE + DOTA_UNIT_TARGET_FLAG_NO_INVIS, 1, false)
		local bounces = 3
		for _, unit in pairs(units) do
		...
		if keys.target then
			self.Knockback.center_x =  self.caster:GetAbsOrigin().x	--施法者为中心
			self.Knockback.center_y =  self.caster:GetAbsOrigin().y	--施法者为中心
			self.Knockback.center_z =  self.caster:GetAbsOrigin().z	--施法者为中心

	  		keys.target:AddNewModifier(self.caster, self.ab, "modifier_knockback", self.Knockback)  --白牛的击退
			keys.target:AddNewModifier(self.caster, self.ab, "modifier_death_voodoo_handling_slow", {duration = 0.3})
		end
end
...
function modifier_death_ward_handling:OnIntervalThink()
	if IsServer() then
		local units = FindUnitsInRadius(self.caster:GetTeamNumber(), self.parent:GetAbsOrigin(), nil, 800, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_NOT_ATTACK_IMMUNE + DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE + DOTA_UNIT_TARGET_FLAG_NO_INVIS, 1, false)
		for _, unit in pairs(units) do
			self.projectile.Target = unit 			
			EmitSoundOn("Hero_WitchDoctor_Ward.Attack", self.parent)
...
function modifier_death_ward_handling:OnDestroy()
	if IsServer() then
		StopSoundEvent("Hero_WitchDoctor.Death_WardBuild", self.caster)
		UTIL_Remove(self.parent)
		self.caster:Stop()
	end
end
