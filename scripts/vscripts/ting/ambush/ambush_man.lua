...
LinkLuaModifier("modifier_ambush_man_debuff", "ting/ambush/ambush_man.lua", LUA_MODIFIER_MOTION_NONE)
...
function ambush_man:OnProjectileHit(target, location)
	...
	local damage = self:GetSpecialValueFor("damage")*self:GetCaster():GetLevel()
	target:AddNewModifier(self:GetCaster(),self,"modifier_ambush_man_debuff",{duration = 6})
...
modifier_ambush_man_debuff = class({})
function modifier_ambush_man_debuff:IsPurgable()return false
end
function modifier_ambush_man_debuff:IsPurgeException()return false
end
function modifier_ambush_man_debuff:IsHidden()return false
end

function modifier_ambush_man_debuff:DeclareFunctions()
	return {
			MODIFIER_PROPERTY_PHYSICAL_ARMOR_BASE_PERCENTAGE,
			}
end

function modifier_ambush_man_debuff:GetModifierPhysicalArmorBase_Percentage()
		return 75
end