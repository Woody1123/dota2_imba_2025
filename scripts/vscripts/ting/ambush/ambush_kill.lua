...
function ambush_kill:OnSpellStart()
	...
	local rocket_particle = ParticleManager:SafeCreateParticle("particles/tgp/sniper/rpg_m.vpcf", PATTACH_CUSTOMORIGIN, nil)
...
function ambush_kill:OnProjectileHit_ExtraData(target, location, keys)
	...
	local enemies = FindUnitsInRadius(caster:GetTeamNumber(), location, nil, radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_ANY_ORDER, false)
		for _, enemy in pairs(enemies) do
			...
			damageTable.damage = damage*(1+math.min(self:GetCaster():GetSpellAmplification(false)*0.5,1))
			ApplyDamage(damageTable)
		end
	if caster:Has_Aghanims_Shard() then
		 AddFOWViewer(caster:GetTeamNumber(), location, radius, 8, false)
	end
...
function modifier_ambush_kill_armor:OnCreated()
	...
	if IsServer() then
		local pfx = ParticleManager:SafeCreateParticle("particles/units/heroes/hero_dazzle/dazzle_armor_enemy.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
