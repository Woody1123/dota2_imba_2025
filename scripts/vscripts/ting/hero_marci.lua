	...
	if IsServer() then
		...
		self.parent:EmitSound("Hero_MonkeyKing.TreeJump.Tree")
			...
			if shard_c and self.use_cd then 
				self.ability:UseResources(false,false, false,self.use_cd)	
				self.tar = nil
				else
			...
			--print("no nin")
				...
				--print("intable")
					if self.tar:TG_TriggerSpellAbsorb(self.ability) then
...
function modifier_imba_marci_2_jump:OnOrder(keys)
	...
	if keys.unit==self.parent then
		if  keys.order_type == DOTA_UNIT_ORDER_HOLD_POSITION then
			...
			local direction = (pos - self.parent:GetAbsOrigin()):Normalized()
				direction.z = 0
			local distance = (self.parent:GetAbsOrigin() - pos):Length2D()
			local tralve_duration = 0.2
			
			if self.parent:HasModifier("modifier_imba_marci_2_jump") then
				if self.marci3 and self.marci3:GetLevel() > 0 then
					local pos_z = self.parent:FindModifierByName("modifier_imba_marci_2_jump").next_pos_z
				--print(tostring(self.caster:GetAbsOrigin().z))
				self.parent:AddNewModifier(self.caster, self.marci3, "modifier_imba_marci_3_down", {duration = 0.2, height = pos_z,direction = direction,dis = distance,pos_x = pos.x, pos_y = pos.y, pos_z = pos_z,isstune = 0})
				self.parent:RemoveModifierByName("modifier_imba_marci_2_jump")
				end
...
function modifier_imba_marci_2_jump:OnCreated(keys)
	if IsServer() then
		if self:GetAbility()~= nil  then
			...
			self.buff_dur = self.ability:GetSpecialValueFor("duration")
			self.buff_t =  1 + self.caster:TG_GetTalentValue("special_bonus_imba_marci_t4")*0.01
			
			self.caster:AddNewModifier(self.caster,self.ability,"modifier_imba_marci_2_buff",{duration = self.buff_dur*self.buff_t})
			...
			self.marci3 = self.caster:FindAbilityByName("imba_marci_3")
...
function modifier_imba_marci_3_down:OnCreated(keys)
	if IsServer() then
		if self:GetAbility()~= nil  then
			...
			self.duration = keys.duration
			self.isstun = keys.isstune or 1
			print(tostring(self.isstun))
			self.pos = Vector(keys.pos_x, keys.pos_y, keys.pos_z)
			self.dis = keys.dis+200
			local dis_t =(self.dis/self.duration)
			self.distance = dis_t*FrameTime()
			...
			self.stun = (self.isstun == 1) and self.stun + self.height_ex*0.3 or 0 
			self.slow = (self.isstun == 0) and 2 or 0
...
function modifier_imba_marci_3_down:OnDestroy()
	if IsServer() then
		...
		if ab and ab:GetLevel() > 0 then
			ab:UseResources(false, false, false, self.use_cd)
		end		
		local shard_c = self.parent:FindModifierByName("modifier_imba_marci_2_shard_buff")
		local shard_tar = nil
		if shard_c then 
	...
	local enemies = FindUnitsInRadius( self:GetCaster():GetTeamNumber(),self.parent:GetAbsOrigin(), nil, self.impact_radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO+DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NONE, FIND_CLOSEST, false )
			for _,enemy in pairs( enemies ) do
				...
				enemy:AddNewModifier(self.parent,self.ability,"modifier_imba_stunned",{duration = self.stun})
				enemy:AddNewModifier(self.parent,self.ability,"modifier_imba_marci_5_slow",{duration = self.slow})
				self.damageInfo.victim = enemy
				self.damageInfo.damage = self.damage
				ApplyDamage( self.damageInfo )
...
function imba_marci_1:OnSpellStart()
	...
	self.caster:AddNewModifier(self.caster,self,"modifier_imba_marci_1_buff",{duration = self.duration*self.buff_t})
...
function modifier_imba_marci_1_move:OnCreated(params)
	...
	self.distance = (self.talent == true) and self.parent:Script_GetAttackRange()*self.caster:TG_GetTalentValue("special_bonus_imba_marci_t5") or self.base_dis
...
function modifier_imba_marci_4_ex:OnDestroy(params)
	...
	self.buff_t =  1 + self.caster:TG_GetTalentValue("special_bonus_imba_marci_t4")*0.01 --天赋额外持续时间
	...
	if aab and aab:GetLevel() > 0 then
		local mod = self.caster:AddNewModifier(self.caster,aab,"modifier_imba_marci_1_buff",{duration = aab:GetSpecialValueFor("duration")*self.buff_t})
