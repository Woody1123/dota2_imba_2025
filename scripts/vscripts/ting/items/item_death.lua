...
LinkLuaModifier("modifier_imba_death_vision", "ting/items/item_death", LUA_MODIFIER_MOTION_NONE)
...
function item_imba_death:OnSpellStart()
	...
	--caster:AddNewModifier(caster,self,"modifier_smoke_of_deceit",{duration = duration})
	--local modifier = caster:AddNewModifier(caster,self,"modifier_imba_death_invisible",{duration = duration})
	
	if modifier~=nil and not caster:IsRangedAttacker() then
...
function modifier_imba_death_invisible:OnCreated()
	...
	self.vision_ra = self.ability:GetSpecialValueFor("vision_ra")+100
...
function modifier_imba_death_invisible:OnIntervalThink()
	if not IsServer() then return end
		local enemies = FindUnitsInRadius(self.parent:GetTeamNumber(), self.parent:GetAbsOrigin(), nil, self.vision_ra, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_INVULNERABLE + DOTA_UNIT_TARGET_FLAG_OUT_OF_WORLD, FIND_ANY_ORDER, false)
		if #enemies > 0 then
			for _, enemy in pairs(enemies) do	
			if enemy and enemy:IS_TrueHero_TG() then 
				self:SetStackCount(0)
				ex = false
				enemy:AddNewModifier(self.parent,self.ability,"modifier_imba_death_vision",{duration = self.vision_duration})
			end
		end
	end
...
--视野
modifier_imba_death_vision = class({})
function modifier_imba_death_vision:IsDebuff()			return true end
function modifier_imba_death_vision:IsHidden() 			return true end
function modifier_imba_death_vision:IsPurgable() 		return true end
function modifier_imba_death_vision:GetTexture()		return "item_death" end	
function modifier_imba_death_vision:DeclareFunctions() return {MODIFIER_PROPERTY_PROVIDES_FOW_POSITION} end
function modifier_imba_death_vision:GetModifierProvidesFOWVision() return 1 end
...
--function modifier_imba_death_passive:DeclareFunctions() return {MODIFIER_PROPERTY_PREATTACK_CRITICALSTRIKE,MODIFIER_EVENT_ON_ABILITY_EXECUTED} end
