...
function modifier_item_imba_linken_passive:GetAbsorbSpell(keys)
	...
	local fx = ParticleManager:SafeCreateParticle("particles/items_fx/immunity_sphere.vpcf", PATTACH_POINT_FOLLOW, self:GetParent())
...
function modifier_item_imba_linken_buff_other:OnCreated()
	...
	if IsServer() then
		...
		local fx = ParticleManager:SafeCreateParticle("particles/items_fx/immunity_sphere_buff.vpcf", PATTACH_POINT_FOLLOW, self.parent)
