...
function modifier_imba_mjollnir_ex_unique:OnAttackFail(keys)
	...
	local pfx = ParticleManager:SafeCreateParticle(particleName, PATTACH_ABSORIGIN_FOLLOW, keys.target)
...
function modifier_item_imba_mjollnir_ex_shield:OnTakeDamage(keys)
	...
	if self:GetStackCount() >= self.ability:GetSpecialValueFor("static_proc_count") then
		...
		for _, enemy in pairs(enemies) do
			local pfx = ParticleManager:SafeCreateParticle("particles/items2_fx/mjollnir_shield_arc_01.vpcf", PATTACH_POINT_FOLLOW, enemy)
