  ...
  -- 	ParticleManager:FireParticle("particles/units/heroes/hero_elder_titan/elder_titan_echo_stomp_magical.vpcf", PATTACH_POINT, caster, {[0] = point})
	...
	Timers:CreateTimer(0.6, function()
			...
			if not self:IsNull() then
				...
				if target  and IsValidEntity(target) and target:GetUnitName() ~= "npc_dota_r_d" and not target:IsBoss() and not target:IsHero() and target:IsDominated() then
					duration = duration_1
					...
					target = new_lane_creep
						...
						target:SetBaseMagicalResistanceValue(50)
				...
				target:AddNewModifier(caster, self, "modifier_unit_remove", {duration = duration+1})
				target:AddNewModifier(caster, self, "modifier_relic", {duration = duration})
				target:SetBaseMoveSpeed(550)
				target:SetHealth(20000)
				target:AddItemByName("item_octarine_core_v2")
				target:AddItemByName("item_basher")
				target:AddItemByName("item_monkey_king_bar")
	...
	--[[
	local particle= ParticleManager:SafeCreateParticle("particles/items4_fx/meteor_relic_spell_ground_impact.vpcf", PATTACH_ABSORIGIN,caster)
