...
function item_imba_crimson_guard:OnSpellStart()
	...
	local pfx = ParticleManager:SafeCreateParticle("particles/items2_fx/vanguard_active_launch.vpcf", PATTACH_CUSTOMORIGIN, caster)
...
LinkLuaModifier("modifier_item_greatwyrm_plate_active", "ting/items/item_vanguard", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_item_greatwyrm_plate_active_cd", "ting/items/item_vanguard", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_greatwyrm_plate_aura_t", "ting/items/item_vanguard", LUA_MODIFIER_MOTION_NONE)
...
modifier_imba_greatwyrm_plate_aura_t = class({})

LinkLuaModifier("modifier_imba_greatwyrm_plate_aura", "ting/items/item_vanguard", LUA_MODIFIER_MOTION_NONE)
function modifier_imba_greatwyrm_plate_aura_t:IsDebuff()			return false end
function modifier_imba_greatwyrm_plate_aura_t:AllowIllusionDuplicate()			return false end
function modifier_imba_greatwyrm_plate_aura_t:IsHidden() 			return true end
function modifier_imba_greatwyrm_plate_aura_t:IsPurgable() 		return false end
function modifier_imba_greatwyrm_plate_aura_t:IsPurgeException() 	return false end
function modifier_imba_greatwyrm_plate_aura_t:GetTexture() return "item_greatwyrm_plate_0" end
function modifier_imba_greatwyrm_plate_aura_t:IsAura()
	return true
end
function modifier_imba_greatwyrm_plate_aura_t:GetAuraDuration() return 1 end
function modifier_imba_greatwyrm_plate_aura_t:GetModifierAura() return "modifier_imba_greatwyrm_plate_aura" end
function modifier_imba_greatwyrm_plate_aura_t:GetAuraRadius()
	return 1200
end

function modifier_imba_greatwyrm_plate_aura_t:GetAuraSearchFlags() return DOTA_UNIT_TARGET_FLAG_NONE end
function modifier_imba_greatwyrm_plate_aura_t:GetAuraSearchTeam() return DOTA_UNIT_TARGET_TEAM_FRIENDLY end
function modifier_imba_greatwyrm_plate_aura_t:GetAuraSearchType() return DOTA_UNIT_TARGET_HERO end
...
function modifier_imba_greatwyrm_plate_passive:DeclareFunctions() return {MODIFIER_EVENT_ON_ATTACK_LANDED,MODIFIER_PROPERTY_INCOMING_DAMAGE_PERCENTAGE,MODIFIER_PROPERTY_PHYSICAL_CONSTANT_BLOCK, MODIFIER_PROPERTY_HEALTH_BONUS, MODIFIER_PROPERTY_HEALTH_REGEN_CONSTANT,MODIFIER_PROPERTY_MANA_BONUS, MODIFIER_PROPERTY_STATUS_RESISTANCE_STACKING,MODIFIER_PROPERTY_PHYSICAL_ARMOR_BONUS} end
...
function modifier_imba_greatwyrm_plate_passive:GetModifierStatusResistanceStacking() return self.statue_re*self:GetStackCount() end
function modifier_imba_greatwyrm_plate_passive:GetModifierPhysicalArmorBonus() return self.armor*self:GetStackCount() end
...
function modifier_imba_greatwyrm_plate_passive:OnCreated()
	...
	self.count = 1
	self.block = self:GetAbility():GetSpecialValueFor("base_damage_block")*0.01
...
function modifier_item_greatwyrm_plate_active:OnCreated()
	if IsServer() then
		local pfx = ParticleManager:SafeCreateParticle("particles/items2_fx/vanguard_active.vpcf", PATTACH_OVERHEAD_FOLLOW, self:GetParent())
