...
function modifier_seer_vision_think:OnCreated()
	...
	if IsServer() then
		self.particle4	= ParticleManager:SafeCreateParticle("particles/items/disk/disk_pfx0.vpcf", PATTACH_WORLDORIGIN, self.parent)
...
function modifier_imba_seer_of_disk_buff:OnCreated()
	...
	if IsServer() and not self.particle then
		...
		self.particle = ParticleManager:SafeCreateParticle("particles/items4_fx/combo_breaker_buff.vpcf", PATTACH_OVERHEAD_FOLLOW, self.parent)
...
function item_imba_crown:OnSpellStart()
	...
	self:GetCaster():EmitSound("Item.CrimsonGuard.Cast")
	    local pfx = ParticleManager:SafeCreateParticle("particles/items/disk/disk_pfx_7.vpcf", PATTACH_WORLDORIGIN, self:GetCaster())
