...
function item_imba_armlet_v2:OnSpellStart()
	...
	if caster:GetModifierStackCount("modifier_imba_armlet_v2_active_unique", nil) == 0 then
		caster:EmitSound("DOTA_Item.Armlet.Activate")
		buff:StartIntervalThink(0.4 / (self:GetSpecialValueFor("unholy_bonus_strength")+self:GetSpecialValueFor("ex_str")*caster:GetBaseStrength()/100))
		caster:CalculateStatBonus(true)
...
function modifier_imba_armlet_v2_active_unique:DeclareFunctions() return {MODIFIER_PROPERTY_EXTRA_STRENGTH_BONUS, MODIFIER_PROPERTY_PHYSICAL_ARMOR_BONUS, MODIFIER_PROPERTY_PREATTACK_BONUS_DAMAGE,MODIFIER_PROPERTY_STATS_STRENGTH_BONUS} end
...
function modifier_imba_armlet_v2_active_unique:GetModifierPreAttack_BonusDamage()
	return (self:GetStackCount() > 0 and self.ad or 0) end
...
function modifier_item_imba_armlet_v2_stacks:GetModifierPreAttack_BonusDamage() return self.ad * self:GetStackCount() end
...
if self:GetAbility() == nil then return end 
	...
	if IsServer() then
		...
		self:SetStackCount((self:GetParent():GetMaxHealth() - self:GetParent():GetHealth())/100)
		self:StartIntervalThink(1)
	end
end
...
function modifier_item_imba_armlet_v2_stacks:GetModifierPreAttack_CriticalStrike(keys)
	if IsServer() and keys.attacker == self:GetParent() and not keys.target:IsBuilding() and not keys.target:IsOther() then
		--[[
			if not keys.attacker:HasModifier("modifier_imba_armlet_v2_exhp") and keys.target ~= CDOTA_PlayerResource.ROSHAN then
			...
			return self.cri + ((self:GetParent():GetMaxHealth() - self:GetParent():GetHealth())/self:GetParent():GetMaxHealth()*100)*self.stack_cri
		else
			return 0
	end
end
...
function modifier_item_imba_armlet_v2_stacks:OnIntervalThink()
	--self:SetStackCount((self:GetParent():GetMaxHealth() - self:GetParent():GetHealth())/self:GetParent():GetMaxHealth()*100)
	self:SetStackCount((self:GetParent():GetMaxHealth() - self:GetParent():GetHealth())/100) --改为每100血量加层数
	--local hp_loss = self:GetParent():GetHealth()*self.hp_cost*0.01
	--如果当前血量小于臂章给与的力量，则不会继续掉血
	if self:GetParent():GetHealth()<= (self:GetAbility():GetSpecialValueFor("unholy_bonus_strength")+self:GetAbility():GetSpecialValueFor("ex_str")*self:GetParent():GetBaseStrength()/100)*20 then
		return
	end
	local hp_loss = self:GetParent():GetMaxHealth()*self.hp_cost*0.03
