...
function item_imba_pipe:OnSpellStart()
	...
	local pfx = ParticleManager:SafeCreateParticle("particles/items2_fx/pipe_of_insight_launch.vpcf", PATTACH_ABSORIGIN, caster)
...
function modifier_item_imba_pipe_block:OnCreated()
	...
	if IsServer() then
		...
		self.particle = ParticleManager:SafeCreateParticle("particles/items2_fx/pipe_of_insight.vpcf", PATTACH_OVERHEAD_FOLLOW, self.parent)
