...
LinkLuaModifier("modifier_imba_poison_touch_hex", "ting/hero_dazzle", LUA_MODIFIER_MOTION_NONE)
...
function imba_dazzle_poison_touch:GetAOERadius() 			return self:GetSpecialValueFor("radius")  end
...
function imba_dazzle_poison_touch:OnSpellStart()
	...
	self.info = 
		...
		local enemy = FindUnitsInRadius(self.caster:GetTeamNumber(), self.pos ,nil, self.radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NONE, FIND_CLOSEST, false)
		for i=1, #enemy do
			self.info.Target = enemy[i]
			ProjectileManager:CreateTrackingProjectile(self.info)
			--if self.caster:Has_Aghanims_Shard() then
			--	self.caster:PerformAttack(enemy[i], false, true, true, false, true, false, false)
			--end
		end
...
function imba_dazzle_poison_touch:OnProjectileHit(target, pos)
	...
	target:AddNewModifier(self.caster, self, "modifier_imba_stunned", {duration = 0.01})
		if not target:IsMagicImmune() and self.caster:Has_Aghanims_Shard() then
			if target:IsIllusion() then
				target:Kill(self, self.caster)
			else
				target:AddNewModifier_RS(self.caster, self, "modifier_imba_poison_touch_hex", {duration = 2})
			end
		end
...
function modifier_imba_poison_touch_slow:OnCreated()
	...
	self.damage_stack = self.ability:GetSpecialValueFor("damage_per")+self.caster:TG_GetTalentValue("special_bonus_imba_dazzle_2")*self.caster:GetIntellect(false)
	self.stun = self.ability:GetSpecialValueFor("stun_duration")+self.caster:TG_GetTalentValue("special_bonus_imba_dazzle_3")
...
modifier_imba_poison_touch_hex = class({})

function modifier_imba_poison_touch_hex:IsDebuff()				return true end
function modifier_imba_poison_touch_hex:IsHidden() 			return false end
function modifier_imba_poison_touch_hex:IsPurgable() 			return false end
function modifier_imba_poison_touch_hex:IsPurgeException() 	return true end
function modifier_imba_poison_touch_hex:CheckState() return { [MODIFIER_STATE_SILENCED] = true, [MODIFIER_STATE_MUTED] = true, [MODIFIER_STATE_DISARMED] = true, [MODIFIER_STATE_HEXED] = true} end

function modifier_imba_poison_touch_hex:DeclareFunctions()
...
function modifier_imba_poison_touch_hex:GetModifierModelChange() return "models/items/hex/fish_hex/fish_hex.vmdl" end
function modifier_imba_poison_touch_hex:GetModifierMoveSpeed_Absolute() return 100 end
function modifier_imba_poison_touch_hex:GetModifierMoveSpeed_Limit() return 100 end
function modifier_imba_poison_touch_hex:GetModifierMoveSpeed_Max() return 100 end

imba_dazzle_shallow_grave = class({})

LinkLuaModifier("modifier_imba_shallow_grave", "ting/hero_dazzle", LUA_MODIFIER_MOTION_NONE)
--LinkLuaModifier("modifier_imba_shallow_buff2", "ting/hero_dazzle", LUA_MODIFIER_MOTION_NONE)
...
function imba_dazzle_shallow_grave:Init()
	...
	self.pfx1 = "particles/econ/items/dazzle/dazzle_dark_light_weapon/dazzle_dark_shallow_grave.vpcf"
end
function imba_dazzle_shallow_grave:OnSpellStart()
	...
	--self.target:AddNewModifier(self.caster, self, "modifier_imba_shallow_buff2", {duration = self:GetSpecialValueFor("duration_tooltip")})
...
function modifier_imba_shallow_grave:OnDestroy()
	if IsServer() then
		...
		if self.caster:TG_HasTalent("special_bonus_imba_dazzle_6") then
			self.parent:ModifyHealth(self.parent:GetHealth()+self.parent:GetMaxHealth()*self.caster:TG_GetTalentValue("special_bonus_imba_dazzle_6")*0.01,self.ability,false,DOTA_DAMAGE_FLAG_REFLECTION)
		end
...
function modifier_imba_shallow_grave:GetDisableHealing()
    return 1
end

function modifier_imba_shallow_grave:GetAbsoluteNoDamageMagical()
    return 1
end

function modifier_imba_shallow_grave:GetAbsoluteNoDamagePhysical()
    return 1
end

function modifier_imba_shallow_grave:GetAbsoluteNoDamagePure()
    return 1
end
...
function imba_dazzle_shadow_wave:OnSpellStart()
	...
	self.shard =  caster:Has_Aghanims_Shard()
	self.damageTable = {
								attacker = caster,
								damage_type = self:GetAbilityDamageType(),
								damage_flags = DOTA_DAMAGE_FLAG_NONE, --Optional.
...
function modifier_imba_dazzle_weave_passive:OnAbilityFullyCast(keys)
	...
	if keys.ability~=nil and not keys.ability:IsToggle() and not keys.ability:IsItem() and keys.ability:GetCooldown(keys.ability:GetLevel())>1 then 

		local target = keys.ability:GetCursorTarget() or nil 
		local point = keys.ability:GetCursorPosition() or self.caster:GetAbsOrigin()
		if target~=nil then
...
function imba_dazzle_help:Init()
	self.pfx1 = "particles/heroes/ting_dazz/help.vpcf"
	self.pfx2 = "particles/units/heroes/hero_dazzle/dazzle_weave.vpcf"
	self.pfx3 = "particles/units/heroes/hero_dazzle/dazzle_weave_b.vpcf"
	self.sound1 = "Hero_Dazzle.Shadow_Wave"
end
...
function imba_dazzle_help:OnSpellStart()
	...
	Notifications:BottomToAll({text ="暗影牧师正在进行复活仪式！", duration = 1,style = {["font-size"] = "100px", color = "#CD2626"}})
end
