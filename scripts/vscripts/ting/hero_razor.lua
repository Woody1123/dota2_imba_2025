...
function modifier_imba_razor_plasma_field:Applydamage(tab)
	if IsServer() then 
		...
		if self.modifier~=nil and tab.victim:IsHero() then
			self.modifier:SetStackCount(math.min(self.modifier:GetStackCount()+1,self:GetParent():GetLevel()*2))
		end
	end
end
...
function modifier_imba_razor_static_link_att:DeclareFunctions()
	return {
		...
		MODIFIER_EVENT_ON_ORDER,MODIFIER_EVENT_ON_ATTACK
	}
end
...
function imba_razor_unstable_current:OnSpellStart()
	if IsServer() then
		...
		if mod~=nil then
			local count = mod:GetStackCount()*(self:GetSpecialValueFor("sh")+self:GetCaster():TG_GetTalentValue("special_bonus_imba_razor_6"))
			local buff = self:GetCaster():FindModifierByName("imba_razor_unstable_current_armor")
			if buff then
				count = count + buff:GetStackCount()
				self:GetCaster():RemoveModifierByName("imba_razor_unstable_current_armor")
			end
			local modifier = self:GetCaster():AddNewModifier(self:GetCaster(),self,"imba_razor_unstable_current_armor",{duration = -1,count = count})
...
function modifier_imba_razor_unstable_current_passive:Applydamage(tab)
	...
	ApplyDamage(tab)
		...
		if modifier ~= nil then
			modifier:SetStackCount(math.min(modifier:GetStackCount()+1,self:GetParent():GetLevel()*2))
		end
end
...
function imba_razor_unstable_current_armor:OnCreated(keys)
	if self:GetAbility() == nil then return end
	--print("create")
	if IsServer() then
		self:GetParent():EmitSound("DOTA_Item.Mjollnir.DeActivate")
		local armor = keys.count
		self.shield_count = keys.count
...
function imba_razor_unstable_current_armor:GetModifierIncomingDamageConstant(keys)
...
function imba_razor_eye_of_the_storm:OnSpellStart()
	...
	for _,v in pairs(modifiers) do
		if v:GetName() == "modifier_imba_razor_eye_of_the_storm" then
			count = count+1
		end
	end
	if count < 2 then
...
function modifier_imba_razor_eye_of_the_storm:Findenmys()
	if IsServer() then
		...
		local radius = math.min(self.caster:Script_GetAttackRange()+self.caster:GetCastRangeBonus(),950)
		local enemytab = {}
		local enemies = FindUnitsInRadius(self.caster:GetTeamNumber(), self.caster:GetAbsOrigin(), nil,radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_CLOSEST, false)
		for _,enemy in pairs(enemies) do
		...
		local enemies2 = FindUnitsInRadius(self.caster:GetTeamNumber(), self.caster:GetAbsOrigin(), nil,radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_CLOSEST, false)
		for _,enemy in pairs(enemies2) do
			if not self:IsInTable(enemy,enemytab) then
			table.insert(enemytab, enemy)
			targets = targets -1
		...
		local enemies3 = FindUnitsInRadius(self.caster:GetTeamNumber(), self.caster:GetAbsOrigin(), nil,radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_CLOSEST, false)
		...
		local enemies3 = FindUnitsInRadius(self.caster:GetTeamNumber(), self.caster:GetAbsOrigin(), nil,radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_BUILDING, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_CLOSEST, false)
		for _,enemy in pairs(enemies3) do
			if not self:IsInTable(enemy,enemytab) then
			table.insert(enemytab, enemy)
			targets = targets -1
...
function modifier_imba_razor_eye_of_the_storm:Applydamage(tab)
	if IsServer() then
		if tab.victim:IsAlive() and not tab.victim:IsBuilding() then
			tab.victim:SetHealth(math.max(1,tab.victim:GetHealth()-tab.victim:GetMaxHealth()*0.01))			
		end
		if self.modifier~=nil and tab.victim:IsHero() then
			self.modifier:SetStackCount(math.min(self.modifier:GetStackCount()+1,self:GetParent():GetLevel()*2))
		end 
...
function imba_razor_whip:OnSpellStart()
	...
	if modifier ~= nil then
			...
			count2 = math.ceil(modifier:GetStackCount()/2)*per

			local buff = self:GetCaster():FindModifierByName("imba_razor_unstable_current_armor")
			if buff and caster:TG_HasTalent("special_bonus_imba_razor_3") then
				count2 = count2 + buff:GetStackCount()
				self:GetCaster():RemoveModifierByName("imba_razor_unstable_current_armor")
			end
			self:GetCaster():AddNewModifier(self:GetCaster(),self,"imba_razor_unstable_current_armor",{duration = -1,count = count2})
	end
...
function imba_razor_whip:Applydamage(tab,mod,count)
	if not IsServer() then return end
	ApplyDamage(tab)
