...
function assassinate:CastFilterResultTarget(target)
	if IsServer() then

		if target~=nil then 
			--print(tostring(target:GetName()))
			if string.find(target:GetUnitName(), "npc_sniper_bait") and self:GetCaster():Has_Aghanims_Shard() then
				return UF_SUCCESS
			end
			
			if Is_Chinese_TG(self:GetCaster(),target) or target:IsMagicImmune() or target:IsBuilding()  then
...
function assassinate:GetCustomCastErrorTarget(target)
	if Is_Chinese_TG(self:GetCaster(),target) or target:IsMagicImmune() or target:IsBuilding()  then
				return "不能以此单位为目标"
	end
end
...
function assassinate:OnProjectileHit_ExtraData(target, location, keys)
	if target then
		...
		if (target:IsCreep() or  target:IsNeutralUnitType() ) and not target:IsBoss() and not target:IsAncient()  then
			...
			local damage = self:GetSpecialValueFor("damage_att")*caster:GetAgility()
			local damageTable = {
								attacker = caster,
								damage_type = self:GetAbilityDamageType(),
								ability = self, --Optional.
								}
			if target:IsHero() then --target:IsElite()
				local Knockback ={
				  should_stun = 0, --打断
				...
				local cast_angle = VectorToAngles(target:GetForwardVector() * -1)
				...
				if degree <= 110 then 
					damage = damage + (caster:GetStrength() + caster:GetIntellect(false) + caster:GetAgility())*self:GetSpecialValueFor("all_s")
					target:AddNewModifier(caster,self,"modifier_imba_stunned",{duration = self:GetSpecialValueFor("stun")})
				end
				
			end
			
			if string.find(target:GetUnitName(), "npc_sniper_bait") then
				target:Kill(self, caster)
				self:EndCooldown()
			end
