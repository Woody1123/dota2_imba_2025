...
function modifier_imba_treant_living_armor_buff:OnRefresh()
	...
	local trees = 10--GridNav:GetAllTreesAroundPoint( self:GetParent():GetOrigin(), self:GetAbility():GetSpecialValueFor("radius"), false )
	local heal = self:GetParent():GetMaxHealth()*0.01*25
...
function imba_treant_overgrowth:OnSpellStart()
	...
	local newpos =RotatePosition(caster:GetAbsOrigin(), QAngle(0, tr*72, 0), caster:GetAbsOrigin() + caster:GetForwardVector():Normalized() * 600)
		for i = 1,5,1 do
			...
			FindUnitsInRadius(self.caster:GetTeamNumber(),
...
function imba_treant_eyes_in_the_forest:OnSpellStart()
	...
	--tree:AddNewModifier(self:GetCaster(),self,"modifier_treant_eyes_in_the_forest",{duration = -1 ,pos_x = pos.x,pos_y = pos.y,po_z = pos.z})  --不知道为什么注释掉树眼就能用了，HG：2022-6-25 13:47:52
...
function modifier_imba_treant_natures_guise_passive:OnIntervalThink()
	...
	--GridNav:IsNearbyTree(self:GetParent():GetAbsOrigin(),self:GetAbility():GetSpecialValueFor("radius"), true)
