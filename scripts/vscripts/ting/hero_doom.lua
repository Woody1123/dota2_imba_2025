...
function imba_doom_bringer_doom:IsRefreshable() 			return false end
...
function imba_doom_bringer_doom:OnSpellStart()
	...
	local duration = duration_long
	if Is_Chinese_TG(target,caster) then
		EmitSoundOn( "TG.DOOM", caster )
		duration = 6
		if caster == target then duration = 12 
			self:EndCooldown()
			self:StartCooldown(66)
		end
		target:AddNewModifier(caster,self,"modifier_imba_doom_bringer_scorched_earth_passive_scepter_ex", {duration = duration})
		else
...
function imba_doom_bringer_infernal_blade:CastFilterResultTarget(hTarget)
	...
	if IsServer() then
		...
		if  hTarget:IsCourier() or hTarget==CDOTA_PlayerResource.ROSHAN then
...
function modifier_imba_doom_bringer_infernal_blade:OnAttackLanded(keys)
	if IsServer() then		
		if keys.target:GetTeamNumber() == keys.attacker:GetTeamNumber() or keys.target:IsOther() or not keys.target:IsAlive() or keys.target ==CDOTA_PlayerResource.ROSHAN then return  end
		...
		self.parent:IsIllusion() and not keys.target:IsBuilding() and not keys.target:IsOther() then		
				...
				self.ab:UseResources(true, false, true, true) 
					end
				end
		end
...
LinkLuaModifier("modifier_imba_doom_bringer_scorched_earth_passive_scepter_ex", "ting/hero_doom", LUA_MODIFIER_MOTION_NONE)
...
function imba_doom_bringer_scorched_earth:OnSpellStart()
	...
	local doom = caster:FindAbilityByName("imba_doom_bringer_doom") 
	if doom and doom:GetLevel() > 0  then
		if RollPseudoRandomPercentage(6.6,0,caster) then
			EmitSoundOn( "TG.DOOM", caster )
			caster:AddNewModifier(caster,doom,"modifier_imba_doom_bringer_scorched_earth_passive_scepter_ex", {duration = self:GetSpecialValueFor("duration_scepter")+0.1})
		end
	end	
end

--隐藏大招
modifier_imba_doom_bringer_scorched_earth_passive_scepter_ex= class({})
LinkLuaModifier("modifier_imba_doom_enemy", "ting/hero_doom", LUA_MODIFIER_MOTION_NONE)
function modifier_imba_doom_bringer_scorched_earth_passive_scepter_ex:IsDebuff()			return false end
function modifier_imba_doom_bringer_scorched_earth_passive_scepter_ex:IsHidden() 			return false end
function modifier_imba_doom_bringer_scorched_earth_passive_scepter_ex:IsPurgable() 		return false end
function modifier_imba_doom_bringer_scorched_earth_passive_scepter_ex:IsPurgeException() 	return false end
function modifier_imba_doom_bringer_scorched_earth_passive_scepter_ex:RemoveOnDeath() 	return true end
--function modifier_imba_doom_bringer_scorched_earth_passive_scepter_ex:RemoveOnDeath() 		return false end
function modifier_imba_doom_bringer_scorched_earth_passive_scepter_ex:GetEffectName() return "particles/heroes/doom/doom_bringer_ex_doom.vpcf" end


function modifier_imba_doom_bringer_scorched_earth_passive_scepter_ex:GetEffectAttachType() return PATTACH_ABSORIGIN_FOLLOW end
function modifier_imba_doom_bringer_scorched_earth_passive_scepter_ex:DeclareFunctions() 
	return	{MODIFIER_PROPERTY_INCOMING_DAMAGE_PERCENTAGE,MODIFIER_PROPERTY_STATUS_RESISTANCE_STACKING} 
end 

function modifier_imba_doom_bringer_scorched_earth_passive_scepter_ex:GetModifierIncomingDamage_Percentage() return -66 end
function modifier_imba_doom_bringer_scorched_earth_passive_scepter_ex:GetModifierStatusResistanceStacking() return 66 end
function modifier_imba_doom_bringer_scorched_earth_passive_scepter_ex:IsAura() return true end
function modifier_imba_doom_bringer_scorched_earth_passive_scepter_ex:GetAuraDuration() return 1 end
function modifier_imba_doom_bringer_scorched_earth_passive_scepter_ex:GetModifierAura() return "modifier_imba_doom_enemy" end
function modifier_imba_doom_bringer_scorched_earth_passive_scepter_ex:GetAuraRadius() return 666 end
function modifier_imba_doom_bringer_scorched_earth_passive_scepter_ex:GetAuraSearchFlags() return DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES end
function modifier_imba_doom_bringer_scorched_earth_passive_scepter_ex:GetAuraSearchTeam() return DOTA_UNIT_TARGET_TEAM_ENEMY end
function modifier_imba_doom_bringer_scorched_earth_passive_scepter_ex:GetAuraSearchType() return DOTA_UNIT_TARGET_HERO end
function modifier_imba_doom_bringer_scorched_earth_passive_scepter_ex:OnCreated()
	if self:GetAbility() == nil then return end
	if IsServer() then
		self:GetParent():EmitSound("Hero_DoomBringer.Doom")
	end
end
function modifier_imba_doom_bringer_scorched_earth_passive_scepter_ex:OnDestroy()
	if IsServer() then
		self:GetParent():StopSound("Hero_DoomBringer.Doom")
	end
end
...
LinkLuaModifier("imba_doom_kobold_taskmaster_speed_aura","ting/hero_doom", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("imba_doom_kobold_taskmaster_speed_aura_speed","ting/hero_doom", LUA_MODIFIER_MOTION_NONE)
...
function imba_doom_bringer_devour:CastFilterResultTarget( hTarget )
	if IsServer() then
		...
		--if hTarget:IsBoss() and self:GetLevel()~=4 then return UF_FAIL_CUSTOM end  --四级吃肉山
		if hTarget:IsBoss()  then return UF_FAIL_CUSTOM end
		--if self:GetCaster()==hTarget then
		--	self:ToggleAutoCast()
		--end
...
--[[	tab_hero_name={"npc_dota_hero_beastmaster",
	...
	{"kobold_taskmaster_speed_aura","imba_doom_kobold_taskmaster_speed_aura"},
...
--	self.target:AddNoDraw()	
		...
		if self.target:HasModifier("modifier_imba_doom_bringer_infernal_blade_debuff") then 
				...
				local over_damage = (self:GetSpecialValueFor("burn_damage")+(self:GetSpecialValueFor("burn_damage_pct")*self.target:GetMaxHealth()/100))*count*2.66
...
function imba_doom_bringer_devour:devour_ability()
	...
	local abilityName = {}
		for i = 0,2 do
			...
			if ability_target~= nil and ability_target:GetName() ~= "neutral_upgrade" then
	...
	self.new1:SetLevel(self:GetLevel())
	--self.new1:SetLevel(self.ability0:GetLevel())
			...
			if self.ability1== nil then 
				...
				self.new2:SetLevel(self:GetLevel())
...
function imba_doom_ghost_frost_attack:IsHidden() 			return false end
...
---------------------------------------狗头人光环------------------------------
imba_doom_kobold_taskmaster_speed_aura= class({})
LinkLuaModifier("imba_doom_kobold_taskmaster_speed_aura_speed","ting/hero_doom", LUA_MODIFIER_MOTION_NONE)
function imba_doom_kobold_taskmaster_speed_aura:IsAura() return true end
function imba_doom_kobold_taskmaster_speed_aura:GetAuraDuration() return 1 end
function imba_doom_kobold_taskmaster_speed_aura:IsHidden() 			return true end
function imba_doom_kobold_taskmaster_speed_aura:DeclareFunctions()
    return
    {
...
function imba_doom_kobold_taskmaster_speed_aura:RemoveOnDeath()			return false end
function imba_doom_kobold_taskmaster_speed_aura:GetModifierAura() return "imba_doom_kobold_taskmaster_speed_aura_speed" end
function imba_doom_kobold_taskmaster_speed_aura:GetAuraRadius() return 1200 end
function imba_doom_kobold_taskmaster_speed_aura:GetAuraSearchFlags() return DOTA_UNIT_TARGET_FLAG_NONE end
function imba_doom_kobold_taskmaster_speed_aura:GetAuraSearchTeam() return DOTA_UNIT_TARGET_TEAM_FRIENDLY end
function imba_doom_kobold_taskmaster_speed_aura:GetAuraSearchType() return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC end

function imba_doom_kobold_taskmaster_speed_aura:GetModifierMoveSpeed_Limit()
    return 1200
end

function imba_doom_kobold_taskmaster_speed_aura:GetModifierIgnoreMovespeedLimit()
    return 1
end
function imba_doom_kobold_taskmaster_speed_aura:GetModifierMoveSpeedReductionPercentage() --减少幅度为  减速*返回值*0.01  负数减速变加速
    return 50
end

imba_doom_kobold_taskmaster_speed_aura_speed= class({})
function imba_doom_kobold_taskmaster_speed_aura_speed:IsDebuff()			return false end
function imba_doom_kobold_taskmaster_speed_aura_speed:IsHidden() 			return false end
function imba_doom_kobold_taskmaster_speed_aura_speed:IsPurgable() 			return false end
function imba_doom_kobold_taskmaster_speed_aura_speed:IsPurgeException() 	return false end
function imba_doom_kobold_taskmaster_speed_aura_speed:DeclareFunctions() return {MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE} end
function imba_doom_kobold_taskmaster_speed_aura_speed:GetModifierMoveSpeedBonus_Percentage() 
	if self.speed~=nil then
		return self.speed
	end
end

function imba_doom_kobold_taskmaster_speed_aura_speed:OnCreated() 
	if self:GetAbility() == nil then return end
	self.speed=24
end

function imba_doom_kobold_taskmaster_speed_aura_speed:OnRemoved() 
	self.speed=nil
end
