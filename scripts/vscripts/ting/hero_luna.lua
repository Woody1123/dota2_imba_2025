...
function imba_luna_lucent_beam:OnSpellStart()
	...
	local pos = self:GetCursorPosition()
		if self:GetCursorTarget() then
			...
			self:start(true,target,false,false,true)
		else
			AddFOWViewer(self:GetCaster():GetTeamNumber(), pos, self:GetSpecialValueFor("v_radius"), self:GetSpecialValueFor("v_duration"), false)	--500高空视野
			local target = CreateModifierThinker(caster, self, "modifier_dummy_thinker", {duration = 1}, pos, caster:GetTeamNumber(), false)
			self:start(true,target:entindex(),true,false,true)
		end
end


function imba_luna_lucent_beam:start(vision,tar,bool,bool_u,bool_stun)	--目标位置，目标，是否是点地放的，是否是大招/触发月幕
	local caster = self:GetCaster()
	local target = tar
	local enemy_count = self:GetSpecialValueFor("enemy_count")
	local enemy_flag = true --是否是生物
	local damage = self:GetSpecialValueFor("beam_damage")+caster:TG_GetTalentValue("special_bonus_imba_luna_2")
	...
	if bool_u then
		...
		if bool_stun then
			stun_duration = caster:TG_HasTalent("special_bonus_imba_luna_4") and  caster:TG_GetTalentValue("special_bonus_imba_luna_4")-0.15 or 0
			else
			stun_duration = 0
		end	
	end
	...
	if bool then
		...
		enemy_flag = false
		if not bool_u then
		local enemies1 = FindUnitsInRadius(caster:GetTeamNumber(), target:GetAbsOrigin(), nil, r_radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO+DOTA_UNIT_TARGET_BASIC , DOTA_UNIT_TARGET_FLAG_NO_INVIS, FIND_CLOSEST, false)
	...
	if enemy_flag and not bool_u then
		local enemies2 = FindUnitsInRadius(caster:GetTeamNumber(), target:GetAbsOrigin(), nil, 500, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO+DOTA_UNIT_TARGET_BASIC+DOTA_UNIT_TARGET_BUILDING , DOTA_UNIT_TARGET_FLAG_NONE, FIND_CLOSEST, false)
		local mod = caster:FindModifierByName("modifier_imba_luna_moon_glaive")
...
function imba_luna_moon_glaive:GlaiveAttck(source, damage, bounce)
	local target = nil
	local enemies = FindUnitsInRadius(self:GetCaster():GetTeamNumber(), source:GetAbsOrigin(), nil, self.range, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC + DOTA_UNIT_TARGET_BUILDING, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_NO_INVIS  + DOTA_UNIT_TARGET_FLAG_NOT_ATTACK_IMMUNE, FIND_ANY_ORDER, false)
...
function imba_luna_moon_glaive:OnProjectileHit_ExtraData(target, location, keys)
	local damage_reduction  = self.damage_reduction --(GameRules:IsDaytime() or target:IsBuilding()) and self.damage_reduction or 0
	...
	if target then
		...
		self.damageTable.damage = target:IsBuilding() and damage*0.5 or damage
...
function modifier_imba_luna_moon_glaive:DeclareFunctions() return {MODIFIER_EVENT_ON_ATTACK_START,MODIFIER_EVENT_ON_ATTACK_LANDED,MODIFIER_EVENT_ON_DEATH,MODIFIER_PROPERTY_ATTACK_RANGE_BONUS} end
function modifier_imba_luna_moon_glaive:OnAttackStart(tg)
...
function modifier_imba_luna_moon_glaive:OnAttackLanded(keys)
	...
	if keys.attacker ~= self:GetParent() or keys.target:IsOther() or self:GetParent():PassivesDisabled()  or not keys.target:IsAlive() or self.miss == false then
		return
	end
		...
		if ability and ability:GetLevel() > 0 then
			...
			for _,enemy in pairs(enemies) do
				--if not enemy:IsInvisible() then
					ability:start(true,enemy,false,true,true)
				...
				ability:start(true,caster,false,true,true)
...
function modifier_lunar_blessing:DeclareFunctions() return {MODIFIER_PROPERTY_BONUS_NIGHT_VISION,MODIFIER_PROPERTY_PREATTACK_BONUS_DAMAGE} end
function modifier_lunar_blessing:GetModifierPreAttack_BonusDamage()
		return self:GetCaster():TG_GetTalentValue("special_bonus_imba_luna_3")+self:GetAbility():GetSpecialValueFor("bonus_all")
end
...
function imba_luna_eclipse:OnSpellStart()
	...
	if not self:GetAutoCastState() then
		local cooldown_remaining = 0.5*self:GetCooldown(self:GetLevel())*caster:GetCooldownReduction()
		self:EndCooldown()
...
function modifier_luna_eclipse_thinker:OnCreated(keys)
	...
	if IsServer() then
		self.auto = self:GetAbility():GetAutoCastState()
		...
		if self.auto and caster:HasModifier("modifier_imba_luna_moon_glaive") then
			self.interval = self:GetDuration()/(self.hits+caster:FindModifierByName("modifier_imba_luna_moon_glaive"):GetStackCount())
		end
...
function modifier_luna_eclipse_thinker:OnIntervalThink()
	...
	for _,enemy in pairs(enemies) do
		if not enemy:HasModifier("modifier_black_king_bar_immune") then
			if self.auto then
				self.lucent:start(nil,enemy,false,true,false)
				else
				self.lucent:start(nil,enemy,false,true,true)
			end			
	...
	if enemy_count then
			local enemies = FindUnitsInRadius(caster:GetTeamNumber(), parent:GetAbsOrigin(), nil, self.radius, DOTA_UNIT_TARGET_TEAM_ENEMY, search_flag , DOTA_UNIT_TARGET_FLAG_NO_INVIS,search_type, false)
		for _,enemy in pairs(enemies) do
			if self.auto then
				self.lucent:start(nil,enemy,false,true,false)
				else
