...
function modifier_goo_stack:DeclareFunctions() return {MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE,MODIFIER_PROPERTY_PHYSICAL_ARMOR_BONUS} end
...
function modifier_goo_auto:OnIntervalThink()
    if self:GetCaster():IsAlive() and  self.ab and self.ab:GetAutoCastState() and self.ab:GetLevel()>0 and self.ab:IsOwnersManaEnough() and self.ab:IsCooldownReady()  then
        ...
        self.ab:UseResources(true, false, false, true)
...
function modifier_imba_bristleback_passive:AllowIllusionDuplicate() return false end
...
function modifier_imba_bristleback_passive:GetModifierIncomingDamage_Percentage(keys)
	if not IsServer() or self.parent:PassivesDisabled() or keys.attacker:IsBuilding() or (bit.band(keys.damage_flags, DOTA_DAMAGE_FLAG_REFLECTION) == DOTA_DAMAGE_FLAG_REFLECTION) then
		return 0
	end
	...
	if degree <= self.min_degree or self:GetParent():HasModifier("modifier_warpath_taunt") then
		local reduce = 0
...
function modifier_imba_ass:GetModifierMoveSpeed_Limit() return 350 end
...
function imba_bristleback_hairball:OnSpellStart()
	...
	direction.z = 0
	local pfxname ="particles/units/heroes/hero_ancient_apparition/ancient_apparition_ice_blast_final.vpcf" 
