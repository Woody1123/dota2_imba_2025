...
LinkLuaModifier("imba_huskar_inner_fire_yidong", "ting/hero_huskar", LUA_MODIFIER_MOTION_NONE)
...
function imba_huskar_inner_fire:CastFilterResultLocation(vLocation)  --缠绕不能释放
	if self:GetCaster():IsRooted() or self:GetCaster():IsStunned() or self:GetCaster():IsSilenced() then
...
function imba_huskar_inner_fire_yidong:OnDestroy()
	...
	self:GetAbility():InnerFire(0,false,true,0)
	self.direction = nil
	self.speed = nil
	self.caster = nil
	self.pos = nil
...
function modifier_imba_berserkers_blood_passive:DeclareFunctions() return {MODIFIER_PROPERTY_MAGICAL_RESISTANCE_BONUS,MODIFIER_PROPERTY_INCOMING_DAMAGE_PERCENTAGE,MODIFIER_PROPERTY_MOVESPEED_BONUS_CONSTANT,MODIFIER_PROPERTY_ATTACKSPEED_BONUS_CONSTANT,MODIFIER_PROPERTY_TRANSLATE_ACTIVITY_MODIFIERS,MODIFIER_PROPERTY_MODEL_SCALE} end
...
function modifier_imba_berserkers_blood_passive:GetModifierMagicalResistanceBonus()
	return  (self.parent:PassivesDisabled() and 0 or (self.parent:TG_GetTalentValue("special_bonus_imba_huskar_4")+self.ab:GetSpecialValueFor("mag")* (self:GetStackCount() / 100)))
end
function modifier_imba_berserkers_blood_passive:OnCreated()	
	if self:GetAbility() == nil then return end
			...
			if IsServer()then	
				self.current = self.parent:GetHealth() - ( self.soulmax/ 100) * self.parent:GetMaxHealth()
...
function modifier_imba_berserkers_blood_passive:OnIntervalThink()
	if IsServer() then
		...
		if self.caster:HasModifier("modifier_imba_huskar_armlet") then
			self.pct = math.min(self.pct+50,100)
		end
		ParticleManager:SetParticleControl(self.pfx, 1, Vector(self.pct, 0, 0))
		self:SetStackCount(self.pct)
		if self.caster:IsAlive() then 
			local heal = self.pct*0.01*self.parent:GetStrength()*self.ab:GetSpecialValueFor("maximum_health_regen")*0.005
...
function imba_huskar_life_break:CastFilterResultTarget(target)
	if IsServer() then
		...
		if target:IsMagicImmune() and not self:GetCaster():HasScepter() then return UF_FAIL_MAGIC_IMMUNE_ENEMY end
...
function modifier_imba_huskar_life_break_yidong:OnDestroy()
	...
	if ability~=nil then
		if ability:GetLevel()>0 then
			ability:InnerFire(0,false,true,0)
		end
	end
	self.caster = nil
	self.ability = nil
