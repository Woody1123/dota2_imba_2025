...
function laser_gun:OnSpellStart()
    ...
    if caster:GetModifierStackCount("modifier_laser_cannon_ch", nil)>=self:GetSpecialValueFor("stack") then
        num=num+math.floor(caster:GetModifierStackCount("modifier_laser_cannon_ch", nil)/self:GetSpecialValueFor("stack"))
    end
   -- print(num)
